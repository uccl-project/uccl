...
def test_main(num_tokens: int, hidden: int, num_experts: int, num_topk: int,
    ...
    def test_func(return_recv_hook: bool):
        ...
        # --- Phase alignment after dispatch --- #
    ...
    dist.barrier(group=group); torch.cuda.synchronize()
    print(f'[rank {rank}] new Dispatch + combine bandwidth: {(num_dispatch_comm_bytes + num_combine_comm_bytes) / 1e9 / avg_t:.2f} GB/s, '
    ...
    for return_recv_hook in (False, True):
        ...
        if not return_recv_hook:
            print(f'[rank {rank}] new Dispatch bandwidth: {num_dispatch_comm_bytes / 1e9 / dispatch_t:.2f} GB/s, avg_t={dispatch_t * 1e6:.2f} us | '
                  f'Combine bandwidth: {num_combine_comm_bytes / 1e9 / combine_t:.2f} GB/s, avg_t={combine_t * 1e6:.2f} us', flush=True)
        else:
            print(f'[rank {rank}] new Dispatch send/recv time: {dispatch_t[0] * 1e6:.2f} + {dispatch_t[1] * 1e6:.2f} us | '
