...
#define UCCL_MSGBUF_FLAGS_TXPULLTIME_FREE (1 << 2)
 ...
 public:
  static FrameBuf* Create(uint64_t frame_offset, void* umem_buffer,
     ...
     */
    return new (reinterpret_cast<void*>(frame_offset + (uint64_t)umem_buffer -
                                        XDP_PACKET_HEADROOM))
...
#define GET_FRAMEBUF_PTR(frame_offset, umem_buffer)                  \
  ...
  static uint32_t get_uccl_frame_len(uint64_t frame_offset, void* umem_buffer) {
    auto msgbuf = GET_FRAMEBUF_PTR(frame_offset, umem_buffer);
...
class AFXDPFactory {
 public:
  ...
  static void init(char const* interface_name, uint64_t num_frames,
...
class AFXDPSocket {
 ...
 public:
  ...
  inline uint64_t send_queue_estimated_latency_ns() {
    auto send_queue_pending_entries = TX_RING_SIZE - send_queue_free_entries();
