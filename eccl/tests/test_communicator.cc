...
void communicator_client_thread(std::shared_ptr<Communicator> comm,
                                int peer_rank) {
...
void communicator_server_thread(std::shared_ptr<Communicator> comm,
                                int peer_rank) {
...
void test_communicator() {
  ...
  std::thread t_client(communicator_client_thread, comm0, 1);
  std::thread t_server(communicator_server_thread, comm1, 0);
