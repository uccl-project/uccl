...
void test_socket_oob() {
    ...
    if (ex.wait_and_fetch("rdma:peer:1:0", remote, 50, 100)) {
        ...
        for (size_t i = 0; i < remote.qps.size(); ++i) {
            ...
            std::cout << "  QP[" << i << "]: qp_num=" << q.qp_num
                      << " psn=" << q.psn
                      << " lid=" << q.lid
                      << " gid=";
            for (int j = 0; j < 16; ++j) {
                std::cout << std::hex << std::setw(2)
                          << std::setfill('0') << static_cast<int>(q.gid[j]) << " ";
            }
            std::cout << std::dec << "\n";
        }
    } else {
...
void rank_thread_socket(int local_rank, int world_size,
    ...
    if (!ex->publish(key, local)) {
        std::cerr << "[ERROR] Rank " << local_rank
                  << " failed to publish meta\n";
        return;
    }
    std::cout << "[INFO] Rank " << local_rank
              << " published meta (" << local.host_id << ")\n";
...
void test_socket_meta_exchange_multi_threads(int world_size) {
    ...
    for (int rank = 0; rank < world_size; ++rank) {
        threads.emplace_back(rank_thread_socket, rank, world_size,
                             "127.0.0.1", 12345);
