...
Communicator::Communicator(int gpu_id, int rank, int world_size, std::shared_ptr<Config> config):
    ...
    std::cout << "Communicator initialized: comm_meta_map_ success" << std::endl;
}
...
bool Communicator::connect_to(int rank) {
    if (check_ready()) {
        ...
        if (meta->host_id == local_meta->host_id) { // same host
            ...
            // return ep->connect_to(meta->uds_listen_fd);
            return true;
        } else {
            auto ep = std::make_shared<RDMAEndpoint>(config_);
            // return ep->connect(meta->server_ip, meta->server_port);
            return true;
        }
    }
    return false;
...
std::tuple<std::shared_ptr<EndpointBase>, bool> Communicator::get_endpoint_by_rank(int rank) {
    ...
    auto it = endpoint_map_.find(rank);
...
bool Communicator::send_async(std::shared_ptr<Request> req) {
    auto [ep, ok] = get_endpoint_by_rank(3);
    if (ok) {
        // ep->send_async(req);
        return true;
    } else {
...
bool Communicator::recv_async(std::shared_ptr<Request> req) {
    auto [ep, ok] = get_endpoint_by_rank(3);
    if (ok) {
        // ep->recv_async(req);
        return true;
...
void Communicator::set_communicator_meta_with_rank(int rank, const CommunicatorMeta& meta) {
    ...
    comm_meta_map_[rank] = std::make_shared<CommunicatorMeta>(meta);
}
std::shared_ptr<CommunicatorMeta> Communicator::get_communicator_meta_by_rank(int rank) {
    std::lock_guard<std::mutex> lock(meta_mu_);
    auto it = comm_meta_map_.find(rank);
    return (it != comm_meta_map_.end()) ? it->second : nullptr;
}

bool Communicator::check_ready() {
    // check meta map
    std::lock_guard<std::mutex> lock(meta_mu_);
    if (static_cast<int>(comm_meta_map_.size()) < world_size_) return false;

    for (int i = 0; i < world_size_; i++) {
        auto it = comm_meta_map_.find(i);
        if (it == comm_meta_map_.end()) {
            return false;
        }
