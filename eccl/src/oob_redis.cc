#include "oob.h"

RedisExchanger::RedisExchanger(const std::string& host, int port, int timeout_ms) {
    struct timeval timeout;
    timeout.tv_sec = timeout_ms / 1000;
    timeout.tv_usec = (timeout_ms % 1000) * 1000;
    ctx_ = redisConnectWithTimeout(host.c_str(), port, timeout);
    if (!ctx_ || ctx_->err) {
        if (ctx_) {
        std::cerr << "Redis connection error: " << ctx_->errstr << std::endl;
        redisFree(ctx_);
        } else {
        std::cerr << "Redis allocation error" << std::endl;
        }
        ctx_ = nullptr;
    }
}

RedisExchanger::~RedisExchanger() {
    if (ctx_) redisFree(ctx_);
}

bool RedisExchanger::publish(const std::string& key, const Exchangeable& obj) {
    if (!ctx_) return false;

    auto kv = obj.to_map();
    std::string cmd = "HSET " + key;
    for (auto& p : kv) {
        cmd += " " + p.first + " " + p.second;
    }

    redisReply* reply = (redisReply*)redisCommand(ctx_, cmd.c_str());
    if (!reply) {
        std::cerr << "Redis publish failed: " << ctx_->errstr << std::endl;
        return false;
    }
    freeReplyObject(reply);
    return true;
}

bool RedisExchanger::fetch(const std::string& key, Exchangeable& obj) {
    if (!ctx_) return false;

    redisReply* reply = (redisReply*)redisCommand(ctx_, "HGETALL %s", key.c_str());
    if (!reply) {
      std::cerr << "Redis fetch failed: " << ctx_->errstr << std::endl;
      return false;
    }

    if (reply->type != REDIS_REPLY_ARRAY || reply->elements == 0) {
      freeReplyObject(reply);
      return false; // key 不存在
    }

    std::map<std::string, std::string> kv;
    for (size_t i = 0; i + 1 < reply->elements; i += 2) {
      std::string field(reply->element[i]->str, reply->element[i]->len);
      std::string value(reply->element[i+1]->str, reply->element[i+1]->len);
      kv[field] = value;
    }

    obj.from_map(kv);
    freeReplyObject(reply);
    return true;
}

bool RedisExchanger::wait_and_fetch(const std::string& key, Exchangeable& obj,
                      int max_retries, int delay_ms) {
    if (max_retries>0) {
        for (int i = 0; i < max_retries; ++i) {
            if (fetch(key, obj)) return true;
            std::this_thread::sleep_for(std::chrono::milliseconds(delay_ms));
        }
    } else { // -1
        while(true) {
            if (fetch(key, obj)) return true;
            std::this_thread::sleep_for(std::chrono::milliseconds(delay_ms));
        }
    }
    return false;
}
