...
static ssize_t send_all_robust(int fd, const char* buf, size_t len) {
    size_t total = 0;
    while (total < len) {
...
static bool recv_line_capped(int fd,
                             std::string& out,
...
static void add_conn_thread(std::vector<std::thread>& vec,
                            std::mutex& mtx,
                            std::thread&& t) {
...
{
    std::thread t(handle_connection_inner,
                  connfd,
                  std::ref(store),
                  std::ref(store_mutex),
                  std::ref(running),
...
SockExchanger::SockExchanger(bool is_server,
...
SockExchanger::~SockExchanger() {
    ...
    try {
        ...
        std::cerr << "[WARN] SockExchanger::~SockExchanger caught exception during shutdown\n";
    }
}
...
bool SockExchanger::send_cmd_and_recv(const std::string& cmd, std::string& resp) {
...
bool SockExchanger::publish(const std::string& key, const Exchangeable& obj) {
    auto kv = obj.to_map();
...
bool SockExchanger::fetch(const std::string& key, Exchangeable& obj) {
    if (is_server_) {
        std::lock_guard<std::mutex> lk(store_mutex_);
...
bool SockExchanger::wait_and_fetch(const std::string& key, Exchangeable& obj,
...
bool SockExchanger::start_server() {
    ...
    if (listen_fd_ < 0) {
        std::cerr << "SockExchanger: create listen socket failed: " << std::strerror(errno) << "\n";
        return false;
    }
    ...
    server_thread_ = std::thread([this]() {
        try {
            ...
            while (running_.load(std::memory_order_relaxed)) {
                ...
                ::setsockopt(conn, SOL_SOCKET, SO_RCVTIMEO, (const char*)&conn_tv, sizeof conn_tv);
                ::setsockopt(conn, SOL_SOCKET, SO_SNDTIMEO, (const char*)&conn_tv, sizeof conn_tv);

                auto registrar = [this](std::thread&& t) {
                    add_conn_thread(this->conn_threads_, this->conn_threads_mutex_, std::move(t));
                };
