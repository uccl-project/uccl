...
def setup_distributed():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("ROCm/CUDA not available. Please install ROCm for AMD GPUs.")
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        train_acc = 100. * total_correct.item() / total_samples.item()
        ...
        test_total_samples = torch.tensor(test_total, dtype=torch.float, device=device)
        test_total_correct = torch.tensor(test_correct, dtype=torch.float, device=device)
        test_total_loss = torch.tensor(test_loss, dtype=torch.float, device=device)
        ...
        test_acc = 100. * test_total_correct.item() / test_total_samples.item()
