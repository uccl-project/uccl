...
#pragma unroll
  ...
  using Sort = hipcub::BlockRadixSort<uint32_t, Threads, kNumSymPerThread>;
...
#pragma unroll
  ...
  // NOTE: we need to have a contiguous blocked arrangement for hipcub::BlockScan
...
#pragma unroll
  ...
  using Scan = hipcub::BlockScan<uint32_t, Threads>;
...
void ansHistogramBatch(
...

  {
   ...
   hipLaunchKernelGGL(( histogramBatch<InProvider, kThreads>)
        , dim3(grid), dim3(kThreads), 0, stream, inProvider, histogram_dev);
  }
}
...
inline void ansCalcWeights(
 ...
 hipLaunchKernelGGL(( quantizeWeights<SizeProvider, kThreads>), dim3(numInBatch), dim3(kThreads), 0, stream, 
