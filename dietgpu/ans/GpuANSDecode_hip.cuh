...
#include "ans/BatchProvider_hip.cuh"
#include "ans/GpuANSCodec_hip.h"
#include "ans/GpuANSInfo_hip.cuh"
...
ANSDecodeStatus ansDecodeBatch(
  ...
  // Build the rANS decoding table from the compression header
  {
   ...
   hipLaunchKernelGGL(( ansDecodeTable<InProvider, kThreads>), dim3(numInBatch), dim3(kThreads), 0, stream, 
...
#define RUN_DECODE(BITS)                                           \
  do {                                                             \
    ...
    auto grid = dim3(perBatchGrid, numInBatch);                    \
                                                                   \
   hipLaunchKernelGGL(( ansDecodeKernel<                                               \
        InProvider,                                                \
        OutProvider,                                               \
        kThreads,                                                  \
        BITS,                                                      \
        kDefaultBlockSize>), dim3(grid), dim3(kThreads), 0, stream,          \
        inProvider,                                                \
        table_dev.data(),                                          \
        outProvider,                                               \
        outSuccess_dev,                                            \
