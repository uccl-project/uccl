...
__device__ void checksumSingle(
  ...
  using BlockReduce = hipcub::BlockReduce<uint32_t, Threads>;
...
void checksumBatch(
  ...
  CUDA_VERIFY(
      hipMemsetAsync(checksum_dev, 0, sizeof(uint32_t) * numInBatch, stream));
 ...
 hipLaunchKernelGGL(( checksumBatch<InProvider, kThreads>)
      , dim3(grid), dim3(kThreads), 0, stream, inProvider, checksum_dev);
