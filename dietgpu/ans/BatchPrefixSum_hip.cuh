...
__global__ void batchExclusivePrefixSum1(
  ...
  using Scan = hipcub::BlockScan<T, Threads>;
...
#define BPS_LEVEL_1(THREADS, TEMP)                        \
 hipLaunchKernelGGL(( batchExclusivePrefixSum1<T, THREADS, TransformFn>)       \
      , dim3(dim3(blocks, numInBatch)), dim3(THREADS), 0, stream,  \
          in_dev, out_dev, TEMP, batchSize, fn)

#define BPS_LEVEL_2(THREADS)           \
 hipLaunchKernelGGL(( batchExclusivePrefixSum2<T, THREADS>) \
      , dim3(numInBatch), dim3(THREADS), 0, stream, temp_dev, batchSize, blocks)

#define BPS_LEVEL_3(THREADS)                              \
 hipLaunchKernelGGL(( batchExclusivePrefixSum3<T, THREADS>)                    \
      , dim3(dim3(blocks, numInBatch)), dim3(THREADS), 0, stream,  \
