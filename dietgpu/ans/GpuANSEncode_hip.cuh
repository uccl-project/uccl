...
#include "ans/BatchPrefixSum_hip.cuh"
#include "ans/GpuANSCodec_hip.h"
#include "ans/GpuANSStatistics_hip.cuh"
#include "ans/GpuANSUtils_hip.cuh"
#include "ans/GpuChecksum_hip.cuh"
#include "utils/DeviceDefs_hip.cuh"
#include "utils/DeviceUtils_hip.h"
#include "utils/PtxUtils_hip.cuh"
...
void ansEncodeBatchDevice(
  ...
  if (histogram_dev) {
    ...
    hipMemcpy(tempHistogram_host.data(), tempHistogram_dev.data(),
               numInBatch * kNumSymbols * sizeof(uint32_t), hipMemcpyDeviceToHost);
  ...
  hipMemcpy(table_host.data(), table_dev.data(),
             numInBatch * kNumSymbols * sizeof(uint4), hipMemcpyDeviceToHost);
...
#define RUN_ENCODE(BITS)                                       \
  do {                                                         \
   hipLaunchKernelGGL(( ansEncodeBatchFull<InProvider, BITS, kDefaultBlockSize>)    \
        , dim3(gridFull), dim3(kThreads), 0, stream,                    \
            inProvider,                                        \
            ...
            table_dev.data());                                 \
                                                               \
   hipLaunchKernelGGL(( ansEncodeBatchPartial<InProvider, BITS, kDefaultBlockSize>) \
        , dim3(gridPartial), dim3(kThreads), 0, stream,                 \
            inProvider,                                        \
            maxNumCompressedBlocks,                            \
            uncoalescedBlockStride,                            \
            compressedBlocks_dev.data(),                       \
...
#undef RUN_ENCODE
  ...
  //   hipMemcpy(compressedWords_host.data(), compressedWords_dev.data(),
  //              numInBatch * maxNumCompressedBlocks * sizeof(uint32_t),
  //              hipMemcpyDeviceToHost);
  //   hipMemcpy(compressedWordsPrefix_host.data(), compressedWordsPrefix_dev.data(),
  //              numInBatch * maxNumCompressedBlocks * sizeof(uint32_t),
  //              hipMemcpyDeviceToHost);
  ...
  CUDA_TEST_ERROR();
  {
    ...
    auto grid = dim3(::max(maxNumCompressedBlocks, 1U), numInBatch);

   hipLaunchKernelGGL(( ansEncodeCoalesceBatch<InProvider, OutProvider, kThreads>)
        , dim3(grid), dim3(kThreads), 0, stream, 
            compressedBlocks_dev.data(),
            inProvider,
            maxNumCompressedBlocks,
            uncoalescedBlockStride,
