        ...
        for dt in [torch.float32]:
            for tm in [True, False]:
                for checksum in [False,True]:
                    ts = [
                        torch.arange(1, 65, device=dev, dtype=dt).view(64),
                        # torch.arange(1, 33, device=dev, dtype=dt).view(32),
                        # torch.normal(0, 1.0, [32], dtype=dt, device=dev),
                        torch.normal(0, 1.0, [128 * 512 * 1024], dtype=dt, device=dev),
                        # torch.normal(0, 1.0, [64], dtype=dt, device=dev),
                    ]
                    if tm:
