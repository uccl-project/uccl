...
#include "ans/GpuANSEncode_hip.cuh"
#include "ans/GpuANSUtils_hip.cuh"
#include "ans/GpuChecksum_hip.cuh"
#include "float/GpuFloatCodec_hip.h"
#include "float/GpuFloatUtils_hip.cuh"
...
#define RUN_SPLIT(FLOAT_TYPE)                                      \
  do {                                                             \
    ...
    auto grid = dim3(perBatchGrid, numInBatch);                    \
                                                                   \
   hipLaunchKernelGGL(( splitFloat<InProvider, OutProvider, FLOAT_TYPE, kBlock>)        \
...
#define RUN_ANS(FT)                                                         \
  do {                                                                      \
    ...
    ansEncodeBatchDevice(                                                   \
        ...
        stream);                                                            \
                                                                            \
   hipLaunchKernelGGL(( incOutputSizes<FT>), dim3(divUp(numInBatch, 128)), dim3(128), 0, stream,          \
