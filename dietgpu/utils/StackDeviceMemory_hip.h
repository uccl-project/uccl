...
struct GpuMemoryReservation {
  ...
  std::vector<T> copyToHost(hipStream_t stream_2) const {
    auto out = std::vector<T>(num);

    CUDA_VERIFY(hipMemcpyAsync(
        out.data(), data(), num * sizeof(T), hipMemcpyDeviceToHost, stream_2));
...
class StackDeviceMemory {
 public:
  ...
  GpuMemoryReservation<T> copyAlloc(
    ...
    CUDA_VERIFY(
        hipMemcpyAsync(mem.data(), ptr, size, hipMemcpyHostToDevice, stream));
  ...
  void* allocPointer(hipStream_t stream, size_t size, AllocType type);
  void deallocPointer(int device, hipStream_t, size_t size, void* p);
 ...
 protected:
  ...
  struct Stack {
    ...
    /// byte aligned
    void* getAlloc(size_t size, hipStream_t stream, AllocType type);

    /// Returns an allocation
    void returnAlloc(void* p, size_t size, hipStream_t stream);
    ...
    /// Free allocations via hipMalloc that we made that couldn't fit inside
    /// our stack
