...
class CustomInstall(install):
    ...
    def run(self):
        ...
        so_files = list(Path(build_lib).glob("dietgpu*.so"))
...
if __name__ == "__main__":
    ...
    sources = ["./DietGpu.cpp"] + glob("./utils/*.cu") + glob("./utils/*.cpp") + glob("./utils/*.cc") + glob("./float/*.cu") + glob("./float/*.cpp") + glob("./float/*.cc") + glob("./ans/*.cu") + glob("./ans/*.cpp") + glob("./ans/*.cc")
    libraries = ["ibverbs", "glog", "nl-3", "nl-route-3", "numa"]
    include_dirs = [PROJECT_ROOT, PROJECT_ROOT / "ans", PROJECT_ROOT / "float", PROJECT_ROOT / "utils", PROJECT_ROOT / ".." / "include"]
    library_dirs = []
    nvcc_dlink = []
    extra_link_args = []
    ...
    if torch.version.cuda:
        ...
        # Enable HIP DSA support
        cxx_flags.append("-DTORCH_USE_HIP_DSA")
        nvcc_flags.append("-DTORCH_USE_HIP_DSA")
    ...
    print("?????????????????????????????????????")
    has_hip = any(src.endswith(".hip") for src in sources)

    if has_hip:
        print("[INFO] HIP source detected (.hip files found)")
    else:
        print("[INFO] No HIP source files detected")

    path = "/opt/rocm/include/hipcub/hipcub.hpp"
