...
def test_engine_creation():
    ...
    try:
        engine1 = kvtrans_engine.Engine(0, 2)
        print(f"✓ Created engine: {engine1}")

        engine2 = kvtrans_engine.Engine(1, 2)
...
def test_connection_operations(engine):
    ...
    success, conn_id = engine.connect("127.0.0.1", 1)
    if success:
    ...
    success, ip_addr, remote_gpu_idx, accept_conn_id = engine.accept()
    if success:
        print(
            f"✓ Accepted connection from {ip_addr}:{remote_gpu_idx} with conn_id: {accept_conn_id}"
        )
    else:
        print("✗ Accept failed")
        accept_conn_id = None
...
def test_kv_operations(engine, conn_id):
    ...
    # Create test data
    test_data = (
        b"Hello, KVTrans Engine! This is test data for the key-value store."
    )
    ...
    success, mr_id = engine.reg_kv(buffer)
    if success:
        print(f"✓ KV registered with mr_id: {mr_id}")
    else:
    ...
    success = engine.send_kv(conn_id, mr_id, send_buffer)
    if success:
    ...
    # Receive KV
    print("Receiving KV...")
    success, received_data = engine.recv_kv(conn_id, mr_id, 1024)
    if success:
...
def test_numpy_integration(engine, conn_id):
    ...
    success1, mr_id1 = engine.reg_kv(array1)
    success2, mr_id2 = engine.reg_kv(array2)

    if success1 and success2:
        print(f"✓ Arrays registered with mr_ids: {mr_id1}, {mr_id2}")
    else:
        print("✗ Array registration failed")
        return

    # Send arrays
    success1 = engine.send_kv(conn_id, mr_id1, array1)
    success2 = engine.send_kv(conn_id, mr_id2, array2)
...
def test_error_handling(engine):
    ...
    success, mr_id = engine.reg_kv(b"test data")
    if not success:
    ...
    success = engine.send_kv(conn_id, 999, b"test data")
    if not success:
...
def test_module_info():
    ...
    available_classes = [
        name for name in dir(kvtrans_engine) if not name.startswith("_")
    ]
    print(f"Available classes/functions: {available_classes}")

    # Test Engine class methods
    if "Engine" in available_classes:
        engine = kvtrans_engine.Engine(0, 2)
        engine_methods = [
            method for method in dir(engine) if not method.startswith("_")
        ]
...
def main():
    ...
    if engine1:
        ...
        if conn_id:
            mr_id = test_kv_operations(engine1, conn_id)
