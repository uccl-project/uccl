      ...
      switch (test_type) {
        ...
        case kAsync: {
          ...
          for (int j = 0; j < kMaxInflight; j++) {
            ...
            PollCtx* poll_ctx;
            poll_ctx = ep.uccl_send_async(conn_id, iter_data, iter_len);
        ...
        case kMt: {
          ...
          std::thread t1([&ep, conn_id, data, send_len]() {
            PollCtx* poll_ctx = ep.uccl_send_async(conn_id, data, send_len);
            ep.uccl_poll(poll_ctx);
          });
          std::thread t2([&ep, conn_id, data2, &recv_len]() {
            PollCtx* poll_ctx = ep.uccl_recv_async(conn_id, data2, &recv_len);
            ep.uccl_poll(poll_ctx);
          });
          t1.join();
          t2.join();
        ...
        case kTput: {
          auto* poll_ctx = ep.uccl_send_async(conn_id, data, send_len);
          poll_ctx->timestamp = rdtsc();
          if (last_ctx) {
      ...
      switch (test_type) {
        ...
        case kAsync: {
          ...
          for (int j = 0; j < kMaxInflight; j++) {
            ...
            PollCtx* poll_ctx;
            poll_ctx = ep.uccl_recv_async(conn_id, iter_data, &recv_lens[j]);
            poll_ctxs.push_back(poll_ctx);
          }
          for (auto poll_ctx : poll_ctxs) {
            ep.uccl_poll(poll_ctx);
        ...
        case kMt: {
          std::thread t1([&ep, conn_id, data, &recv_len]() {
            PollCtx* poll_ctx = ep.uccl_recv_async(conn_id, data, &recv_len);
            ep.uccl_poll(poll_ctx);
          });
          std::thread t2([&ep, conn_id, data2, send_len]() {
            PollCtx* poll_ctx = ep.uccl_send_async(conn_id, data2, send_len);
            ep.uccl_poll(poll_ctx);
          });
          t1.join();
          t2.join();
        ...
        case kTput: {
          auto* poll_ctx = ep.uccl_recv_async(conn_id, data, &recv_len);
