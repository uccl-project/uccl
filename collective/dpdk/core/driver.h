...
class Driver {
 public:
  ...
  void start(int argc, char** argv) {
    ...
    LOG(INFO, DPDK) << "Mbuf pool created";
  ...
  inline void process_packet(struct rte_mbuf* mbuf) {
    ...
    if (icmp->icmp_type == RTE_ICMP_TYPE_ECHO_REQUEST) {
      LOG(INFO, DPDK) << "Received ICMP Echo Request from"
                      << ((ip->src_addr) & 0xff) << "."
                      << ((ip->src_addr >> 8) & 0xff) << "."
                      << ((ip->src_addr >> 16) & 0xff) << "."
                      << ((ip->src_addr >> 24) & 0xff);
