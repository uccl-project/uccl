...
class Dpdk {
 public:
  ...
  void InitDpdk(int argc, char** argv) {
    if (initialized_) {
      LOG(WARNING, DPDK) << "DPDK is already initialized.";
      return;
    }

    LOG(INFO, DPDK) << "Initializing DPDK with args";

    int ret = rte_eal_init(argc, argv);
    if (ret < 0) {
      LOG(FATAL, DPDK) << "rte_eal_init() failed: ret = " << ret
                       << " rte_errno = " << rte_errno << " ("
    ...
    if (rte_eal_iova_mode() == RTE_IOVA_VA) {
      LOG(INFO, DPDK) << "DPDK runs in VA mode.";
    } else {
      LOG(INFO, DPDK) << "DPDK runs in PA mode.";
    }

    initialized_ = true;
    LOG(INFO, DPDK) << "DPDK initialized successfully";
  }
  ...
  void DeInitDpdk() {
    if (initialized_) {
      ...
      if (ret != 0) {
        LOG(FATAL, DPDK) << "rte_eal_cleanup() failed: ret = " << ret
                         << " rte_errno = " << rte_errno << " ("
                         << rte_strerror(rte_errno) << ")";
      }
