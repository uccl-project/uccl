...
void socket_recv(struct socket_t* socket, Packet** pkts, int queue_id) {
  ...
  for (uint32_t i = 0; i < rcvd; i++) {
    ...
    if (pkt->length() < sizeof(Ethernet)) {
      LOG(WARNING, DPDK) << "received non-ethernet packet " << queue_id;
    ...
    if (eth->eth_type.value() != Ethernet::kIpv4) [[unlikely]] {
      LOG(WARNING, DPDK) << "received non-ipv4 packet " << std::hex
                         << eth->eth_type.value();
      socket->dpdk_socket->push_packet(pkt);
      continue;
    }
    ...
    if (pkt->length() != sizeof(Ethernet) + ipv4->total_length.value()) {
      LOG(WARNING, DPDK) << "IPv4 packet length mismatch (expected: "
    ...
    if (ipv4->next_proto_id != Ipv4::Proto::kUdp) [[unlikely]] {
      LOG(WARNING, DPDK) << "received non-udp packet " << queue_id;
      socket->dpdk_socket->push_packet(pkt);
      continue;
    }
...
static void* send_thread(void* arg) {
  ...
  LOG(INFO, DPDK) << "started socket send thread for queue #" << queue_id;
...
static void* recv_thread(void* arg) {
  ...
  LOG(INFO, DPDK) << "started socket recv thread for queue #" << queue_id;
  ...
  LOG(INFO, DPDK) << "Stopped socket recv thread for queue #" << queue_id;
...
int main(int argc, char* argv[]) {
  ...
  if (strcmp(argv[1], "--server") == 0) {
    is_client = false;
    LOG(INFO, DPDK) << "[server] " << server_mac_str << " " << server_ip_str;
    client_port_id = dpdk.GetPmdPortIdByMac(server_mac_str);
  } else if (strcmp(argv[1], "--client") == 0) {
    LOG(INFO, DPDK) << "[client] " << client_mac_str << " " << client_ip_str;
    client_port_id = dpdk.GetPmdPortIdByMac(client_mac_str);
  } else {
    LOG(INFO, DPDK) << "Usage: " << argv[0] << " --server | --client";
    return 1;
  }

  if (client_port_id == (uint16_t)-1) {
    LOG(INFO, DPDK) << "Client port not found";
    return 1;
  }
