...
[[maybe_unused]] static rte_mempool* CreateSpScPacketPool(
  ...
  if (mp == nullptr) {
    LOG(ERROR, DPDK) << "rte_mempool_create() failed. ";
    return nullptr;
  }
...
PacketPool::PacketPool(uint32_t nmbufs, uint16_t mbuf_size,
  ...
  if (is_dpdk_primary_process_) {
    ...
    LOG(INFO, DPDK) << "[ALLOC] [type:mempool, name:" << mpool_name
    ...
    if (mpool_ == nullptr) {
      LOG(FATAL, DPDK) << "[LOOKUP] [type: mempool, name: " << mempool_name
                       << "] failed. rte_errno = " << rte_errno << " ("
                       << rte_strerror(rte_errno) << ")";
    } else {
      LOG(INFO, DPDK) << "[LOOKUP] [type: mempool, name " << mempool_name
...
PacketPool::~PacketPool() {
  LOG(INFO, DPDK) << "[FREE] [type:mempool, name:" << this->GetPacketPoolName()
                  << "]";
