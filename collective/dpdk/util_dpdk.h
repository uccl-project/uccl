...
class DPDKSocket {
 public:
  ...
  inline void push_packet(Packet* pkt) {
    // UCCL_LOG(INFO, DPDK) << "push_packet packet: " << pkt;
    Packet::Free(pkt);
  }
  ...
  inline Packet* pop_packet(uint16_t pkt_len) {
    ...
    // UCCL_LOG(INFO, DPDK) << "pop_packet(uint16_t) avail_packets: "
  ...
  uint32_t send_packets(Packet** pkts, uint32_t nb_pkts) {
    ...
    //   UCCL_LOG(INFO, DPDK) << "send_packets [" << i << "] packet: " <<
    //   pkts[i];
    // }
    // UCCL_LOG(INFO, DPDK) << "send_packets: " << nb_pkts;

    uint32_t sent = tx_ring_->TrySendPackets(pkts, nb_pkts);
    total_sent_packets_ += sent;
    // UCCL_LOG(INFO, DPDK) << "send_packets sent: " << sent;
    return sent;
  }
  ...
  std::string to_string() {
    ...
    UCCL_LOG(INFO, DPDK) << "rx: " << stats.ipackets
...
class DPDKFactory {
 public:
  ...
  DPDKSocket* CreateSocket(int queue_id) {
    if (!initialized_) {
      UCCL_LOG(WARNING, DPDK) << "DPDKFactory is not initialized";
      return nullptr;
    }
