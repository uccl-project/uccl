...
void TXTracking::receive_acks(uint32_t num_acked_pkts) {
  ...
  while (num_acked_pkts) {
    // UCCL_LOG(INFO, DPDK) << "receive_acks: " << "\n\tnum_acked_pkts " <<
    // num_acked_pkts
    ...
    if (msgbuf->is_last()) {
      // UCCL_LOG(INFO, DPDK) << "Transmitted a complete message";
...
std::optional<PacketBuf*> TXTracking::get_and_update_oldest_unsent() {
  ...
  // UCCL_LOG(INFO, DPDK) << "get_and_update_oldest_unsent: num_unacked_msgbufs_
  // "
...
RXTracking::ConsumeRet RXTracking::consume(swift::Pcb* pcb, Packet* pkt) {
  ...
  //   UCCL_LOG(INFO, DPDK) << "Received packet: " << seqno << " < " <<
  //   expected_seqno;

  if (swift::seqno_lt(seqno, expected_seqno)) {
    // UCCL_VLOG(3) << "Received old packet: " << seqno << " < " <<
    // expected_seqno; UCCL_LOG(INFO, DPDK) << "Received old packet: " << seqno
    // << " < " << expected_seqno;
  ...
  if (distance >= kReassemblyMaxSeqnoDistance) {
    ...
    // UCCL_LOG(INFO, DPDK) << "Packet too far ahead. Dropping as we can't
    // handle SACK. "
    //           << "seqno: " << seqno << ", expected: " << expected_seqno;
    socket_->push_packet(pkt);
  ...
  if (seqno != expected_seqno) {
    ...
    if (it != reass_q_.end() && it->first == seqno) {
      ...
      // UCCL_LOG(INFO, DPDK) << "Received duplicate packet: " << seqno;
    ...
    // UCCL_LOG(INFO, DPDK) << "Received OOO trackable packet: " << seqno
    //           << " payload_len: " << frame_len - kNetHdrLen - kUcclHdrLen
    //           << " reass_q size " << reass_q_.size();
  } else {
    // UCCL_VLOG(3) << "Received expected packet: " << seqno
    //         << " payload_len: " << frame_len - kNetHdrLen - kUcclHdrLen;
    // UCCL_LOG(INFO, DPDK) << "Received expected packet: " << seqno
    //           << " payload_len: " << frame_len - kNetHdrLen - kUcclHdrLen;
  }
...
void UcclFlow::rx_messages() {
  ...
  for (auto pkt : pending_rx_frames_) {
    ...
    switch (ucclh->net_flags) {
      ...
      case UcclPktHdr::UcclFlags::kData:
        ...
        // UCCL_LOG(INFO, DPDK) << "Received data packet: " <<
        // ucclh->seqno.value();
      ...
      case UcclPktHdr::UcclFlags::kRssProbeRsp:
        ...
        UCCL_LOG_EVERY_N(INFO, DPDK, 10000)
  ...
  //   UCCL_LOG(INFO, DPDK) << "num_data_frames_recvd: " <<
  //   num_data_frames_recvd;
  ...
  if (num_data_frames_recvd) {
    ...
    if (rx_tracking_.ready_msg_queue_.size() <= kMaxReadyRxMsgbufs) {
      ...
      // UCCL_LOG(INFO, DPDK) << "Sending ACK packet: " << pcb_.seqno() << " "
      //           << pcb_.ackno();
  ...
  if (swift::seqno_lt(ackno, pcb_.snd_una)) {
    ...
    if (pcb_.duplicate_acks < kFastRexmitDupAckThres) {
      ...
      UCCL_VLOG(2) << "Fast recovery " << ackno << " sack_bitmap_count "
                   << sack_bitmap_count;
      // UCCL_LOG(INFO, DPDK) << "Fast recovery " << ackno << "
      // sack_bitmap_count "
      //           << sack_bitmap_count;
      ...
      while (sack_bitmap_count && msgbuf && index < hard_budget) {
        ...
        if ((sack_bitmap & (1ULL << sack_bitmap_idx_in_bucket)) == 0) {
          ...
          // UCCL_LOG(INFO, DPDK) << "Fast recovery retransmitting " << seqno <<
          ...
          if (seqno == msgbuf->get_seqno()) {
            ...
            // UCCL_LOG(INFO, DPDK) << "process ack: " <<
      ...
      if (!missing_frames_.empty()) {
        ...
        // UCCL_LOG(INFO, DPDK) << "Fast recovery retransmitting " <<
    ...
    // UCCL_LOG(INFO, DPDK) << "Received ACK for untransmitted data.";
  } else {
    UCCL_VLOG(3) << "Received valid ACK " << ackno;
    // UCCL_LOG(INFO, DPDK) << "Received valid ACK " << ackno;
...
void UcclFlow::fast_retransmit() {
  ...
  if (msgbuf && seqno != pcb_.snd_nxt) {
...
#endif
    // Nelson: re-allocated packet and restore the payload.
    // UCCL_LOG(INFO, DPDK) << "fast_retransmit: " << socket_->avail_packets()
    // << " "
    ...
    msgbuf->mark_not_txpulltime_free();
    // UCCL_LOG(INFO, DPDK) << "fast_retransmit msgbuf: " << msgbuf
...
#endif
  // Nelson: re-allocated packet and restore the payload.
  // UCCL_LOG(INFO, DPDK) << "rto_retransmit: " << socket_->avail_packets() << "
  // "
  //           << socket_->in_use_packets() << " seqno: " << seqno;
  prepare_retransmission_payload(msgbuf);
  prepare_datapacket(msgbuf, path_id, seqno, UcclPktHdr::UcclFlags::kData);
...
void UcclFlow::transmit_pending_packets() {
  ...
  //     UCCL_LOG(INFO, DPDK) << "transmitting success rate: "
  ...
  for (uint32_t i = 0; i < permitted_packets; i++) {
    ...
    // UCCL_LOG(INFO, DPDK) << "Transmitting seqno: " << seqno
    //           << " Packet: " << msgbuf->get_pkt();
...
void UcclFlow::deserialize_and_append_to_txtracking() {
  ...
  while (tx_tracking_.num_unsent_msgbufs() < kMaxTwPkts &&
    ...
    // UCCL_LOG(INFO, DPDK) << "deserialize_and_append_to_txtracking [" << i
    ...
    while (cur_msgbuf != nullptr && num_tx_frames < deser_budget) {
      ...
      // UCCL_LOG(INFO, DPDK) << "deserialize_and_append_to_txtracking [" << j
...
Packet* UcclFlow::craft_rssprobe_packet(uint16_t dst_port) {
  ...
  // UCCL_LOG(INFO, DPDK) << "craft_rssprobe_packet: " <<
  // socket_->avail_packets() << " "
  //           << socket_->in_use_packets();
...
void UcclEngine::run() {
  ...
  while (!shutdown_) {
    ...
    if (Channel::dequeue_sc(channel_->tx_deser_q_, &tx_deser_work)) {
      ...
      // UCCL_LOG(INFO, DPDK) << "Tx deser jring dequeue flow_id: " <<
  ...
  while (!engines[0]->shutdown_) {
    ...
    for (auto engine : engines) {
      if (Channel::dequeue_sc(engine->channel_->tx_task_q_, &tx_deser_work)) {
        ...
        // thread.
        tx_deser_work.poll_ctx->read_barrier();
        // UCCL_LOG(INFO, DPDK) << "Tx jring dequeue";
        UCCL_VLOG(3) << "Tx jring dequeue";
        // UCCL_LOG(INFO, DPDK) << "Tx jring dequeue flow_id: " <<
        // tx_deser_work.flow_id;
        ...
        while (remaining_bytes > 0) {
          ...
          // UCCL_LOG(INFO, DPDK) << "deser_th_func: [" << i << "] " <<
          ...
          // UCCL_LOG(INFO, DPDK) << "deser_th_func [" << i << "] msgbuf: " <<
          // msgbuf
          //           << " payload_len: " << payload_len
          //           << " packet: " << msgbuf->get_pkt()
      ...
      if (Channel::dequeue_sc(engine->channel_->rx_deser_q_, &rx_deser_work)) {
        ...
        // UCCL_LOG(INFO, DPDK) << "Rx ser jring dequeue flow_id: " <<
        ...
        while (ready_msg != nullptr) {
          ...
          // auto const* ucclh =
          //     reinterpret_cast<UcclPktHdr const*>(pkt_addr + kNetHdrLen);
          // UCCL_LOG(INFO, DPDK) << "payload_len: " << payload_len << " seqno:
          // " << std::dec
          //         << ucclh->seqno.value();
#ifndef EMULATE_ZC
          ...
          if (ready_msg->is_last()) {
            *app_buf_len_p = cur_offset;
            ...
            // UCCL_LOG(INFO, DPDK) << "Received a complete message " <<
            // cur_offset
            //           << " bytes";
          }
...
void UcclEngine::process_rx_msg(Packet** pkts, uint32_t rcvd) {
  for (uint32_t i = 0; i < rcvd; i++) {
    ...
    if (it == active_flows_map_.end()) {
      UCCL_LOG_EVERY_N(ERROR, DPDK, 1000000)
...
void UcclEngine::handle_install_flow_on_engine(Channel::CtrlMsg& ctrl_work) {
  UCCL_LOG(INFO, DPDK) << "[Engine] handle_install_flow_on_engine "
                       << local_engine_idx_;
  int ret;
  ...
  UCCL_LOG(INFO, DPDK) << "[Engine] start RSS probing";
  ...
  for (uint32_t i = BASE_PORT; i < 65536;
    ...
    // UCCL_LOG(INFO, DPDK) << "dst_port: " << dst_port << " sent: " << sent;
  ...
  UCCL_LOG(INFO, DPDK) << "[Engine] install FlowID " << std::hex << "0x"
                       << flow_id << ": " << local_ip_str
                       << Format("(%d)", local_engine_idx_) << " <-> "
...
Endpoint::Endpoint(uint16_t port_id, int num_queues, int engine_cpu_start,
  ...
  UCCL_LOG(INFO, DPDK) << "Creating DPDKFactory";
  ...
  UCCL_LOG(INFO, DPDK) << "Creating Channels";
  ...
  UCCL_LOG(INFO, DPDK) << "Creating Engines";
    ...
    engine_th_vec_.emplace_back(std::make_unique<std::thread>(
        [this, num_queues, i, engine_th_cpuid = engine_cpu_start + i,
         engine_promise = std::move(engine_promise)]() mutable {
          ...
          UCCL_LOG(INFO, DPDK) << "[Engine] thread " << i << " running on CPU "
                               << engine_th_cpuid;
    ...
    deser_th_vec_.emplace_back(std::make_unique<std::thread>(
        [i, deser_th_cpuid = engine_cpu_start + num_queues + i,
         engines = std::vector<UcclEngine*>{engines[i]}]() {
          ...
          UCCL_LOG(INFO, DPDK) << "[Engine] deser thread " << i
  ...
  UCCL_LOG(INFO, DPDK) << "[Endpoint] server ready, listening on port "
                       << kBootstrapPort;
}
...
ConnID Endpoint::uccl_connect(std::string bootstrap_remote_ip,
  ...
  UCCL_LOG(INFO, DPDK) << "[Endpoint] connecting to " << bootstrap_remote_ip
                       << ":" << kBootstrapPort;
  ...
  while (true) {
    ...
    UCCL_LOG(INFO, DPDK) << "[Endpoint] connect: receive proposed FlowID: "
                         << std::hex << "0x" << flow_id;
  ...
  UCCL_LOG(INFO, DPDK) << "[Endpoint] remote IP: " << remote_ip;
...
ConnID Endpoint::uccl_accept() {
  ...
  UCCL_LOG(INFO, DPDK) << "[Endpoint] accept from " << client_ip << ":"
                       << cli_addr.sin_port;
  ...
  while (true) {
    ...
    UCCL_LOG(INFO, DPDK) << "[Endpoint] accept: propose FlowID: " << std::hex
                         << "0x" << flow_id;
