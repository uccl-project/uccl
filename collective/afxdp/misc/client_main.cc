...
static char const* server_mac_str = "9c:dc:71:56:af:45";
static char const* client_mac_str = "9c:dc:71:5b:22:91";
static char const* server_ip_str = "192.168.6.1";
static char const* client_ip_str = "192.168.6.2";
...
int const PAYLOAD_BYTES = 1000;
// tune this to change packet rate
int const MAX_INFLIGHT_PKTS = 128;
...
int client_init(struct client_t* client, char const* interface_name) {
    ...
    for (struct ifaddrs* iap = addrs; iap != NULL; iap = iap->ifa_next) {
      if (iap->ifa_addr && (iap->ifa_flags & IFF_UP) &&
        ...
        if (strcmp(interface_name, iap->ifa_name) == 0) {
          LOG(INFO, AFXDP) << "found network interface: '" << iap->ifa_name
                           << "'\n";
          client->interface_index = if_nametoindex(iap->ifa_name);
          if (!client->interface_index) {
            LOG(ERROR, AFXDP) << "error: if_nametoindex failed\n";
            return 1;
          }
          found = true;
          break;
  ...
  LOG(INFO, AFXDP) << "loading ebpf_client...";

  client->program =
      xdp_program__open_file("ebpf_client.o", "ebpf_client", NULL);
  if (libxdp_get_error(client->program)) {
    LOG(ERROR, AFXDP) << "could not load ebpf_client program";
    return 1;
  }

  LOG(INFO, AFXDP) << "ebpf_client loaded successfully.";

  LOG(INFO, AFXDP) << "attaching ebpf_client to network interface";
  ...
  if (ret == 0) {
    ...
    LOG(WARNING, AFXDP) << "falling back to skb mode...";
    ret = xdp_program__attach(client->program, client->interface_index,
                              XDP_MODE_SKB, 0);
    if (ret == 0) {
      client->attached_skb = true;
    } else {
      LOG(ERROR, AFXDP) << "failed to attach ebpf_client program to interface";
      return 1;
    }
  }
  ...
  if (setrlimit(RLIMIT_MEMLOCK, &rlim)) {
    LOG(ERROR, AFXDP) << "could not setrlimit";
    return 1;
  }
  ...
  for (int i = 0; i < MY_NUM_QUEUES; i++) {
    ...
    if (posix_memalign(&client->socket[i].umem_buffer, getpagesize(),
                       buffer_size)) {
      LOG(ERROR, AFXDP) << "could not allocate umem_buffer";
      return 1;
    }
    ...
    if (ret) {
      LOG(ERROR, AFXDP) << "could not create umem";
      return 1;
    }
    ...
    ret = xsk_socket__create(&client->socket[i].xsk, interface_name, queue_id,
                             client->socket[i].umem,
                             &client->socket[i].recv_queue,
                             &client->socket[i].send_queue, &xsk_config);
    if (ret) {
      LOG(ERROR, AFXDP) << "could not create xsk socket [" << queue_id << "]";
      return 1;
    }
  ...
  for (int i = 0; i < MY_NUM_QUEUES; i++) {
    ret = pthread_create(&client->recv_thread[i], NULL, recv_thread,
                         &client->socket[i]);
    if (ret) {
      LOG(ERROR, AFXDP) << "could not create socket recv thread #" << i;
      return 1;
    }
    ...
    ret = pthread_create(&client->send_thread[i], NULL, send_thread,
                         &client->socket[i]);
    if (ret) {
      LOG(ERROR, AFXDP) << "could not create socket send thread #" << i;
      return 1;
    }
  }
  ...
  if (ret) {
    LOG(ERROR, AFXDP) << "could not create stats thread";
    return 1;
  }
...
void socket_send(struct socket_t* socket, int queue_id) {
  if (inflight_pkts >= MAX_INFLIGHT_PKTS) {
    ...
    if (socket->last_stall_time == 0) {
      ...
      LOG(WARNING, AFXDP) << "queue " << queue_id
  ...
  if (completed > 0) {
    for (int i = 0; i < completed; i++) {
      uint64_t frame_offset =
          *xsk_ring_cons__comp_addr(&socket->complete_queue, complete_index++);
...
void socket_recv(struct socket_t* socket, int queue_id) {
  ...
  if (stock_frames > 0) {
    ...
    while (ret != stock_frames)
      ret = xsk_ring_prod__reserve(&socket->fill_queue, stock_frames, &idx_fq);
...
static void* send_thread(void* arg) {
  ...
  LOG(INFO, AFXDP) << "started socket send thread for queue #" << queue_id;
...
static void* recv_thread(void* arg) {
  ...
  if (xsk_map_fd < 0) {
    LOG(ERROR, AFXDP) << "no xsks map found: " << strerror(xsk_map_fd);
    exit(0);
  }
  int ret = xsk_socket__update_xskmap(socket->xsk, xsk_map_fd);
  if (ret) {
    LOG(ERROR, AFXDP) << "xsks map update fails: " << strerror(xsk_map_fd);
    exit(0);
  }
  ...
  int queue_id = socket->queue_id;
  LOG(INFO, AFXDP) << "started socket recv thread for queue #" << queue_id;
...
static void* stats_thread(void* arg) {
  ...
  while (!quit) {
    ...
    LOG(INFO, AFXDP) << "send delta: " << sent_delta
                     << ", med rtt: " << med_latency
  ...
  LOG(INFO, AFXDP) << "Throughput: " << std::fixed << std::setprecision(2)
