...
void runServer() {
  ...
  if (serverSocket == -1) {
    LOG(FATAL, AFXDP) << "Socket creation failed: " << strerror(errno);
    exit(EXIT_FAILURE);
  }
  int flag = 1;
  if (setsockopt(serverSocket, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(int)) <
      0)
    LOG(FATAL, AFXDP) << "setsockopt(SO_REUSEADDR) failed";
  ...
  if (bind(serverSocket, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) <
      0) {
    LOG(FATAL, AFXDP) << "Bind failed: " << strerror(errno);
    close(serverSocket);
    exit(EXIT_FAILURE);
  }

  if (listen(serverSocket, 5) < 0) {
    LOG(FATAL, AFXDP) << "Listen failed: " << strerror(errno);
    close(serverSocket);
    exit(EXIT_FAILURE);
  }

  LOG(INFO, AFXDP) << "Server listening on port " << PORT;
  ...
  while (true) {
    ...
    if (clientSocket < 0) {
      LOG(WARNING, AFXDP) << "Accept failed: " << strerror(errno);
      continue;
    }
    setsockopt(clientSocket, IPPROTO_TCP, TCP_NODELAY, (void*)&flag,
               sizeof(int));

    LOG(INFO, AFXDP) << "Connection accepted from "
    ...
    close(clientSocket);
    LOG(INFO, AFXDP) << "Client disconnected.";
  }
...
void runClient(std::string const& serverIP) {
  ...
  if (inet_pton(AF_INET, serverIP.c_str(), &serverAddr.sin_addr) <= 0) {
    LOG(FATAL, AFXDP) << "Invalid server IP address: " << serverIP;
    exit(EXIT_FAILURE);
  }
  ...
  for (int i = 0; i < NUM_ITERATIONS; ++i) {
    ...
    if (connect(clientSocket, (struct sockaddr*)&serverAddr,
                sizeof(serverAddr)) < 0) {
      LOG(FATAL, AFXDP) << "Connection to server failed: " << strerror(errno);
      close(clientSocket);
      exit(EXIT_FAILURE);
    }
    ...
    LOG(INFO, AFXDP) << "Iteration " << i + 1
                     << ": Median Latency = " << medLatencies.back()
                     << " µs, Tail Latency = " << tailLatencies.back() << " µs";
  }

  LOG(INFO, AFXDP) << "Completed " << NUM_ITERATIONS << " iterations.";
  LOG(INFO, AFXDP) << "Median of median Latencies: "
                   << uccl::Percentile(medLatencies, 50) << " µs";
  LOG(INFO, AFXDP) << "Tail of median Latencies: "
                   << uccl::Percentile(medLatencies, 99) << " µs";
  LOG(INFO, AFXDP) << "Median of tail Latencies: "
                   << uccl::Percentile(tailLatencies, 50) << " µs";
  LOG(INFO, AFXDP) << "Tail of tail Latencies: "
...
int main(int argc, char* argv[]) {
  ...
  if (FLAGS_role == "server") {
    ...
    LOG(FATAL, AFXDP) << "Unknown role: " << FLAGS_role
