...
ncclResult_t pluginGetProperties(int pdev, ncclNetProperties_v8_t* props) {
  ...
  UCCL_LOG(INFO, UCCL_EFA) << "pluginGetProperties dev " << pdev << " guid "
                           << props->guid << " name " << props->name
                           << " pciPath " << props->pciPath;
...
ncclResult_t pluginListen(int vdev, void* opaque_handle, void** listenComm) {
  ...
  if (vdev != gpu_idx) {
    UCCL_LOG_FIRST_N(INFO, UCCL_EFA, 1)
        << "pluginListen detects different vdev " << vdev << " vs. gpu_idx "
        << gpu_idx << ", forcely setting vdev to gpu_idx";
    vdev = gpu_idx;
  }

  UCCL_LOG(INFO, UCCL_EFA) << "[pluginListen] pid=" << getpid()
                           << ", using GPU " << gpu_idx << ", vdev=" << vdev;
  ...
  *listenComm = lcomm;
  ...
  UCCL_LOG(INFO, UCCL_EFA) << "pluginListen on vdev: " << vdev
                           << " listen_port " << listen_port << " listen_fd "
                           << listen_fd << " gpu_idx " << gpu_idx;
...
ncclResult_t pluginConnect(int vdev, void* opaque_handle, void** sendComm,
  ...
  if (handle->state == kConnInit) {
    UCCL_LOG(INFO, UCCL_EFA)
        << "pluginConnect on vdev: " << vdev << " remote_ip_str "
        << remote_ip_str << " dest_port " << handle->listen_port << " gpu_idx "
        << gpu_idx;
  ...
  if (*sendComm) {
    UCCL_LOG(INFO, UCCL_EFA)
        << Format("Connected to %s/%d on vdev %d with flow_id %lu\n",
...
ncclResult_t pluginAccept(void* listenComm, void** recvComm,
  ...
  if (lcomm->state == kConnInit) {
    ...
    UCCL_LOG(INFO, UCCL_EFA)
        << "pluginAccept on vdev: " << vdev << " listen_fd " << lcomm->listen_fd
        << " gpu_idx " << gpu_idx;
  ...
  if (*recvComm) {
    UCCL_LOG(INFO, UCCL_EFA)
        << Format("Accepted from %s/%d on vdev %d with flow_id %lu\n",
...
ncclResult_t pluginRegMr(void* collComm, void* data, size_t size, int type,
  ...
  UCCL_LOG(INFO, UCCL_EFA) << "pluginRegMr, size " << size << " flow_id "
...
ncclResult_t pluginRegMrDmaBuf(void* collComm, void* data, size_t size,
  ...
  UCCL_LOG(INFO, UCCL_EFA) << "pluginRegMrDmaBuf, size " << size << " flow_id "
                           << base->conn_id.flow_id << " vdev_idx " << vdev_idx
                           << " data ptr " << std::hex << data;
...
ncclResult_t pluginDeregMr(void* collComm, void* mhandle) {
  ...
  UCCL_LOG(INFO, UCCL_EFA) << "pluginDeregMr, " << base->conn_id.flow_id;
...
ncclResult_t pluginIsend(void* sendComm, void* data, int size, int tag,
  ...
  *request = req;
  // UCCL_LOG(INFO, UCCL_EFA) << "pluginIsend on size " << size;
