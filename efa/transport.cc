...
namespace uccl
{
    ...
    uint32_t TXTracking::convert_permitted_packets_to_bytes(uint32_t permitted_packets)
    {
        ...
        while (msgbuf != nullptr)
        {
            ...
            if (total_packets >= permitted_packets)
                break;
            msgbuf = msgbuf->next();
        }
    ...
    uint32_t TXTracking::convert_permitted_bytes_to_packets(uint32_t permitted_bytes)
    {
        ...
        while (msgbuf != nullptr)
        {
            ...
            permitted_packets++;
            if (total_bytes >= permitted_bytes)
                break;
            msgbuf = msgbuf->next();
        }
    ...
    {
        if (unlikely(ts4 <= ts1 || ts3 <= ts2))
            return;
        auto sender_latency = ts4 - ts1;
        auto receiver_latency = ts3 - ts2;
        if (unlikely(sender_latency <= receiver_latency))
            return;
    ...
    void UcclFlow::process_ack(const UcclPktHdr *ucclh)
    {
        ...
        else if (swift::seqno_eq(ackno, pcb_.snd_una))
        {
            ...
            else
            {
                ...
                if (num_unacked_pkts >= kMaxUnackedPktsPerEngine)
                    return;
    ...
    bool Endpoint::create_engine_and_add_to_engine_future(int engine_idx, int gpu_idx, std::vector<std::future<std::unique_ptr<UcclEngine>>> &engine_futures)
    {

        LOG(INFO) << "Creating Engines by Engine Idx " << engine_idx << " GPU Idx " << gpu_idx;
        {
    ...
    Endpoint::Endpoint() : stats_thread_([this]()
                                         { stats_thread_fn(); })
    {
    ...
    // TODO(MaoZiming): Deprecate this constructor.
    Endpoint::Endpoint(int gpu) : gpu_(gpu), stats_thread_([this]()
                                                           { stats_thread_fn(); })
    {
    ...
    Endpoint::~Endpoint()
    {
        ...
        for (auto &engine_th : engine_th_vec_)
            engine_th->join();
        for (auto &copy_th : copy_th_vec_)
            copy_th->join();
        for (int i = 0; i < kNumEngines; i++)
            delete channel_vec_[i];
        for (auto listen_fd : listen_fd_vec_)
            close(listen_fd);
        delete ctx_pool_;
                                          ...
                                          int bootstrap_fd, bool is_sender)
    {
        ...
        // Wait until the flow has been installed on the engine.
        {
            ...
            poll_ctx->cv.wait(lock, [&poll_ctx]
                              { return poll_ctx->done.load(); });
        }
        delete poll_ctx;
    ...
    inline int Endpoint::find_dedicated_engine_idx(int vdev_idx, bool is_sender)
    {
        ...
        if (!is_sender)
            minElementIter = engine_load_vec_.begin() + ei - 1;
