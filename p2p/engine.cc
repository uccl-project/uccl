...
bool Endpoint::read(uint64_t conn_id, uint64_t mr_id, void* dst, size_t size,
  ...
  // No application-level chunking - RDMA layer handles it
  ucclRequest ureq = {};
  FifoItem curr_slot_item = slot_item;
  curr_slot_item.size = size;

  while (uccl_read_async(ep_, conn, mhandle, dst, size, curr_slot_item, &ureq) == -1);
...
bool Endpoint::readv(uint64_t conn_id, std::vector<uint64_t> mr_id_v,
  ...
  // IOV-level throttling: limit concurrent IOVs, let RDMA handle chunking
  ...
  while (iov_finished < num_iovs) {
    ...
    while (iov_issued < num_iovs &&
      ...
      if (unlikely(mhandle == nullptr)) {
        std::cerr << "[readv] Error: Invalid mr_id " << mr_id_v[iov_issued] << std::endl;
        return false;
      }

      auto rc = uccl_read_async(
          ep_, conn, mhandle, dst_v[iov_issued],
    ...
    while (iov_finished < iov_issued &&
           done[iov_finished % kMaxInflightOps]) {
      iov_finished++;
    }
  }
...
bool Endpoint::writev(uint64_t conn_id, std::vector<uint64_t> mr_id_v,
  ...
  while (iov_finished < num_iovs) {
    ...
    while (iov_issued < num_iovs &&
      ...
      P2PMhandle* mhandle = get_mhandle(mr_id_v[iov_issued]);
      if (unlikely(mhandle == nullptr)) {
        std::cerr << "[writev] Error: Invalid mr_id " << mr_id_v[iov_issued] << std::endl;
        return false;
      }

      auto rc = uccl_write_async(
          ep_, conn, mhandle, src_v[iov_issued],
          size_v[iov_issued], slot_item_v[iov_issued],
          &ureq[iov_issued % kMaxInflightOps]);
      if (rc == -1) break;
      done[iov_issued % kMaxInflightOps] = false;
...
bool Endpoint::write(uint64_t conn_id, uint64_t mr_id, void* src, size_t size,
  ...
  while (uccl_write_async(ep_, conn, mhandle, src, size, curr_slot_item, &ureq) == -1);
