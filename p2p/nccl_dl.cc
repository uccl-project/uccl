...
static void* nccl_resolve(char const* name) { return uccl::nccl_dl::resolve(name); }
...
ncclResult_t ncclSend(const void* sendbuff, size_t count, ncclDataType_t datatype,
  ...
  using FnType = ncclResult_t(*)(const void*, size_t, ncclDataType_t, int,
...
const char* ncclGetErrorString(ncclResult_t result) {
  using FnType = const char*(*)(ncclResult_t);
  static FnType fn = reinterpret_cast<FnType>(nccl_resolve("ncclGetErrorString"));
  return fn(result);
}
