 ...
 * WARNING: Must match plugin implementation - changes to plugin may require updates.
 * Reference: nccl-plugin-gpudirecttcpx/src/work_queue.h, devcomm/unpack_defs1.h
 */
...
// Scatter-gather descriptor (one fragment to copy from bounce buffer to destination)
// Layout must be exactly 16 bytes for GPU kernel efficiency
struct loadMeta {
...
static_assert(sizeof(loadMeta) == 16, "loadMeta must remain 16 bytes for GPU alignment");
