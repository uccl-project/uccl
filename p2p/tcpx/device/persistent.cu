...
IovMultiFifo
*/
IovFifo** IovMultiFifo::get_fifo_vec() {
    return (struct IovFifo **)fifo_vec_;
}

std::tuple<uint64_t, volatile struct Iov *> IovMultiFifo::reserve_fifo_slot(int fifo_idx) {
...
PersistentKernel
*/
PersistentKernel::PersistentKernel(const PersistentKernelConfig &config)
...
bool PersistentKernel::launch(KernelFuncType kernelFunc) {
    ...
    kernelFunc<<<cfg_.numThBlocks, cfg_.numThPerBlock, cfg_.smem_size, stream_>>>(fifo_->get_fifo_vec(), cfg_.fifoCap);
...
uint64_t PersistentKernel::submit(int fifo_idx, const Iov &iov) {
    ...
    const tcpx::rx::UnpackDescriptorBlock &desc_block)
{
...

    {
        ...
        desc_id_to_info_.emplace(desc_id, DescriptorInfo{channel_id, std::move(slot_ids)});
    }
