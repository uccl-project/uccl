...
public:
    IovMultiFifo(int num_fifo, int fifo_cap) : num_fifo_(num_fifo), fifo_cap_(fifo_cap) {
        fifo_vec_ = new volatile IovFifo*[num_fifo_];
...
public:
    ...
    uint64_t submit(int fifo_id, const Iov &iov);
