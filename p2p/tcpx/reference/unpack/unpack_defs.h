...
#define NET_UNPACK_MAX_SLICE_PAGES               \
  (NET_UNPACK_MAX_SLICE_SIZE / SLICE_PAGE_SIZE * \
   2)  // * 2 for slack, wasteful..
...
#define NET_UNPACK_MAX_NPEERS \
  2  // The most you should have is 2 network peers per-group (indexed by index)
...
struct unpackGroupShmem {
  int unpackNetDeviceIndexMask;  // We store a single unpackNetDeviceIndex
                                 // because only one peer can be network recv
  uint64_t head[NET_UNPACK_MAX_NPEERS];
  struct netUnpackMeta*
      g_meta[NET_UNPACK_MAX_NPEERS];  // head of handle to index into meta for
                                      // meta copy
};
