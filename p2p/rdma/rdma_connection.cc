...
int64_t SendConnection::send(std::shared_ptr<RDMASendRequest> req) {
  ...
  // get_total_inflight_bytes() only reflects requests actually being sent.
  int64_t wr_id = tracker_->send_packet(0);
  req->wr_id = wr_id;
...
int SendConnection::process_send_requests(
    std::shared_ptr<RDMASendRequest> req) {
  poll_control_channel();
  ...
  SendReqMeta meta;
  std::shared_lock<std::shared_mutex> lock(ctrl_channel_mutex_);
  int index = ctrl_channel_->get_one_send_request_meta(meta);
  if (index < 0) {
    return -1;
  }
  int64_t wr_id = tracker_->send_packet(req->get_local_len());
  ...
  process_once_send_requests(req, meta, index);
  return wr_id;
}
...
void SendConnection::process_send_requests() {
  if (unlikely(ctrl_channel_ == nullptr)) {
    return;
  }
  ...
  // {
  std::shared_lock<std::shared_mutex> lock(ctrl_channel_mutex_);
  has_meta = ctrl_channel_->has_send_request();
  // }
  while (has_meta) {
    std::shared_ptr<RDMASendRequest> req;
    size_t inflight_limit_bytes = current_inflight_limit_bytes();
    if (current_inflight_bytes() > inflight_limit_bytes ||
      ...
      if (current_inflight_bytes() > inflight_limit_bytes) {
        UCCL_LOG(WARN) << "SendConnection: In-flight bytes exceed "
                       ...
                       << current_inflight_bytes() << " bytes in-flight.";
      }
      break;
    }
    // Register actual packet size now that we are about to send it.
    tracker_->update_packet_size(req->wr_id, req->get_local_len());
    index = ctrl_channel_->get_one_send_request_meta(meta);
    UCCL_LOG(INFO, UCCL_RDMA)
        << "SendConnection: Processing send request meta: " << meta;
    process_once_send_requests(req, meta, index);
    has_meta = ctrl_channel_->has_send_request();
...
void SendConnection::process_once_send_requests(
  ...
  // kUndefined means prepare_split_context was skipped; params_dev is null and
  // compression would SIGABRT.
  bool const ctx_ready =
      req->compress_ctx &&
      req->compress_ctx->get_float_type() != FloatType::kUndefined;
  if (ctx_ready && Compressor::get_instance().should_compress_and_split_first(
                       req->local_mem->size)) {
    compress_send_request_split_first(req, meta.expected_chunk_count);
    return;
  }
  if (ctx_ready &&
      Compressor::get_instance().should_compress(req->local_mem->size)) {
    compress_send_request(req);
  }
  tracker_->update_expected_ack_count(
      req->wr_id,
      ChunkSplitStrategy::get_message_chunk_count(req->local_mem->size));
  post_chunked_request(req, meta.expected_chunk_count);
}
...
void RecvConnection::start_polling() {
  ...
  if (ctrl_channel_) ctrl_channel_->set_has_concurrent_poller(true);
  running_.store(true);
  poll_thread_ =
...
void RecvConnection::stop_polling() {
  ...
  if (ctrl_channel_) ctrl_channel_->set_has_concurrent_poller(false);
}

int64_t RecvConnection::recv(std::shared_ptr<RDMARecvRequest> req) {
  if (unlikely(!setup_recv_request_channel_and_memory_region(req))) {
    UCCL_LOG(WARN)
        << "RecvConnection: Failed to setup recv request with round robin";
    return -1;
  }
  // Skip prepare_decompress when float_type is kUndefined
  // (prepare_split_context was never called); it would wrongly replace
  // local_mem with decompressBuffer.
  bool const recv_ctx_ready =
      req->compress_ctx &&
      req->compress_ctx->get_float_type() != FloatType::kUndefined;
  if (recv_ctx_ready &&
      Compressor::get_instance().should_compress(req->local_mem->size)) {
    Compressor::get_instance().prepare_decompress(req);
  }
  return ctrl_channel_->post_send_req(req);
}
...
void RecvConnection::set_remote_ack_ring(RemoteMemInfo const& m) {
  ...
  // poll_async, so user-driven recv_routine() never runs. Force the
        ...
        if (cq_data.has_imm()) {
          ...
          if (ctrl_channel_) {
            ...
            for (int i = 0; i < cq_data.imm.chunk_count(); ++i) {
              ...
              UCCL_LOG(INFO, UCCL_RDMA)
                  << "RecvConnection::poll_and_process_completions - Called "
                     "recv_done("
                  << cq_data.imm.index() << ")";
            }
            if (req_meta && req_meta->float_type != FloatType::kUndefined &&
                Compressor::get_instance().should_compress(
                    req_meta->local_mem.size)) {
              Compressor::get_instance().decompress(req_meta->remote_mem,
                                                    req_meta->local_mem,
                                                    req_meta->float_type);
            }

          } else {
            UCCL_LOG(WARN) << "RecvConnection::poll_and_process_completions - "
...
bool RecvConnection::collect_rkey_for_channel(
...
bool RecvConnection::setup_recv_request_channel_and_memory_region(
  ...
  auto [channel_id, context_id] = select_next_channel_round_robin();
  // auto [channel_id, context_id] = select_next_channel_random();
  if (channel_id == 0) {
    return false;
  }
  req->channel_id = channel_id;
