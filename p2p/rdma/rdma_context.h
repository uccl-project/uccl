...
class RdmaContext {
 public:
  ...
  void getGID(int gid_index, union ibv_gid* gid, int port = 1) const {
    ...
    LOG(INFO, RDMA) << "GID[" << gid_index << "]: " << inet_ntoa(ip);
  }
  ...
  union ibv_gid detectGid(int gid_index, int port = 1) const {
    ...
    if (env) {
      ...
      LOG(INFO, RDMA) << "Using GID index from environment: " << env_gid_index;
            ...
            if (fp) {
              ...
              if (fgets(gid_type, sizeof(gid_type), fp)) {
                if (strstr(gid_type, "RoCE v2") != nullptr) {
                  fclose(fp);
                  LOG(INFO, RDMA) << "RoCE v2 device " << device_name
                                  << ": using GID index " << i;
                  gid_index_ = i;
                  return gid;
                }
    ...
    LOG(INFO, RDMA) << "Auto-detect GID failed, using default " << gid_index;
