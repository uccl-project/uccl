...
std::string const& RdmaDevice::name() const { return name_; }

struct ibv_device* RdmaDevice::get() const {
  return dev_;
}

std::shared_ptr<struct ibv_context> RdmaDevice::open() {
...
RdmaDeviceManager& RdmaDeviceManager::instance() {
...
int RdmaDeviceManager::get_numa_node(size_t id) {
  if (id >= devices_.size()) {
...
RdmaDeviceManager::RdmaDeviceManager() = default;

RdmaDeviceManager::~RdmaDeviceManager() = default;

void RdmaDeviceManager::initialize() {
  int num = 0;
