...
class RDMAChannel {
 public:
  ...
  int64_t read(std::shared_ptr<RDMASendRequest> req) {
    ...
    if (ret != 0) {
      LOG(ERROR, RDMA) << "Failed to post read request, wr_id=" << req->wr_id;
      return -1;
    }
    return req->wr_id;
  }
  ...
  int64_t send(std::shared_ptr<RDMASendRequest> req) {
    int ret = postRequest(req);
    if (ret != 0) {
      LOG(ERROR, RDMA) << "Failed to post send request, wr_id=" << req->wr_id;
      return -1;
    }
    return req->wr_id;
  }
  ...
  int64_t recv(std::shared_ptr<RDMARecvRequest> req) {
    ...
    if (ibv_post_recv(qp_, &wr, &bad_wr)) {
      LOG(ERROR, RDMA) << "ibv_post_recv failed: " << strerror(errno);
    }
    return wr_id;
  }
 ...
 private:
  ...
  inline int __postRequest_ex(std::shared_ptr<RDMASendRequest> req) {
    ...
    if (req->send_type == SendType::Send) {
      ...
      LOG(ERROR, RDMA) << "Unknown SendType in RDMAChannel::postRequest";
      return -1;
    }
    ...
    if (ret) {
      ...
      LOG(ERROR, RDMA) << "ibv_wr_complete failed in postRequest: " << ret
  ...
  inline int __postRequest(std::shared_ptr<RDMASendRequest> req) {
    ...
    if (ret) {
      ...
      LOG(ERROR, RDMA) << "ibv_post_send failed: " << ret << " "
                       << strerror(ret) << ", ah_=" << (void*)ah_
                       << ", remote_qpn=" << remote_meta_->qpn
                       << ", local_qpn=" << qp_->qp_num
                       << ", wr_id=" << req->wr_id
