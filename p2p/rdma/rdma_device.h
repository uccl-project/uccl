...
class RdmaDeviceManager {
 public:
  ...
  std::vector<size_t> get_best_dev_idx(int gpu_idx) {
    ...
    if (dist.empty()) {
      UCCL_LOG(WARNING, RDMA) << "no NIC found, defaulting to empty";
    } else {
    ...
    for (auto const& nic_name : selected_nic_names) {
      ...
      if (dev_idx < 0) {
        UCCL_LOG(FATAL, RDMA) << "Selected RDMA NIC '" << nic_name
  ...
  int get_numa_node(size_t id) {
    if (id >= devices_.size()) {
      UCCL_LOG(WARNING, RDMA) << "Invalid device id: " << id;
      return -1;
    }
