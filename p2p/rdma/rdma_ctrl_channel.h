...
class SendControlChannel : public RDMAChannel {
 public:
  ...
  bool getOneSendRequest(std::shared_ptr<RDMASendRequest>& req) {
    ...
    if (index < 0) {
      LOG(INFO, RDMA) << "getOneSendRequest - Ring buffer is empty, cannot pop";
      return false;
    }
    ...
    LOG(INFO, RDMA) << "getOneSendRequest - Received request: " << *req;
    LOG(INFO, RDMA) << "  SendReqMeta from ring buffer: " << meta;
...
class RecvControlChannel : public RDMAChannel {
 public:
  ...
  int postSendReq(std::shared_ptr<RDMARecvRequest> rev_req) {
    ...
    LOG(INFO, RDMA) << "postSendReq - Created SendReqMeta: " << req_meta;

    int index = rb_->push_with_convert(req_meta, to_ring_meta);
    if (index < 0) {
      LOG(INFO, RDMA) << "postSendReq - Failed to push to ring buffer, index: "
                      << index;
      return index;
    }
