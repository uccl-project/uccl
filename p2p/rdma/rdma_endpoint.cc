...
bool RDMAEndpoint::checkRecvComplete_once(uint64_t peer_id, uint64_t index) {
  UCCL_LOG(INFO, UCCL_RDMA)
      << "check_recv_complete - Checking for peer_id: " << peer_id
      << ", index: " << index;
...
void RDMAEndpoint::check_recv_complete(uint64_t peer_id, uint64_t index) {
  UCCL_LOG(INFO, UCCL_RDMA)
      << "check_recv_complete - Checking for peer_id: " << peer_id
      << ", index: " << index;
  auto it = recv_channel_groups_.find(peer_id);
  ...
  UCCL_LOG(INFO, UCCL_RDMA)
      << "check_recv_complete - Completed for peer_id: " << peer_id
      << ", index: " << index;
}
...
int RDMAEndpoint::send_without_inner_queue(
    std::shared_ptr<RDMASendRequest> req) {
  if (!req) {
    UCCL_LOG(WARN) << "RDMAEndpoint::send_routine - null request";
    return -1;
  }
  ...
  if (it == send_channel_groups_.end()) {
    UCCL_LOG(WARN)
        << "RDMAEndpoint::send_routine - Send channel group not found "
           "for peer_id: "
        << peer_id;
    return -1;
  }
  ...
  auto send_group = it->second;
  if (!send_group) {
    UCCL_LOG(WARN) << "RDMAEndpoint::send_routine - Send channel group is null "
                      "for peer_id: "
                   << peer_id;
    return -1;
  }
  ...
  // from two threads).
  if (send_group->is_running()) {
    return send_group->send(req);
  }

  return send_group->process_send_requests(req);
}
