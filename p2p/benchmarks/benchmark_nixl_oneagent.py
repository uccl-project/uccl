...
def init_transfer_metadata_mc(role: str, operation: str, register_descs, zmq_socket):
    """
...
def do_transfer_mc(role: str, transfer_handle, zmq_socket, uid="TRANSFER"):
    global agent
    if "client" in role:
...
def do_transfer_ucx(role: str, transfer_handle, uid="TRANSFER"):
    global agent
    if "client" in role:
        state = agent.transfer(transfer_handle)
        assert state != "ERR", "Error in transfer"
...
def start_transfer(size, num_kvblocks, args):
    ...
    try:
        ...
        try:
            if args.backend == "mooncake" or args.backend == "uccl":
                register_descs = create_nixl_agent_mc(args.role, dataset, zmq_socket)
                transfer_handle = init_transfer_metadata_mc(
                    args.role, op, register_descs, zmq_socket
                )
            else:
                register_descs = create_nixl_agent_ucx(args.role, dataset)
            ...
            # Restore stdout to see the error
        ...
        if args.backend == "mooncake" or args.backend == "uccl":
            for _ in range(args.iters):
                do_transfer_mc(args.role, transfer_handle, zmq_socket)
                total_size += size
        else:
            for _ in range(args.iters):
                do_transfer_ucx(args.role, transfer_handle)
                total_size += size
...
def main():
    global agent  # Declare that we're using the global agent variable
    ...
    if args.backend == "mooncake" or args.backend == "uccl":
        # Map lowercase command line args to capitalized backend names
        backend_name = "Mooncake" if args.backend == "mooncake" else "Uccl"
    ...
    agent = nixl_agent(args.role, config)
