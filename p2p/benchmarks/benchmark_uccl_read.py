...
def send_oob(payload: bytes, port: int):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        conn, _ = s.accept()
        with conn: conn.sendall(payload)
...
def _run_server_read(args):
    ...
    ok, r_ip, r_gpu, conn_id = ep.accept();  assert ok
...
def _run_client_recv(args):
    if args.remote_ip is None: sys.exit("--remote-ip required")
...
def parse_sizes(v:str)->List[int]:
    try: return [int(x) for x in v.split(",") if x]
    except ValueError: raise argparse.ArgumentTypeError("bad --sizes")
...
def main():
    ...
    if args.async_transfer: print("Async path enabled")
