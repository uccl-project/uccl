"""
Benchmark NCCL AllToAll (baseline for comparison with UCCL)

Usage:
    torchrun --nnodes=2 --nproc_per_node=8 --node-rank=0 --master_addr=<IP> --master_port=19999 \
        benchmark_nccl_alltoall_collective.py --sizes 1024,4096,16384,65536,262144,1048576 --num-iters 100

    # Single node multi-GPU:
    torchrun --standalone --nproc_per_node=4 benchmark_nccl_alltoall_collective.py --sizes 65536,262144,1048576 --num-iters 100
"""
...
def warmup_alltoall_check(
    ...
    """
    Warmup and verification for NCCL alltoall operation.
    """
    print("\n🔧 Running NCCL warmup_alltoall_check...")
    ...
    if rank == 0:
        if all_passed:
            print(f"[Rank {rank}] ✅ NCCL AllToAll verification PASSED")
        else:
            print(f"[Rank {rank}] ❌ NCCL AllToAll verification FAILED")
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark NCCL AllToAll (baseline for UCCL comparison)"
    )
    parser.add_argument(
        "--sizes",
    ...
    parser.add_argument(
        "--csv-output",
        type=str,
        default="nccl_alltoall_results.csv",
    ...
    try:
        if rank == 0:
            print("=" * 70)
            print("NCCL AllToAll Benchmark (Baseline)")
            print("=" * 70)
