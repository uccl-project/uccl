// epoll_client.h
// Header file containing epoll-based client implementation with connection reuse
...
struct ClientConnection {
    ...
    std::queue<std::function<void(const std::string&)>> response_callbacks;
...
public:
    ...
    std::string connect_to_server(const std::string& server_ip, int server_port) {
        ...
        std::cout << "Connected to " << server_ip << ":" << server_port << " (fd=" << sock << ")\n";
        return conn_key;
    }
    ...
    bool send_meta(const std::string& conn_key, const std::string& payload, ResponseCallback callback = nullptr) {
        ...
        if (sent < (ssize_t)pkt.size()) {
            conn.out_buf.insert(conn.out_buf.end(), pkt.data() + sent, pkt.data() + pkt.size());
        }
...
private:
    ...
    void handle_read(const std::string& conn_key) {
        char buf[4096];
    ...
    void handle_write(const std::string& conn_key) {
        std::lock_guard<std::mutex> lk(conns_mtx_);
        auto it = conns_.find(conn_key);
        if (it == conns_.end()) return;
        ClientConnection& conn = it->second;
    ...
    void close_connection(const std::string& conn_key) {
