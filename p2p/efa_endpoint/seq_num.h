...
class AtomicBitmapPacketTracker {
 ...
 public:
  ...
  uint32_t sendPacket(size_t packet_size = 0) {
    ...
    packet_sizes_[seq_num % WINDOW_SIZE].store(packet_size, std::memory_order_release);
    return seq_num;
  }
  ...
  size_t getTotalInflightBytes() const {
    ...
    for (uint32_t i = 0; i < WINDOW_SIZE; ++i) {
      ...
      if (seq_num < next &&
          !ack_bitmap_[seq_num % WINDOW_SIZE].load(std::memory_order_acquire)) {
        total_bytes += packet_sizes_[seq_num % WINDOW_SIZE].load(std::memory_order_acquire);
      }
    }
    return total_bytes;
  }
