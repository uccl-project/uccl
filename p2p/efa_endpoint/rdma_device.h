...
public:
    ...
    struct ibv_device* get() const { return dev_; }
    ...
    std::shared_ptr<struct ibv_context> open() {
        ...
        return std::shared_ptr<struct ibv_context>(ctx, [](ibv_context* c) {
            ibv_close_device(c);
        });
    }
...
private:
    ...
    void initialize() {
        ...
        std::cout << "RdmaDeviceManager: Found " << num << " RDMA device(s)" << std::endl;
        for (int i = 0; i < num; ++i) {
