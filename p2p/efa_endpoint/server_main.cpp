...
class MetaProcessor {
public:
    MetaProcessor(const std::string& name) : name_(name), count_(0) {}
...
// ---------------------------
void my_process_meta(const std::string& input, std::string& output) {
    // Deserialize input to MetaInfoToExchange
    MetaInfoToExchange meta = deserialize<MetaInfoToExchange>(input);
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1 && (std::string(argv[1]) == "-h" || std::string(argv[1]) == "--help")) {
        ...
        std::cout << "\nExample:\n";
        std::cout << "  " << argv[0] << " 0 member    # Auto-assign port, use member function\n";
    ...
    if (use_member_func) {
        ...
        // Create server with lambda that captures processor and calls member function
        EpollServer server(port,
            [&processor](const std::string& input, std::string& output) {
        ...
        std::cout << "Server started on 0.0.0.0:" << actual_port << " (epoll, non-blocking)\n";
        std::cout << "Processing mode: Synchronous (blocking callback)\n";
        std::cout << "Callback type: Member function (MetaProcessor::process_meta)\n";
        ...
        server.stop();
        std::cout << "Server stopped. Total requests processed: " << processor.get_count() << "\n";
    } else {
