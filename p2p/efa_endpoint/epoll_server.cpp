// epoll_server.cpp
// g++ -std=c++17 epoll_server.cpp -o epoll_server -pthread
...
// ---------------------------
void run_test_client(const std::string& server_ip, int port, int client_id) {
    ...
    m.value = 3.14 + client_id;
    snprintf(m.message, sizeof(m.message), "hello from client %d", client_id);

    std::string payload = serialize(m);
    ...
    ssize_t s = send(sock, pkt.data(), pkt.size(), 0);
    if (s < 0) perror("send");
    ...
    if (n > 0) {
        buf[n] = '\0';
        std::cout << "Client " << client_id << " got ack: " << buf;
    } else if (n == 0) {
        std::cout << "Client " << client_id << " server closed\n";
    } else {
        perror("recv");
    }
    close(sock);
...
// ---------------------------
// main: start server in background and spawn some test clients
// ---------------------------
int main() {
    int port = 9000;
    EpollServer server(port, my_process_meta, 8 /*thread pool size*/);

    if (!server.start()) {
        std::cerr << "Failed to start server\n";
        return 1;
    }
    std::cout << "Server started on port " << port << " (epoll, non-blocking)\n";

    // Spawn test clients to demonstrate concurrency
    std::vector<std::thread> clients;
    for (int i = 0; i < 16; ++i) {
        clients.emplace_back(run_test_client, "127.0.0.1", port, i);
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
    }
    for (auto& t : clients) t.join();

    // Let server run a bit to process
