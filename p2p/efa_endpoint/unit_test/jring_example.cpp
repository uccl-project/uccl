...
void example_spsc() {
  ...
  struct jring* ring =
      (struct jring*)aligned_alloc(CACHE_LINE_SIZE, ring_mem_size);
  if (!ring) {
  ...
  printf("Ring count: %u, free count: %u, empty: %d, full: %d\n",
         jring_count(ring), jring_free_count(ring), jring_empty(ring),
         jring_full(ring));
  ...
  for (unsigned int i = 0; i < dequeued; i++) {
    printf("  Message[%u]: id=%u, data=%u, payload=%s\n", i, recv_msgs[i].id,
           recv_msgs[i].data, recv_msgs[i].payload);
  }
...
void example_mpmc() {
  ...
  int const num_producers = 2;
  int const num_consumers = 2;
  int const items_per_thread = 5;
