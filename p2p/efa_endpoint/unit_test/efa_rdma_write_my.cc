...
void run_server(struct rdma_context* rdma, int gid_index) {
  ...
  // Now server sends response back to client
  printf("Server sending response to client...\n");

#ifdef USE_GPU
  h_data = (char*)malloc(MSG_SIZE);
  strcpy(h_data, "Server Response: Message received!");
  cudaMemcpy(rdma->local_buf, h_data, MSG_SIZE, cudaMemcpyHostToDevice);
  free(h_data);
#else
  strcpy(rdma->local_buf, "Server Response: Message received!");
#endif
  ...
  ibv_wr_rdma_write_imm(qpx, remote_meta.rkey, remote_meta.addr, 0x3);

  struct ibv_sge sge_send[1] = {
      {(uintptr_t)rdma->local_buf, MSG_SIZE, rdma->mr->lkey}};

  ibv_wr_set_sge_list(qpx, 1, sge_send);
  ibv_wr_set_ud_addr(qpx, rdma->ah, remote_meta.qpn, QKEY);

  if (ibv_wr_complete(qpx)) {
    printf("ibv_wr_complete failed\n");
  }
  printf("Server: Response sent: wr_id=%lx, length=%d\n", qpx->wr_id, MSG_SIZE);

  // Wait for send completion
  total_pull_count = 0;
  while (total_pull_count < 1) {
    total_pull_count += poll_cq(rdma, IBV_WC_RDMA_WRITE);
  }
...
void run_client(struct rdma_context* rdma, char const* server_ip,
  ...
  // prepare message
  char* h_data = (char*)malloc(MSG_SIZE);
  strcpy(h_data, "Hello World from Client!");
  cudaMemcpy(rdma->local_buf, h_data, MSG_SIZE, cudaMemcpyHostToDevice);
#else
  strcpy(rdma->local_buf, "Hello World from Client!");
#endif
  ...
  // Wait for send completion
  printf("Client polling message send completion...\n");
  ...
  // Now client waits for server response
  printf("Client waiting for server response...\n");

  // Post receive buffer to get server response
  struct ibv_sge sge_recv[1] = {
      {(uintptr_t)rdma->local_buf, MSG_SIZE, rdma->mr->lkey}};
  ...
  totol_pull_count = 0;
  while (totol_pull_count < 1) {
    totol_pull_count += poll_cq(rdma, IBV_WC_RECV_RDMA_WITH_IMM);
  }
...
void test_atomic_operations(struct rdma_context* rdma, metadata* remote_meta) {
  ...
  try {
    ...
    if (ret != 0) {
      printf(
          "  ERROR: ibv_wr_complete failed for atomic operation (error: %d)\n",
          ret);
      printf(
          "  This indicates atomic operations are NOT supported by this "
          "device/QP\n");
    } else {
