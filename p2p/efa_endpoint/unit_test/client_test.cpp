...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1 &&
    ...
    std::cout << "Usage: " << argv[0]
              << " [server_ip] [port] [num_messages] [client_id_start]\n";
  ...
  std::cout << "Client started, connecting to " << server_ip << ":"
            << server_port << "\n";

  // Connect once
  std::string conn_key = client.connect_to_server(server_ip, server_port);
  if (conn_key.empty()) {
    std::cerr << "Failed to connect to server at " << server_ip << ":"
              << server_port << "\n";
  ...
  for (int i = 0; i < num_messages; ++i) {
    ...
    // Send with callback
    int msg_id = i;
    bool sent = client.send_meta(
        conn_key, serialized_meta,
        [msg_id, &responses_received](std::string const& response) {
          ...
          std::cout << "  channel_meta.qpn: " << response_meta.channel_meta.qpn
                    << "\n";
          std::cout << "  mem_meta.addr: 0x" << std::hex
                    << response_meta.mem_meta.addr << std::dec << "\n";
  ...
  std::cout << "Received " << responses_received << " / " << num_messages
            << " responses\n";
  ...
  // You can call connect_to_server with different IPs/ports and reuse those
  // connections too
