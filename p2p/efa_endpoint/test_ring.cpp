...
enum class MetaDataFlag : int16_t {
    PENDING = 0,
    IN_PROGRESS = 1,
    IS_DONE = 2
};
...
int main() {
    std::cout << "RingBuffer Test with MetaData and shared_ptr<RegMemBlock>" << std::endl;
    ...
    try {
        ...
        bool modified = rb.modify_at(idx3, [](MetaData& item) {
            item.flag.store(static_cast<int16_t>(MetaDataFlag::IS_DONE), std::memory_order_relaxed);
            std::cout << "Modified item, changed flag to IS_DONE" << std::endl;
        });
        std::cout << "Modification result: " << (modified ? "success" : "failed") << std::endl;
        ...
        rb.push(MetaData(0x7000, MetaDataFlag::PENDING));  // This should stop removal
        ...
        size_t removed = rb.remove_while([](const MetaData& item) {
            return item.flag.load(std::memory_order_relaxed) == static_cast<int16_t>(MetaDataFlag::IS_DONE);
        });

        std::cout << "Removed " << removed << " items with IS_DONE flag (expected: 3)" << std::endl;

        // Verify remaining items
        MetaData remaining;
        int count = 0;
        while (rb.pop(remaining)) {
            std::cout << "Remaining item " << count++ << ": address=0x"
                      << std::hex << remaining.address
                      << ", flag=" << std::dec << remaining.flag.load() << std::endl;
        }
        std::cout << "Total remaining items: " << count << " (expected: 2)" << std::endl;
        ...
        //     bulk_data[i].flag.store(static_cast<int16_t>(MetaDataFlag::PENDING), std::memory_order_relaxed);
        // }
        ...
        //     std::cout << "  Item " << i << ": address=0x" << std::hex << popped_data[i].address
        //               << ", flag=" << std::dec << popped_data[i].flag.load() << std::endl;
        // }
