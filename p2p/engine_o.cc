...
bool Endpoint::poll_async(uint64_t transfer_id, bool* is_done) {
  ...
  *is_done = task->done.load(std::memory_order_acquire);
  if (*is_done) {
    fprintf(stderr,
            "[DEBUG poll_async] Deleting HEAP task=%p, type=%d, is_batch=%d\n",
            (void*)task, static_cast<int>(task->type),
            task->is_batch_task() ? 1 : 0);
    delete task;
  }
  return true;
}
...
void Endpoint::send_proxy_thread_func() {
  ...
  UnifiedTask task;
  fprintf(stderr, "[DEBUG send_proxy] STACK task variable at %p\n",
          (void*)&task);
  ...
  while (!stop_.load(std::memory_order_acquire)) {
    if (jring_sc_dequeue_bulk(send_unified_task_ring_, &task, 1, nullptr) ==
        1) {
      fprintf(stderr,
              "[DEBUG send_proxy] Dequeued task: type=%d, self_ptr=%p (heap), "
              "is_batch=%d\n",
              static_cast<int>(task.type), (void*)task.self_ptr,
              task.is_batch_task() ? 1 : 0);
      switch (task.type) {
  ...
  fprintf(stderr,
          "[DEBUG send_proxy] EXITING - stack task destructor will run now! "
          "task.type=%d, task.self_ptr=%p\n",
          static_cast<int>(task.type), (void*)task.self_ptr);
}
...
void Endpoint::recv_proxy_thread_func() {
  uccl::pin_thread_to_numa(numa_node_);
  UnifiedTask task;
  fprintf(stderr, "[DEBUG recv_proxy] STACK task variable at %p\n",
          (void*)&task);
  ...
  while (!stop_.load(std::memory_order_acquire)) {
    if (jring_sc_dequeue_bulk(recv_unified_task_ring_, &task, 1, nullptr) ==
        1) {
      fprintf(stderr,
              "[DEBUG recv_proxy] Dequeued task: type=%d, self_ptr=%p (heap), "
              "is_batch=%d\n",
              static_cast<int>(task.type), (void*)task.self_ptr,
              task.is_batch_task() ? 1 : 0);
      switch (task.type) {
  ...
  fprintf(stderr,
          "[DEBUG recv_proxy] EXITING - stack task destructor will run now! "
          "task.type=%d, task.self_ptr=%p\n",
          static_cast<int>(task.type), (void*)task.self_ptr);
}