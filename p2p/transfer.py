...
class TransferManager:
    ...
    def __init__(self, gpu_idx_list: list[int], num_cpus: int, zmq_port: int = None):
    ...
    def connect(self, remote_ip: str, remote_zmq_port: int) -> bool:
        ...
        socket.send_string(self.local_ip)
        remote_ip = socket.recv_string()
        ...
        for local_gpu_idx in self.gpu_idx_list:
            ...
            if self.local_ip == remote_ip:
                ...
                success, conn_id = self.eps[local_gpu_idx].connect(
                    remote_ip, remote_gpu_idx, self.remote_ep_ports[remote_gpu_idx]
                )
    ...
    def accept(self) -> bool:
        ...
        self.socket.send_string(self.local_ip)
        remote_ip = self.socket.recv_string()
    ...
    def fetch_transfer_metadata(self, transfer_id: int) -> py.bytes:
        transfer_state = self.transfer_table[transfer_id]
        conn = transfer_state.conn
    ...
    def transfer_tensor(self, transfer_id: int) -> bool:
        transfer_state = self.transfer_table[transfer_id]
        conn = transfer_state.conn
        if conn.is_local:
            success = self.eps[conn.local_gpu_idx].send_ipc(
                conn.conn_id, transfer_state.data_ptr, transfer_state.size
            )
        else:
            success = self.eps[conn.local_gpu_idx].send(
                conn.conn_id,
                transfer_state.mr_id,
                transfer_state.data_ptr,
                transfer_state.size,
