...
void test_memory_manager() {
  ...
  constexpr uint32_t kBufA = 101;
  constexpr uint32_t kBufB = 102;
  constexpr uint32_t kBufAResized = 103;

  auto tracked_a = mrm.create_local_mr(kBufA, buf_a.data(), buf_a.size());
  auto tracked_a_again =
      mrm.create_local_mr(kBufA, buf_a.data(), buf_a.size());
  auto tracked_b = mrm.create_local_mr(kBufB, buf_b.data(), buf_b.size());
  MR mr_a = tracked_a.mr;
  MR mr_a_again = tracked_a_again.mr;
  MR mr_b = tracked_b.mr;

  require(mr_a.id == mr_a_again.id, "local buffer id should be stable");
  require(mr_a.id == kBufA, "local MR should use explicit buffer_id");
  require(mr_b.id == kBufB, "local MR should use explicit buffer_id");
  ...
  auto resized =
      mrm.create_local_mr(kBufAResized, buf_a.data(), buf_a.size() / 2);
  require(resized.mr.id == kBufAResized,
          "resized registration should use provided buffer_id");
