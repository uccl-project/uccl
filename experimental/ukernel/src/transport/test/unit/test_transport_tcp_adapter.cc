...
void test_tcp_connect_and_transfer() {
  ...
  unsigned bad_wait = client.wait_signal_async(
      /*peer_rank=*/0, /*expected_tag=*/123, std::nullopt,
      /*comm_rid=*/5);
  ...
  unsigned signal_wait = server.wait_signal_async(
      /*peer_rank=*/1, /*expected_tag=*/42, std::nullopt,
      /*comm_rid=*/3);
  ...
  unsigned signal_wait_bad = server.wait_signal_async(
      /*peer_rank=*/1, /*expected_tag=*/99, std::nullopt,
      /*comm_rid=*/7);
  require(signal_wait_bad != 0, "mismatched signal wait should enqueue");
  unsigned signal_send_bad =
      client.send_signal_async(/*peer_rank=*/0, /*tag=*/100,
                               /*comm_rid=*/8);
...
void test_tcp_invalid_peer_paths() {
  ...
  wait_target.local_buffer_id = 0;
  require(
      adapter.wait_signal_async(/*peer_rank=*/1, /*expected_tag=*/0,
