  ...
  for (int peer_rank = 0; peer_rank < world_size_; ++peer_rank) {
    ...
    if (peer->local_inbox.ring != nullptr) {
      if (peer->local_inbox.creator) {
        UKernel::Transport::destroy_shared_ring(
...
bool ShmRingExchanger::ensure_local_ring(int peer_rank) {
  ...
  peer->local_inbox.ring = UKernel::Transport::create_shared_ring(
...
void ShmRingExchanger::cache_ipc_cache_req_message(int peer_rank,
                                                   uint64_t seq) {
  std::lock_guard<std::mutex> lk(pending_mu_);
