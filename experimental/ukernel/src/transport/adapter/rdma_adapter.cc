...
RdmaTransportAdapter::RdmaTransportAdapter(int local_gpu_idx,
  ...
  signal_mr_ = ibv_reg_mr(pd_, buf, 8,
...
int RdmaTransportAdapter::select_qp(RdmaPeer& p, uint32_t msize) {
  if (msize <= kCacheSizeThresh && p.cached_qp_valid_.load(std::memory_order_acquire)) {
    uint32_t prev = p.consecutive_cached_bytes_.fetch_add(msize, std::memory_order_relaxed);
...
int RdmaTransportAdapter::find_qp_idx(RdmaPeer& p, uint32_t qp_num) {
  for (int i = 0; i < p.num_qps; ++i) {
    if (p.qps[i] && p.qps[i]->qp_num == qp_num) return i;
  }
  return -1;
}
...
bool RdmaTransportAdapter::ensure_put_path(PeerConnectSpec const& spec) {
  ...
  return do_connect(spec.peer_rank, std::get<RdmaPeerConnectSpec>(spec.detail));
}
...
bool RdmaTransportAdapter::ensure_wait_path(PeerConnectSpec const& spec) {
  ...
  if (!std::holds_alternative<RdmaPeerConnectSpec>(spec.detail)) return false;
  return do_accept(spec.peer_rank, std::get<RdmaPeerConnectSpec>(spec.detail));
}
...
bool RdmaTransportAdapter::do_connect(int rank,
  ...
  if (!p) {
    auto new_peer = std::make_unique<RdmaPeer>();
    new_peer->num_qps = remote.num_qps > 0 ? remote.num_qps
                                             : static_cast<uint8_t>(config_.num_qps);
    if (!init_qps_to_init(*new_peer)) return false;
    new_peer->qps_in_init = true;
    {
      std::lock_guard<std::mutex> lk(mu_);
      peers_[rank] = std::move(new_peer);
      p = peers_[rank].get();
    }
  }

  memcpy(p->remote_qpns, remote.remote_qpns, sizeof(p->remote_qpns));
  ...
  if (!qps_to_rtr(*p, remote)) return false;
  if (!qps_to_rts(*p)) return false;
  if (!init_recv_pools(*p)) return false;
  p->put_ready = true;
...
bool RdmaTransportAdapter::do_accept(int rank,
                                     RdmaPeerConnectSpec const& remote) {
  {
    std::lock_guard<std::mutex> lk(mu_);
    auto it = peers_.find(rank);
    if (it != peers_.end() && it->second->wait_ready) return true;
    if (it != peers_.end() && !it->second->qps_in_init) {
      it->second.reset();
    }
  }
...
bool RdmaTransportAdapter::init_qps_to_init(RdmaPeer& p) {
  int cq_size = config_.recv_wr_pool_per_qp * p.num_qps * 2 + kQpMaxSendWr;
  p.cq = ibv_create_cq(ctx_, cq_size, nullptr, nullptr, 0);
  if (!p.cq) return false;
  ...
  for (int i = 0; i < p.num_qps; ++i) {
    p.qps[i] = ibv_create_qp(pd_, &attr);
    if (!p.qps[i]) {
      for (int j = 0; j < i; ++j) {
        ibv_destroy_qp(p.qps[j]);
        p.qps[j] = nullptr;
      }
      ibv_destroy_cq(p.cq);
      p.cq = nullptr;
    ...
    if (ibv_modify_qp(p.qps[i], &qp_attr,
...
bool RdmaTransportAdapter::qps_to_rtr(RdmaPeer& p,
                                      RdmaPeerConnectSpec const& remote) {
  for (int i = 0; i < p.num_qps; ++i) {
    ibv_qp_attr attr = {};
    attr.qp_state = IBV_QPS_RTR;
    attr.path_mtu = IBV_MTU_4096;
    attr.dest_qp_num = remote.remote_qpns[i];
    attr.rq_psn = 0;
    ...
    if (remote.remote_lid != 0) {
      ...
      attr.ah_attr.port_num = 1;
      attr.ah_attr.grh.dgid = p.remote_gid;
    ...
    if (ibv_modify_qp(p.qps[i], &attr, flags) != 0) return false;
  }
  return true;
}

bool RdmaTransportAdapter::qps_to_rts(RdmaPeer& p) {
  for (int i = 0; i < p.num_qps; ++i) {
    ibv_qp_attr attr = {};
...
bool RdmaTransportAdapter::init_recv_pools(RdmaPeer& p) {
  ...
  for (int q = 0; q < p.num_qps; ++q) {
    ...
    if (ibv_post_recv(p.qps[q], &pool->wrs[0], &bad) != 0) {
      return false;
    }
...
bool RdmaTransportAdapter::repost_one_recv(RdmaPeer& p, int qp_idx) {
  ...
  ibv_recv_wr* bad = nullptr;
  if (ibv_post_recv(p.qps[qp_idx], &pool->wrs[idx], &bad) != 0) return false;
...
unsigned RdmaTransportAdapter::put_async(int rank, void* local_ptr,
  ...
  for (uint32_t ci = 0; ci < ck.count; ++ci) {
    ...
    uint32_t imm = imm_data_chunks(static_cast<uint16_t>(ck.count),
    ...
    if (ibv_post_send(p->qps[q], &wr, &bad) != 0) {
...
unsigned RdmaTransportAdapter::signal_async(int rank, uint64_t tag) {
  ...
  int q = select_qp(*p, 0);
  uint64_t wr_id = static_cast<uint64_t>(rid) << 32;

  ibv_sge sge = {};
  sge.addr = signal_addr_;
  sge.length = 0;
  ...
  wr.imm_data = imm_signal(tag);
  wr.wr.rdma.remote_addr = p->remote_signal_addr;
...
unsigned RdmaTransportAdapter::wait_async(int rank, uint64_t expected_tag,
  ...
  if (!target.has_value()) {
    ...
    p->pending_signal_wait_id.store(rid, std::memory_order_release);
  } else {
    slot->kind = RequestKind::DataWait;
    p->pending_data_chunks_total = 0;
    p->pending_data_chunks_done = 0;
    p->pending_data_wait_id.store(rid, std::memory_order_release);
  }
  ...
  while (!stop_.load(std::memory_order_acquire)) {
    std::vector<std::pair<int, RdmaPeer*>> active;
    {
      ...
      for (auto& [rank, p] : peers_) {
        if (p->cq) active.push_back({rank, p.get()});
      }
    }

    bool any = false;
    for (auto& [rank, p] : active) {
      if (poll_cq(*p, rank)) any = true;
    }
    if (!any) {
      std::this_thread::sleep_for(kPollSleep);
    }
...
bool RdmaTransportAdapter::poll_cq(RdmaPeer& p, int rank) {
  ibv_wc wc;
  bool any = false;
  while (ibv_poll_cq(p.cq, 1, &wc) > 0) {
    any = true;
    if (wc.status != IBV_WC_SUCCESS) {
      if (wc.opcode == IBV_WC_RECV ||
          wc.opcode == IBV_WC_RECV_RDMA_WITH_IMM) {
        int qp = find_qp_idx(p, wc.qp_num);
    ...
    switch (wc.opcode) {
      ...
      case IBV_WC_RECV_RDMA_WITH_IMM: {
        ...
        if (imm & kSignalImmBit) {
          uint64_t tag = imm_to_tag(imm);
          unsigned wait_id =
              p.pending_signal_wait_id.exchange(0, std::memory_order_acq_rel);
          if (wait_id) {
            RequestSlot* s = resolve_slot(wait_id);
            if (s && s->kind == RequestKind::SignalWait &&
                s->peer_rank == rank && s->expected_tag == tag) {
              s->completed.store(true, std::memory_order_release);
            }
          }
          cv_.notify_all();
        } else {
          if (p.pending_data_wait_id.load(std::memory_order_acquire) == 0) break;
          uint16_t total = imm_total(imm);
          if (p.pending_data_chunks_total == 0) {
            p.pending_data_chunks_total = total;
          }
          p.pending_data_chunks_done++;
          if (p.pending_data_chunks_done >= p.pending_data_chunks_total) {
            unsigned wait_id =
                p.pending_data_wait_id.exchange(0, std::memory_order_acq_rel);
            p.pending_data_chunks_total = 0;
            p.pending_data_chunks_done = 0;
            if (wait_id) {
              RequestSlot* s = resolve_slot(wait_id);
              if (s && s->kind == RequestKind::DataWait &&
                  s->peer_rank == rank) {
                s->completed.store(true, std::memory_order_release);
              }
            }
