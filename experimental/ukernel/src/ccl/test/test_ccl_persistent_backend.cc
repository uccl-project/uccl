...
int main() {
  ...
  using CCL::Testing::ck;
  using CCL::Testing::feq;
  using CCL::Testing::fill;
  ...
  CCL::PersistentKernelBackend gather_backend(
      kernel, gather_buffers, Device::DataType::Fp32, Device::ReduceType::Sum,
      Device::TransferPath::RegisterOp, config.numBlocks);
  ...
  CCL::PersistentKernelBackend reduce_backend(
      kernel, reduce_buffers, Device::DataType::Fp32, Device::ReduceType::Sum,
      Device::TransferPath::RegisterOp, config.numBlocks);
    ...
    if (!feq(h_gather_out[i], local_expected)) {
      if (gather_bad < 8) {
        std::cerr << "[ALLGATHER LOCAL MISMATCH] i=" << i
                  << " got=" << h_gather_out[i] << " exp=" << local_expected
                  << "\n";
      }
      ++gather_bad;
    }
