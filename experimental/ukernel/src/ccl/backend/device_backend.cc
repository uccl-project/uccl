...
char const* DeviceBackend::name() const {
  return "device";
}
...
void const* DeviceBackend::byte_offset(void const* base,
                                     size_t offset) const {
  return static_cast<void const*>(static_cast<char const*>(base) + offset);
}

void* DeviceBackend::resolve_dst(BufferRole role, size_t offset) const {
  switch (role) {
    ...
    case BufferRole::RemoteReduced:
      throw std::invalid_argument("invalid dst buffer role for device backend");
  }
  throw std::invalid_argument("unknown dst buffer role");
}

void const* DeviceBackend::resolve_src(BufferRole role,
                                     size_t offset) const {
  switch (role) {
    case BufferRole::LocalInput:
    ...
    case BufferRole::FinalOutput:
      return byte_offset(buffers_.final_output, offset);
    case BufferRole::None:
      throw std::invalid_argument("invalid src buffer role for device backend");
  }
