...
template <typename T>
__device__ __forceinline__ void reg_copy(T* dst, const T* src, size_t count,
                                         int tid, int nthread) {
...
template <typename T>
__device__ __forceinline__ void reg_reduce(T* dst, const T* src, size_t count,
                                           int tid, int nthread,
                                           ReduceType op) {
  size_t chunk = (count + nthread - 1) / nthread;
  size_t start = tid * chunk;
...
template <typename T>
__device__ __forceinline__ void reg_multi_reduce(T* dst, const T* const* srcs,
                                                 uint32_t n_src, size_t count,
                                                 int tid, int nthread,
                                                 ReduceType op) {
  size_t chunk = (count + nthread - 1) / nthread;
  size_t start = tid * chunk;
  ...
  for (size_t i = start; i < end; ++i) {
    T val = srcs[0][i];
    for (uint32_t s = 1; s < n_src; ++s)
      val = apply_reduce(val, srcs[s][i], op);
    dst[i] = val;
  }
}

template <typename T>
__device__ __forceinline__ void reg_fill(T* dst, T val, size_t count, int tid,
                                         int nthread) {
  size_t chunk = (count + nthread - 1) / nthread;
  size_t start = tid * chunk;
  size_t end = start + chunk < count ? start + chunk : count;
...
__device__ __forceinline__ void reg_copy_vectorized(T* dst, const T* src,
  ...
  auto* src_vec = reinterpret_cast<VecT const*>(src);

  for (size_t i = start; i < end; ++i) dst_vec[i] = src_vec[i];
  size_t remaining = count - n_vec * N;
  if (remaining > 0 && tid == 0) {
    for (size_t i = 0; i < remaining; ++i)
      dst[n_vec * N + i] = src[n_vec * N + i];
  }
}

template <int N>
__device__ __forceinline__ void reg_copy_float4(float4* dst, float4 const* src,
                                                size_t count, int tid,
                                                int nthread) {
  size_t chunk = (count + nthread - 1) / nthread;
  size_t start = tid * chunk;
...
template <typename T>
__device__ __forceinline__ void reg_to_shmem_async(void* smem, const T* reg_src,
                                                   size_t count) {
  ...
  for (size_t i = 0; i < aligned_count; i += kAlign) {
    asm volatile("cp.async.ca.shared.global [%0], [%1], %2;" ::"l"(dst + i),
                 "l"(src + i), "n"(kAlign));
  }
...
template <typename T>
__device__ __forceinline__ void reg_from_shmem_async(T* reg_dst,
                                                     void const* smem,
                                                     size_t count) {
  ...
  size_t aligned_count = (count * sizeof(T)) & ~(kAlign - 1);
  for (size_t i = 0; i < aligned_count; i += kAlign) {
    asm volatile("cp.async.ca.global.shared [%0], [%1], %2;" ::"l"(dst + i),
                 "l"(src + i), "n"(kAlign));
  }
  if (aligned_count < count * sizeof(T)) {
    size_t remaining = count * sizeof(T) - aligned_count;
...
template <typename T>
__device__ __forceinline__ void reg_shmem_copy_async(void* dst_smem,
                                                     void const* src_smem,
                                                     size_t bytes) {
  ...
  for (size_t i = 0; i < aligned_bytes; i += kAlign) {
    asm volatile("cp.async.ca.shared.shared [%0], [%1], %2;" ::"l"(dst + i),
                 "l"(src + i), "n"(kAlign));
  }
...
template <typename T>
__device__ __forceinline__ void reg_unroll_copy(T* dst, const T* src,
                                                size_t count, int tid,
                                                int nthread) {
  size_t chunk = (count + nthread - 1) / nthread;
  size_t start = tid * chunk;
