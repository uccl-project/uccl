...
template <typename T, ReduceType op>
__device__ __forceinline__ T warp_reduce(T val) { return val; }

template <ReduceType op>
__device__ __forceinline__ float warp_reduce(float val) { return val; }

template <typename T, ReduceType op>
__device__ __forceinline__ T block_reduce(T val, int tid) { return val; }

template <typename T, ReduceType op>
__device__ __forceinline__ T block_reduce_sum(T* smem, T val, int tid) { return val; }

__device__ __forceinline__ unsigned lanemask_lt() { return 0; }

template <typename T, ReduceType op>
__device__ __forceinline__ T warp_scan(T val) { return val; }

template <typename T, ReduceType op>
__device__ __forceinline__ T warp_scan_add(T val) { return val; }
