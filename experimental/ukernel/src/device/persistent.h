...
class PersistentKernel {
 public:
  ...
  void startReclaimThread() {
    // This reclaim path is intentionally kept in-tree for future d2c-triggered
    // argument recycling, but it is disabled in launch() for now because the
    // current kernel path no longer emits completion triggers.
  ...
  bool launch() {
    ...
    auto* d_coll = UKernel::Device::TaskManager::instance().d_coll();
    auto* d_gemm = UKernel::Device::TaskManager::instance().d_gemm();
    ...
    GPU_RT_CHECK(
        gpuLaunchKernel(basePersistentKernel, grid, block, args, cfg_.smemSize,
                        stream_));

    launched_ = true;

    // Keep the reclaim thread implementation around for future d2c-triggered
    // completion handling, but do not start it on the current path.
    // startReclaimThread();
    return true;
  };
        ...
        if ((int64_t)(doneBefore - tid) > 0) {
          ...
          switch (p.type) {
            ...
            case TaskType::CollReduce:
              UKernel::Device::TaskManager::instance().free_coll_args(
                  p.argsId);
              break;
            case TaskType::TkGemm:
              UKernel::Device::TaskManager::instance().free_gemm_args(
                  p.argsId);
              break;
  ...
  void stop() {
    ...
    *h_stopFlag_ = true;
    ...
    // The reclaim thread is disabled for the current path; keep the shutdown
    // logic commented as a reminder for when d2c completion triggers return.
  ...
  void freeArgs(mscclpp::ProxyTrigger const& trig) {
    ...
    switch (type) {
      case TaskType::CollCopy:
      case TaskType::CollReduce:
        UKernel::Device::TaskManager::instance().free_coll_args(argsId);
        break;

      case TaskType::TkGemm:
        UKernel::Device::TaskManager::instance().free_gemm_args(argsId);
        break;
