...
union alignas(16) ProxyTrigger {
  ...
  struct {
    uint64_t type : UKernel::Device::TaskTypeSize;
    uint64_t dataType : UKernel::Device::DataTypeSize;
    uint64_t blockId : UKernel::Device::BlockIdSize;
    uint64_t : (64 - UKernel::Device::TaskTypeSize -
                UKernel::Device::DataTypeSize - UKernel::Device::BlockIdSize);
    uint64_t argsId : UKernel::Device::TaskArgsIndexSize;
    uint64_t : (64 - UKernel::Device::TaskArgsIndexSize);
  } fields;
  ...
  MSCCLPP_DEVICE_INLINE ProxyTrigger(UKernel::Device::TaskType type,
                                     UKernel::Device::DataType dType,
    ...
    MSCCLPP_ASSERT_DEVICE(t < (1ULL << UKernel::Device::TaskTypeSize),
                          "type is too large");
    MSCCLPP_ASSERT_DEVICE(dt < (1ULL << UKernel::Device::DataTypeSize),
                          "dType is too large");
    MSCCLPP_ASSERT_DEVICE(bi < (1ULL << UKernel::Device::BlockIdSize),
                          "blockIndex is too large");
    MSCCLPP_ASSERT_DEVICE(ai < (1ULL << UKernel::Device::TaskArgsIndexSize),
