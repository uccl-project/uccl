...
void test_single_sm_copy() {
  ...
  for (size_t i = 0; i < bytes; i++) {
    if (h_dst[i] != h_src[i]) {
      match = false;
      std::cerr << "Mismatch at " << i << ": " << (int)h_dst[i] << " vs "
                << (int)h_src[i] << std::endl;
      break;
    }
  }
...
void test_data_types() {
  ...
  GPU_RT_CHECK(
      gpuMemcpy(d_src_f32, h_src_f32.data(), bytes_f32, gpuMemcpyHostToDevice));
  GPU_RT_CHECK(
      gpuMemcpy(d_src_f16, h_src_f16.data(), bytes_f16, gpuMemcpyHostToDevice));

  auto testDataType = [&](void* d_src, void* d_dst, size_t bytes,
    ...
    Task task =
        TaskManager::instance().create_task(args, TaskType::CollCopy, dtype, 0);
    uint64_t taskId = wp.enqueue(task, 0);
    ...
    if (dtype == DataType::Fp32) {
      for (int i = 0; i < 256; i++) {
        ...
        if (abs(val - expected) > 0.01f) {
          std::cerr << name << " mismatch at " << i << ": got " << val
      ...
      for (int i = 0; i < 256; i++) {
        ...
        if (abs(fval - expected) > 0.01f) {
          std::cerr << name << " mismatch at " << i << ": got " << fval
                    << ", expected " << expected << std::endl;
          match = false;
          break;
        }
...
void test_repeated_launch() {
  ...
  for (int iter = 0; iter < 5; iter++) {
    ...
    TEST_ASSERT(match,
                std::string("Iteration ") + std::to_string(iter) + " matches");
  }
...
void test_multiple_fifos() {
  ...
  for (int fifo = 0; fifo < 3; fifo++) {
    ...
    GPU_RT_CHECK(gpuMemcpy(d_src[fifo], h_src[fifo].data(), bytes,
  ...
  bool all_match = true;
  for (int fifo = 0; fifo < 3; fifo++) {
    GPU_RT_CHECK(gpuMemcpy(h_dst[fifo].data(), d_dst[fifo], bytes,
...
void test_boundary() {
  ...
  auto testSize = [&](size_t bytes, char const* name) {
    void *d_src, *d_dst;
    GPU_RT_CHECK(gpuMalloc(&d_src, bytes));
    GPU_RT_CHECK(gpuMalloc(&d_dst, bytes));
