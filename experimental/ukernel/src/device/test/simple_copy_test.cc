...
int main(int argc, char** argv) {
    ...
    GPU_RT_CHECK(gpuMemcpy(d_src_copy, h_src_copy.data(), bytes, gpuMemcpyHostToDevice));
    GPU_RT_CHECK(gpuMemset(d_dst_copy, 0, bytes));
    GPU_RT_CHECK(gpuMemcpy(d_src_reduce, h_src_reduce.data(), bytes, gpuMemcpyHostToDevice));
    GPU_RT_CHECK(gpuMemcpy(d_dst_reduce, h_dst_reduce_init.data(), bytes, gpuMemcpyHostToDevice));
    ...
    Task copy_task = TaskManager::instance().create_task(args_copy, TaskType::CollCopy, DataType::Int8, 0);
    Task reduce_task = TaskManager::instance().create_task(args_reduce, TaskType::CollReduce, DataType::Fp32, 0);
    
    uint64_t copy_taskId = wp.enqueue(copy_task, 0);
    uint64_t reduce_taskId = wp.enqueue(reduce_task, 1);
    std::cout << "Enqueued copy task " << copy_taskId << " on fifo 0" << std::endl;
    std::cout << "Enqueued reduce task " << reduce_taskId << " on fifo 1" << std::endl;
    ...
    GPU_RT_CHECK(gpuMemcpy(h_dst_copy.data(), d_dst_copy, bytes, gpuMemcpyDeviceToHost));
    GPU_RT_CHECK(gpuMemcpy(h_dst_reduce.data(), d_dst_reduce, bytes, gpuMemcpyDeviceToHost));
    
    bool copy_match = true;
    for (size_t i = 0; i < bytes; i++) {
        if (h_dst_copy[i] != h_src_copy[i]) {
            std::cerr << "Copy mismatch at " << i << ": got " << (int)h_dst_copy[i] << ", expected " << (int)h_src_copy[i] << std::endl;
            copy_match = false;
            break;
        }
    }
    ...
    for (size_t i = 0; i < count; i++) {
        if (abs(dst_reduce_float[i] - h_src_reduce[i]) > 0.01f) {
            std::cerr << "Reduce mismatch at " << i << ": got " << dst_reduce_float[i] << ", expected " << h_src_reduce[i] << std::endl;
            reduce_match = false;
            break;
        }
    }
