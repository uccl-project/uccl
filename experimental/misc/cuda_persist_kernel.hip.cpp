 ...
 public:
  iovMultiFifo(int num_fifo) : num_fifo_(num_fifo) {
    for (int i = 0; i < num_fifo; i++) {
      hipHostAlloc(&fifo_vec_[i], sizeof(struct IovFifo), hipHostMallocMapped);
      cudaCheckErrors("hipMallocManaged failed");
    }
    init();
  }
...
int main() {
  ...
  for (int i = 0; i < kTestIters; i += kTestSteps) {
    ...
    for (int k = 0; k < kTestSteps; k++) {
      ...
      //                     gpu_iov->lens[j],
      //                     hipMemcpyDeviceToDevice, stream3);
      //     cudaCheckErrors("hipMemcpyAsync failed");
      // }
