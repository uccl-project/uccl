    ...
    for step in range(num_iters + 1):
        ...
        if step > 0:
            ...
            send_transfer_ids[slot_idx] = _post_send(
                ep,
                conn_ids[slot_idx % len(conn_ids)],
...
def _run_server(args, ep, remote_metadata):
    conn_ids = _accept_many(ep, args.num_connections)
...
def _run_client(args, ep, remote_metadata):
    conn_ids = _connect_many(ep, remote_metadata, args.num_connections)
...
def main():
    ...
    p.add_argument("--num-connections", type=int, default=2)
    ...
    if args.num_connections < 1:
        raise ValueError("--num-connections must be >= 1")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for benchmark_uccl_staged_multiconn.py")
    ...
    print("UCCL Staged send/recv Benchmark (multi-connection)")
    print("=" * 60)
    ...
    print(f"Connections: {args.num_connections}")
