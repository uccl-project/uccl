...
/// A RAII guard that will cudaThreadExchangeStreamCaptureMode to
/// cudaStreamCaptureModeRelaxed on construction and restore the previous mode
/// on destruction. This is helpful when we want to avoid CUDA/HIP graph
/// capture.
struct AvoidCudaGraphCaptureGuard {
  AvoidCudaGraphCaptureGuard();
  ~AvoidCudaGraphCaptureGuard();
  AvoidCudaGraphCaptureGuard(AvoidCudaGraphCaptureGuard const&) = delete;
  AvoidCudaGraphCaptureGuard& operator=(AvoidCudaGraphCaptureGuard const&) =
      delete;
...
/// A RAII guard that will set the current device on construction and restore
...
/// A RAII wrapper around cudaStream_t that will call cudaStreamDestroy on
/// destruction.
struct CudaStreamWithFlags {
  /// Constructor without flags. This will not create any stream. set() can be
  /// called later to create a stream with specified flags.
  CudaStreamWithFlags();

  /// Constructor with flags. This will create a stream with the specified flags
  ...
  /// Set the stream with the specified flags. The current device at the time of
  /// the construction will be used. If the stream was already created, it will
  /// raise an error with ErrorCode::InvalidUsage.
  /// @param flags The flags to create the stream with.
...
/// When this object is destroyed, it will return the underlying
/// CudaStreamWithFlags to the pool.
class GpuStream {
 protected:
  /// Constructor. Only called by a GpuStreamPool.
  /// @param pool A shared pointer to the GpuStreamPool that manages this
  /// stream.
  /// @param stream A shared pointer to the CudaStreamWithFlags that represents
  /// the underlying stream.
  GpuStream(std::shared_ptr<GpuStreamPool> pool,
            std::shared_ptr<CudaStreamWithFlags> stream);

 public:
  /// Destructor. This will return the underlying CudaStreamWithFlags to the
  /// pool, not destroy it.
  ~GpuStream();
...
class GpuStreamPool {
 public:
  ...
  /// Get a non-blocking GPU stream from the pool. If no streams are available,
 ...
 protected:
  ...
  std::unordered_map<int, std::vector<std::shared_ptr<CudaStreamWithFlags>>>
      streams_;
};
...
void gpuMemcpy(void* dst, void const* src, size_t bytes,
               cudaMemcpyKind kind = cudaMemcpyDefault);
...
/// @return An object of type @p Memory that will free the allocated memory when
/// destroyed.
///
...
auto gpuCallocShared(size_t nelems = 1) {
  return detail::safeAlloc<T, detail::GpuDeleter<T>, std::shared_ptr<T>>(
...
auto gpuCallocHostShared(size_t nelems = 1,
  ...
  return detail::safeAlloc<T, detail::GpuHostDeleter<T>, std::shared_ptr<T>>(
...
auto gpuCallocUncachedShared(size_t nelems = 1) {
  return detail::safeAlloc<T, detail::GpuDeleter<T>, std::shared_ptr<T>>(
      detail::gpuCallocUncached, nelems);
}
...
auto gpuCallocPhysicalShared(size_t nelems = 1, size_t gran = 0,
                             size_t align = 0) {
  return detail::safeAlloc<T, detail::GpuPhysicalDeleter<T>,
                           std::shared_ptr<T>>(detail::gpuCallocPhysical,
                                               nelems, gran, align);
}
...
auto gpuCallocPhysicalUnique(size_t nelems = 1, size_t gran = 0,
                             size_t align = 0) {
  return detail::safeAlloc<T, detail::GpuPhysicalDeleter<T>,
                           UniqueGpuPhysicalPtr<T>>(detail::gpuCallocPhysical,
                                                    nelems, gran, align);
}
...
/// Allocates a GPU memory space specialized for communication. The memory is
/// zeroed out. Get the device pointer by `GpuBuffer::data()`.
///
/// Use this function for communication buffers, i.e., only when other devices
/// (CPU, GPU, NIC, etc.) may access this memory space at the same time with the
/// local device (GPU). Running heavy computation over this memory space may
/// perform bad and is not recommended in general.
///
/// The allocated memory space is managed by the `memory_` object, not by the
/// class instance. Which means, the class destructor will NOT free the
/// allocated memory if `memory_` is shared with and alive in other contexts.
///
...
class GpuBuffer {
 public:
  ...
  /// @param nelems Number of elements to allocate. If it is zero, `data()` will
...
#if (CUDA_NVLS_API_AVAILABLE)
    if (isNvlsSupported()) {
      size_t gran = detail::getMulticastGranularity(
  ...
  /// Returns the number of bytes that is actually allocated. This may be larger
  /// than `nelems() * sizeof(T)`.
  ...
  /// `memory().get()`. If `nelems()` is zero, this function will return a null
  /// pointer.
