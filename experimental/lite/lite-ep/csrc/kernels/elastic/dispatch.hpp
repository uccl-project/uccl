...
class DispatchPrologueRuntime final
    : public jit::LaunchRuntime<DispatchPrologueRuntime> {
 public:
  struct Args {
    // Templated arguments
    int num_warps;
...
static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&dispatch_deterministic_prologue_impl<{}, {}, {}, {}, {}, {}>);
                       ...
                       args.launch_args.grid_dim.first, args.num_warps,
  ...
  static void launch_impl(jit::KernelHandle const& kernel,
    ...
    EP_CUDA_UNIFIED_CHECK(jit::launch_kernel(
        kernel, config, args.topk_idx, args.rank_count_buffer,
        args.dst_buffer_slot_idx, args.num_tokens, args.rank_idx));
  }
};

static void launch_dispatch_deterministic_prologue(
    topk_idx_t* topk_idx, int* rank_count_buffer, int* dst_buffer_slot_idx,
    int const& num_tokens, int const& num_max_tokens_per_rank,
    int const& num_experts, int const& num_topk, int const& rank_idx,
    int const& num_ranks, int const& num_sms, int const& num_smem_bytes,
    at::cuda::CUDAStream const& stream) {
  constexpr auto num_warps = 8;
  constexpr auto num_threads = num_warps * 32;
  EP_HOST_ASSERT((2 * num_warps + 1) * num_ranks * sizeof(int) <=
                     num_smem_bytes and
  ...
  const DispatchPrologueRuntime::Args args = {
      ...
      .rank_idx = rank_idx,
      .launch_args =
          jit::LaunchArgs(num_sms, num_threads, num_smem_bytes, 1, true)};
  auto const code = DispatchPrologueRuntime::generate(args);
  auto const runtime =
      jit::compiler->build("dispatch_deterministic_prologue", code);
  ...
  static std::string generate_impl(Args const& args) {
    ...
    if (args.num_scaleout_ranks == 1) {
      ...
      func_name = fmt::format(
          ...
          args.launch_args.grid_dim.first, args.num_notify_warps,
          args.num_dispatch_warps, args.num_scaleup_ranks,
          args.num_hidden_bytes, args.num_sf_packs,
          args.num_max_tokens_per_rank, args.num_experts, args.num_topk,
          args.expert_alignment, args.num_qps, args.num_timeout_cycles);
    } else {
      header_name = "hybrid_dispatch";
      func_name = fmt::format(
          "hybrid_dispatch_impl<{}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, "
          "{}, {}, {}, {}, {}>",
          args.do_cpu_sync, args.reuse_slot_indices,
          args.launch_args.grid_dim.first, args.num_notify_warps,
          args.num_scaleout_warps, args.num_forward_warps,
...
static void __instantiate_kernel() {{
  ...
  static void launch_impl(jit::KernelHandle const& kernel,
    ...
    if (args.num_scaleout_ranks == 1) {
      EP_CUDA_UNIFIED_CHECK(jit::launch_kernel(
          kernel, config, args.x, args.sf, args.topk_idx, args.topk_weights,
          args.copied_topk_idx, args.cumulative_local_expert_recv_stats,
          args.psum_num_recv_tokens_per_scaleup_rank,
          args.psum_num_recv_tokens_per_expert, args.dst_buffer_slot_idx,
          args.num_tokens, args.sf_token_stride, args.sf_hidden_stride,
          args.nccl_dev_comm, args.nccl_window, args.buffer, args.workspace,
          args.mapped_host_workspace, args.scaleup_rank_idx,
      ...
      EP_CUDA_UNIFIED_CHECK(jit::launch_kernel(
          ...
          args.psum_num_recv_tokens_per_scaleup_rank,
          args.psum_num_recv_tokens_per_expert, args.dst_buffer_slot_idx,
          args.token_metadata_at_forward, args.num_tokens, args.sf_token_stride,
          args.sf_hidden_stride, args.nccl_dev_comm, args.nccl_window,
          args.buffer, args.workspace, args.mapped_host_workspace,
...
static int get_num_notify_smem_bytes(int const& num_ranks,
                                     int const& num_experts) {
  return math::align(num_ranks + num_experts, kNumNotifyWarps * 32) *
         sizeof(int);
}

static layout::TokenLayout get_dispatch_token_layout(int const& hidden,
                                                     int const& elem_size,
                                                     int const& num_sf_packs,
                                                     int const& num_topk) {
  return layout::TokenLayout(hidden * elem_size,
                             num_sf_packs * sizeof(sf_pack_t), num_topk, true);
}

static void launch_dispatch(
    ...
    int const& num_max_tokens_per_rank, int const& hidden, int const& elem_size,
    int const& num_sf_packs, int const& sf_token_stride,
    int const& sf_hidden_stride, int const& num_experts, int const& num_topk,
    int const& expert_alignment, ncclDevComm_t const& nccl_dev_comm,
    ncclWindow_t const& nccl_window, void* buffer, void* workspace,
    void* mapped_host_workspace, int const& scaleout_rank_idx,
    int const& scaleup_rank_idx, int const& num_scaleout_ranks,
    int const& num_scaleup_ranks, bool const& is_scaleup_nvlink,
    int const& num_sms, int const& num_channels_per_sm,
    int const& num_smem_bytes, int const& num_qps,
    int64_t const& num_timeout_cycles, bool const& cached_mode,
    bool const& deterministic, bool const& do_cpu_sync,
    uint64_t const* uccl_d2h_channel_addrs,
  ...
  // Utils
  auto const num_ranks = num_scaleout_ranks * num_scaleup_ranks;

  // Notify warps
  // TODO: why don't we use 4 notify warps?
  int const num_notify_warps = cached_mode ? 0 : kNumNotifyWarps;
  bool const reuse_slot_indices = cached_mode or deterministic;
  int const num_notify_smem_bytes =
      cached_mode ? 0 : get_num_notify_smem_bytes(num_ranks, num_experts);
  ...
  if (num_scaleout_ranks == 1) {
    // Too many warps may cause performance degrade, so we limit the total warps
    // into 512
    auto const token_layout =
        get_dispatch_token_layout(hidden, elem_size, num_sf_packs, num_topk);
    num_dispatch_warps =
        std::min<int>(std::min<int>((num_smem_bytes - num_notify_smem_bytes) /
  ...
  const DispatchRuntime::Args args = {
      ...
      .num_scaleout_warps = num_scaleout_warps,
      ...
      .launch_args = jit::LaunchArgs(num_sms, num_threads, num_smem_bytes,
                                     2 - (num_sms % 2), true)};
  auto const code = DispatchRuntime::generate(args);
  auto const runtime = jit::compiler->build("dispatch", code);
  DispatchRuntime::launch(runtime, args, stream);
}

class DispatchCopyEpilogueRuntime final
    : public jit::LaunchRuntime<DispatchCopyEpilogueRuntime> {
 public:
  struct Args {
    // Templated arguments
...
static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&dispatch_copy_epilogue_impl<{}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}>);
        ...
        args.do_expand, args.cached_mode, args.launch_args.grid_dim.first,
        args.num_channels, args.num_warps, args.num_scaleout_ranks,
        args.num_scaleup_ranks, args.num_hidden_bytes, args.num_sf_packs,
        args.num_max_tokens_per_rank, args.num_experts, args.num_topk);
  }
  ...
  static void launch_impl(jit::KernelHandle const& kernel,
                          jit::LaunchConfigHandle const& config, Args args) {
    EP_CUDA_UNIFIED_CHECK(jit::launch_kernel(
        kernel, config, args.buffer, args.workspace,
        args.psum_num_recv_tokens_per_scaleup_rank,
        args.psum_num_recv_tokens_per_expert, args.recv_x, args.recv_sf,
        args.recv_topk_idx, args.recv_topk_weights, args.recv_src_metadata,
        args.channel_linked_list, args.num_recv_tokens,
        args.recv_sf_token_stride, args.recv_sf_hidden_stride,
        args.scaleout_rank_idx, args.scaleup_rank_idx));
  }
};

static void launch_dispatch_copy_epilogue(
    ...
    int* channel_linked_list, int const& num_recv_tokens,
    int const& num_max_tokens_per_rank, int const& num_hidden_bytes,
    int const& num_sf_packs, int const& recv_sf_token_stride,
    int const& recv_sf_hidden_stride, int const& num_experts,
    int const& num_topk, int const& scaleout_rank_idx,
    int const& scaleup_rank_idx, int const& num_scaleout_ranks,
    int const& num_scaleup_ranks, int const& num_sms, int const& num_smem_bytes,
    int const& num_channels, bool const& do_expand, bool const& cached_mode,
    at::cuda::CUDAStream const& stream) {
  // Maximize shared memory utilization
  ...
  const DispatchCopyEpilogueRuntime::Args args = {
      ...
      .recv_sf_token_stride = recv_sf_token_stride,
      .recv_sf_hidden_stride = recv_sf_hidden_stride,
      .scaleout_rank_idx = scaleout_rank_idx,
      .scaleup_rank_idx = scaleup_rank_idx,
      .launch_args = jit::LaunchArgs(num_sms, num_threads, num_smem_bytes, 1,
                                     false, true)};
  auto const code = DispatchCopyEpilogueRuntime::generate(args);
  auto const runtime = jit::compiler->build("dispatch_copy_epilogue", code);
