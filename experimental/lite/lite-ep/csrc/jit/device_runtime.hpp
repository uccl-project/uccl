 ...
 public:
  int64_t get_clock_rate() {
    if (cached_clock_rate == 0) {
      ...
      CUDA_RUNTIME_CHECK(cudaGetDevice(&device_idx));
      CUDA_RUNTIME_CHECK(
          cudaDeviceGetAttribute(&rate, cudaDevAttrClockRate, device_idx));
  ...
  std::string get_arch(bool const& number_only = false,
                       bool const& support_arch_family = false) {
    auto const [major, minor] = get_arch_pair();
    if (major == 10 and minor != 1) {
      if (number_only) return "100";
      return support_arch_family ? "100f" : "100a";
    }
    return std::to_string(major * 10 + minor) +
           (number_only or major < 9 ? "" : "a");
  }

  int get_arch_major() { return get_arch_pair().first; }
};
