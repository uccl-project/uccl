...
class IncludeParser {
  ...
  static std::vector<std::string> get_includes(
      std::string const& code, std::filesystem::path const& file_path = "") {
    ...
    for (; iter != end; ++iter) {
      auto const include_str = iter->str();
      int const len = include_str.length();
      if (include_str.substr(0, 10) == "#include <" and
          include_str[len - 1] == '>' and include_str[10] != ' ' and
          include_str[len - 2] != ' ') {
        ...
        std::string error_info =
            fmt::format("Non-standard include: {}", include_str);
        if (file_path != "")
 ...
 public:
  ...
  std::string get_hash_value(std::string const& code,
                             bool const& exclude_code = true) {
    std::stringstream ss;
    for (auto const& i : get_includes(code))
      ss << get_hash_value_by_path(library_include_path / i) << "$";
    if (not exclude_code) ss << "#" << get_hex_digest(code);
    return get_hex_digest(ss.str());
  }

  std::string get_hash_value_by_path(std::filesystem::path const& path) {
    // Check whether hit in cache
    // ReSharper disable once CppUseAssociativeContains
    if (cache.count(path) > 0) {
      auto const opt = cache[path];
      if (not opt.has_value())
        EP_HOST_UNREACHABLE(
            fmt::format("Circular include may occur: {}", path.string()));
      return opt.value();
    }
