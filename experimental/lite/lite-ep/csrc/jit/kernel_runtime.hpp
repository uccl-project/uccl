...
class KernelRuntime final {
 public:
  ...
  explicit KernelRuntime(std::filesystem::path const& dir_path) {
    EP_HOST_ASSERT(not cuda_home.empty());

    // NOLINT(*-pro-type-member-init)
    auto const cuobjdump_path = cuda_home / "bin" / "cuobjdump";
    auto const cubin_path = dir_path / "kernel.cubin";
    ...
    auto const [exit_code, symbols] = call_external_command(fmt::format(
        "{} -symbols {}", cuobjdump_path.c_str(), cubin_path.c_str()));
    EP_HOST_ASSERT(exit_code == 0);
    std::istringstream iss(symbols);
    std::vector<std::string> symbol_names;
    for (std::string line; std::getline(iss, line);) {
      if (line.find("STT_FUNC") == 0 and
          line.find("STO_ENTRY") != std::string::npos and
          std::none_of(illegal_names.begin(), illegal_names.end(),
                       [&](auto const name) {
                         return line.find(name) != std::string::npos;
                       })) {
        auto const last_space = line.rfind(' ');
    ...
    if (symbol_names.size() != 1) {
      printf(
          "Corrupted JIT cache directory (expected 1 kernel symbol, found "
          "%zu): %s, "
          "please run `rm -rf %s` and restart your task.\n",
          symbol_names.size(), dir_path.c_str(), dir_path.c_str());
      printf("Symbol names: ");
      for (auto const& symbol : symbol_names) printf("%s, ", symbol.c_str());
      printf("\n");
  ...
  static void prepare_init(std::string const& cuda_home_path_by_python) {
    cuda_home = cuda_home_path_by_python;
  }

  static bool check_validity(std::filesystem::path const& dir_path) {
    if (not std::filesystem::exists(dir_path)) return false;
    // NOTES: if the directory exists, kernel.cu and kernel.cubin must both
    // exist, because the directory is created atomically via rename
    if (not std::filesystem::exists(dir_path / "kernel.cu") or
        not std::filesystem::exists(dir_path / "kernel.cubin")) {
      printf(
          "Corrupted JIT cache directory (missing kernel.cu or kernel.cubin): "
          "%s, "
          "please run `rm -rf %s` and restart your task.\n",
  ...
  ~KernelRuntime() noexcept(false) { unload_library(library); }
};
