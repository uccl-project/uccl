...
class Compiler {
 public:
  ...
  static void prepare_init(std::string const& library_root_path,
                           std::string const& cuda_home_path_by_python,
                           std::string const& nccl_root_path_by_python) {
    // NOTES: if you are adding some third-party includes for kernels, please
    // add its hash value
  ...
  Compiler() {
    ...
    // Cache settings
    cache_dir_path =
        std::filesystem::path(get_env<std::string>("HOME")) / ".deep_ep";
    if (auto const env_cache_dir_path =
            get_env<std::string>("EP_JIT_CACHE_DIR");
        not env_cache_dir_path.empty())
      cache_dir_path = env_cache_dir_path;

    // The compiler flags applied to all derived compilers
    signature = "unknown-compiler";
    flags = fmt::format(
        "-std=c++{} --diag-suppress=39,161,174,177,186,940,3012 "
    ...
    if (int num_topk_idx_bits = get_env("EP_NUM_TOPK_IDX_BITS", 0);
        num_topk_idx_bits != 0)
      flags += fmt::format(" -DEP_NUM_TOPK_IDX_BITS={}", num_topk_idx_bits);
    flags +=
        " -DDEEP_EP_USE_NCCL_SIGNAL_BARRIER -DDEEP_EP_USE_VA_SIGNAL_BARRIER "
        "-DDEEP_EP_EXPLICIT_LSA_BASE -DDEEP_EP_PUT_VALUE_BARRIER "
        "-DDEEP_EP_SYSTEM_FENCE_BEFORE_GIN_PUT";
    if (get_env("DISABLE_AGGRESSIVE_PTX_INSTRS", 1))
      flags += " -DDISABLE_AGGRESSIVE_PTX_INSTRS";
    if (get_env("EP_FORCE_NO_NVLINK", 0)) flags += " -DEP_FORCE_NO_NVLINK";
    if (get_env("EP_UCCL_PROXY_ACTIVE", get_env("UCCL_FORCE_NO_GDR", 0))) {
      flags += fmt::format(
          " -DEP_USE_UCCL_PROXY -DEP_UCCL_PROXY_TRANSPORT_VERSION=52 -I "
          "{}/../csrc/uccl/include -I {}/../../../../include",
          library_root_path.c_str(), library_root_path.c_str());
    }
    if (get_env("EP_UCCL_DEVICE_TRACE", 0)) flags += " -DEP_UCCL_DEVICE_TRACE";
  }
  ...
  static void fsync_path(std::filesystem::path const& path) {
  ...
  // Recursively fsync a directory: files and subdirectories first (bottom-up),
  // then the directory itself NOTES: ensures data and directory entries are
  // visible on other nodes in distributed filesystems
  static void fsync_dir(
      std::filesystem::path const& dir_path) {  // NOLINT(*-no-recursion)
  ...
  static void put(std::filesystem::path const& path, std::string const& data) {
    std::ofstream out(path, std::ios::binary);
    EP_HOST_ASSERT(out.write(data.data(), data.size()));
    out.close();

    // NOTES: fsync to ensure the data is visible to other processes (e.g.,
    // NVCC) on distributed filesystems, where `close()` alone does not
    // guarantee persistence
    fsync_path(path);
  }

  static std::shared_ptr<KernelRuntime> wait_for_runtime_cache(
    ...
    for (int i = 0; i < kMaxAttempts; ++i) {
      if (std::filesystem::exists(dir_path / "kernel.cu") and
        ...
        if (auto const runtime = kernel_runtime_cache->get(dir_path);
            runtime != nullptr)
          return runtime;
      }
  ...
  std::shared_ptr<KernelRuntime> build(std::string const& name,
                                       std::string const& code) const {
    auto const kernel_signature =
    ...
    // Compile into a temporary directory, then atomically rename the whole
    // directory NOTES: renaming a directory is atomic on both local and
    // distributed filesystems, avoiding the stale inode issue that occurs when
    // renaming individual files
    auto const tmp_dir_path = make_tmp_dir() / get_uuid();
    make_dirs(tmp_dir_path);

    // Compile into the temporary directory
    auto const tmp_cubin_path = tmp_dir_path / "kernel.cubin";
    if (get_env<int>("EP_JIT_DUMP_ASM") or get_env<int>("EP_JIT_DUMP_PTX")) {
      auto const tmp_ptx_path = tmp_dir_path / "kernel.ptx";
    ...
    if (get_env<int>("EP_JIT_DUMP_ASM") or get_env<int>("EP_JIT_DUMP_SASS")) {
      auto const tmp_sass_path = tmp_dir_path / "kernel.sass";
    ...
    // NOTES: if another rank already created dir_path, rename will fail —
    // that's fine
    ...
    auto const runtime = error_code ? wait_for_runtime_cache(dir_path)
                                    : kernel_runtime_cache->get(dir_path);
    EP_HOST_ASSERT(runtime != nullptr);
    return runtime;
  }

  static void disassemble(std::filesystem::path const& cubin_path,
                          std::filesystem::path const& sass_path) {
    // Disassemble the CUBIN file to SASS
    auto const command =
        fmt::format("{} --dump-sass {} > {}", cuobjdump_path.c_str(),
                    cubin_path.c_str(), sass_path.c_str());
    if (get_env("EP_JIT_DEBUG", 0) or
        get_env("EP_JIT_PRINT_COMPILER_COMMAND", 0))
      printf("Running cuobjdump command: %s\n", command.c_str());
    auto const [return_code, output] = call_external_command(command);
    if (return_code != 0) {
  ...
  virtual void compile(std::string const& code,
                       std::filesystem::path const& dir_path,
                       std::filesystem::path const& cubin_path,
                       std::optional<std::filesystem::path> const& ptx_path =
                           std::nullopt) const = 0;
};
...
class NVCCCompiler final : public Compiler {
  ...
  std::pair<int, int> get_nvcc_version() const {
    ...
    auto const command = std::string(nvcc_path) + " --version";
    auto const [return_code, output] = call_external_command(command);
    ...
    std::smatch match;
    EP_HOST_ASSERT(
        std::regex_search(output, match, std::regex(R"(release (\d+\.\d+))")));
    std::sscanf(match[1].str().c_str(), "%d.%d", &major, &minor);
    EP_HOST_ASSERT((major > 12 or (major == 12 and minor >= 3)) and
 ...
 public:
  NVCCCompiler() {
    ...
    if (auto const env_nvcc_path = get_env<std::string>("EP_JIT_NVCC_COMPILER");
        not env_nvcc_path.empty())
      nvcc_path = env_nvcc_path;
    auto const [nvcc_major, nvcc_minor] = get_nvcc_version();
    ...
    auto const [arch_major, arch_minor] = device_runtime->get_arch_pair();
    auto const arch =
        device_runtime->get_arch(false, nvcc_major > 12 or nvcc_minor >= 9);
    flags = fmt::format(
        "{} -I{} --gpu-architecture=sm_{} "
        "--compiler-options=-fPIC,-O3,-fconcepts,-Wno-deprecated-declarations,-"
        "Wno-abi "
  ...
  void compile(
      std::string const& code, std::filesystem::path const& dir_path,
      std::filesystem::path const& cubin_path,
      std::optional<std::filesystem::path> const& ptx_path) const override {
    // Write the code into the cache directory
    auto const code_path = dir_path / "kernel.cu";
    put(code_path, code);

    // Compile to CUBIN
    // Avoid cwd files shadowing C++ standard library headers
    auto const compile_dir = make_tmp_dir();
    auto const command = fmt::format(
        "cd {} && {} {} -cubin -o {} {}", compile_dir.c_str(),
    ...
    if (ptx_path.has_value()) {
      auto const ptx_command = fmt::format(
          "cd {} && {} {} -ptx -o {} {}", compile_dir.c_str(),
          nvcc_path.c_str(), code_path.c_str(), ptx_path->c_str(), flags);
      if (get_env("EP_JIT_DEBUG", 0) or
          get_env("EP_JIT_PRINT_COMPILER_COMMAND", 0))
        printf("Running NVCC PTX command: %s\n", ptx_command.c_str());
      auto const [ptx_return_code, ptx_output] =
          call_external_command(ptx_command);
    ...
    if (get_env("EP_JIT_PTXAS_CHECK", 0))
      EP_HOST_ASSERT(
          not std::regex_search(output, std::regex(R"(Local memory used)")));
