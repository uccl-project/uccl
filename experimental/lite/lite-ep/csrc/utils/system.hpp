...
static dtype_t get_env(std::string const& name,
                       dtype_t const& default_value = dtype_t()) {
  auto const c_str = std::getenv(name.c_str());
...
static std::filesystem::path make_dirs(std::filesystem::path const& path) {
  // OK if existed
  std::error_code capture;
  bool const created = std::filesystem::create_directories(path, capture);
  if (not(created or capture.value() == 0)) {
    EP_HOST_UNREACHABLE(
        fmt::format("Failed to make directory: {}, created: {}, value: {}",
...
static std::string get_uuid() {
  ...
  std::stringstream ss;
  ss << getpid() << "-" << std::hex << std::setfill('0') << std::setw(8)
     << dist(gen) << "-" << std::setw(8) << dist(gen) << "-" << std::setw(8)
     << dist(gen);
  return ss.str();
}

static void safe_remove_all(std::filesystem::path const& path) {
  std::error_code ec;
