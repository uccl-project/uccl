...
static void cu_mem_set_access_all(void* ptr, size_t size) {
  ...
  CUDA_DRIVER_CHECK(lazy_cuMemSetAccess(reinterpret_cast<CUdeviceptr>(ptr),
                                        size, access_desc, device_count));
}
...
static void cu_mem_free(void* ptr) {
  ...
  size_t size = 0;
  CUDA_DRIVER_CHECK(lazy_cuMemGetAddressRange_v2(
      nullptr, &size, reinterpret_cast<CUdeviceptr>(ptr)));

  CUDA_DRIVER_CHECK(lazy_cuMemUnmap(reinterpret_cast<CUdeviceptr>(ptr), size));
  CUDA_DRIVER_CHECK(
      lazy_cuMemAddressFree(reinterpret_cast<CUdeviceptr>(ptr), size));
  CUDA_DRIVER_CHECK(lazy_cuMemRelease(handle));
}

class SharedMemoryAllocator {
 public:
  explicit SharedMemoryAllocator(bool const& use_fabric)
  ...
  void malloc(void** ptr, size_t size) const {
    if (use_fabric) {
      ...
      CUDA_DRIVER_CHECK(lazy_cuMemGetAllocationGranularity(
          &alignment, &prop, CU_MEM_ALLOC_GRANULARITY_MINIMUM));
      size = ((size + alignment - 1) / alignment) * alignment;

      CUmemGenericAllocationHandle handle;
      CUDA_DRIVER_CHECK(lazy_cuMemCreate(&handle, size, &prop, 0));
      CUDA_DRIVER_CHECK(lazy_cuMemAddressReserve(
          reinterpret_cast<CUdeviceptr*>(ptr), size, alignment, 0, 0));
      CUDA_DRIVER_CHECK(lazy_cuMemMap(reinterpret_cast<CUdeviceptr>(*ptr), size,
                                      0, handle, 0));
  ...
  void get_mem_handle(MemHandle* mem_handle, void* ptr) const {
    ...
    if (use_fabric) {
      ...
      CUDA_DRIVER_CHECK(lazy_cuMemExportToShareableHandle(
          &mem_handle->inner.cu_mem_fabric_handle, handle,
          CU_MEM_HANDLE_TYPE_FABRIC, 0));
    } else {
      CUDA_RUNTIME_CHECK(
          cudaIpcGetMemHandle(&mem_handle->inner.cuda_ipc_mem_handle, ptr));
    }
  }
  ...
  void open_mem_handle(void** ptr, MemHandle* mem_handle) const {
    if (use_fabric) {
      ...
      CUDA_DRIVER_CHECK(lazy_cuMemImportFromShareableHandle(
          &handle, &mem_handle->inner.cu_mem_fabric_handle,
          CU_MEM_HANDLE_TYPE_FABRIC));

      CUDA_DRIVER_CHECK(lazy_cuMemAddressReserve(
          reinterpret_cast<CUdeviceptr*>(ptr), size, 0, 0, 0));
      CUDA_DRIVER_CHECK(lazy_cuMemMap(reinterpret_cast<CUdeviceptr>(*ptr), size,
                                      0, handle, 0));
      cu_mem_set_access_all(*ptr, size);
    } else {
      CUDA_RUNTIME_CHECK(
          cudaIpcOpenMemHandle(ptr, mem_handle->inner.cuda_ipc_mem_handle,
