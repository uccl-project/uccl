...
struct WorkspaceLayout {
  ...
  static constexpr int64_t kNumGinBarrierSignalBytes =
      kNumGinBarrierTags * kNumMaxRanks * sizeof(uint64_t);

  __forceinline__ __device__ __host__
  WorkspaceLayout(void* workspace, int const& num_scaleout_ranks,
                  int const& num_scaleup_ranks, int const& num_experts)
      : workspace(workspace),
  ...
  get_notify_reduction_workspace_ptr() const {
    return math::advance_ptr<int64_t>(
        workspace, kNumBarrierSignalBytes + kNumGinBarrierSignalBytes);
  }

  template <bool kIsSendBuffer>
  __forceinline__ __device__ __host__ int64_t*
  get_scaleup_rank_expert_count_ptr() const {
    auto const base_ptr = math::advance_ptr<int64_t>(
  ...
  template <bool kIsSendBuffer>
  __forceinline__ __device__ __host__ int* get_scaleout_rank_expert_count_ptr()
      const {
    auto const base_ptr = math::advance_ptr<int>(
  ...
  __forceinline__ __device__ __host__ int* get_scaleout_rank_count_ptr(
      int const& scaleout_rank_idx = 0, int const& scaleup_rank_idx = 0) const {
    auto const base_ptr = get_scaleout_rank_expert_count_ptr<kIsSendBuffer>();
  ...
  __forceinline__ __device__ __host__ int* get_scaleout_expert_count_ptr(
      int const& scaleout_rank_idx = 0, int const& expert_idx = 0) const {
    auto const base_ptr =
  ...
  __forceinline__ __device__ __host__ int64_t*
  get_scaleout_channel_signaled_tail_ptr(int const& channel_idx,
                                         int const& scaleout_rank_idx) const {
    auto const base_ptr = math::advance_ptr<int64_t>(
  ...
  __forceinline__ __device__ __host__ int* get_channel_scaleup_tail_ptr(
      int const& channel_idx, int const& scaleup_rank_idx) const {
    auto const base_ptr = math::advance_ptr<int>(
  ...
  __forceinline__ __device__ __host__ int64_t* get_pp_recv_count_ptr(
      int const& offset) const {
    auto const base_ptr = math::advance_ptr<int64_t>(get_pp_send_count_ptr(0),
                                                     2 * sizeof(int64_t));
    return base_ptr + offset;
  }

  __forceinline__ __device__ __host__ int* get_agrs_recv_signal_ptr(
      int const& slot, int const& rank_idx) const {
    auto const base_ptr =
        math::advance_ptr<int>(get_pp_recv_count_ptr(0), 2 * sizeof(int64_t));
    return base_ptr + slot * kNumMaxRanks + rank_idx;
  }

  __forceinline__ __device__ __host__ int* get_agrs_session_signal_ptr(
      int const& rank_idx) const {
    auto const base_ptr = math::advance_ptr<int>(
...
struct TokenLayout {
  ...
  __forceinline__ __device__ __host__ TokenLayout(int const& num_hidden_bytes,
                                                  int const& num_sf_bytes,
                                                  int const& num_topk,
                                                  bool const& with_metadata,
                                                  void* base = nullptr)
...
struct BufferLayout {
  ...
  __forceinline__ __device__ __host__
  BufferLayout(TokenLayout const& token_layout, int const& num_ranks,
               int const& max_num_tokens_per_rank, void* base = nullptr)
      : token_layout(token_layout),
        num_ranks(num_ranks),
        num_max_tokens_per_rank(max_num_tokens_per_rank),
        base(base) {}

  __forceinline__ __device__ __host__ int64_t get_num_bytes_per_token() const {
  ...
  get_rank_buffer(int const& rank_idx) const {
    return BufferLayout(
  ...
  __forceinline__ __device__ __host__ BufferLayout
  get_channel_buffer(int const& channel_idx) const {
    EP_UNIFIED_ASSERT(num_max_tokens_per_rank % kNumTokensPerChannel == 0);
    return BufferLayout(token_layout,
                        // Do not use `num_max_tokens_per_rank /
                        // kNumTokensPerChannel` as the false stride
                        num_ranks, num_max_tokens_per_rank,
                        static_cast<int8_t*>(base) + get_num_bytes_per_token() *
                                                         kNumTokensPerChannel *
                                                         channel_idx);
  }

  __forceinline__ __device__ __host__ TokenLayout
  get_token_buffer(int const& token_idx, bool const& global = false) const {
    ...
    return TokenLayout(
        ...
        static_cast<int8_t*>(base) +
            token_layout.get_num_bytes<kWithMBarrier, int64_t>() * token_idx);
  }
};
