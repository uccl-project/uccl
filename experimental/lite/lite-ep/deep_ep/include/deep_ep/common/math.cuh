...
template <typename dtype_t>
__forceinline__ __device__ __host__ dtype_t
encode_decode_positive(dtype_t const& value) {
  return -value - static_cast<dtype_t>(1);
}

template <typename dtype_t = void>
__forceinline__ __device__ __host__ dtype_t* advance_ptr(
    void* ptr, const int64_t num_bytes) {
  return reinterpret_cast<dtype_t*>(static_cast<int8_t*>(ptr) + num_bytes);
}

__forceinline__ __device__ __host__ ptrdiff_t ptr_diff(void const* ptr,
                                                       void const* base) {
  return static_cast<int8_t const*>(ptr) - static_cast<int8_t const*>(base);
}
...
template <typename dtype_a_t, typename dtype_b_t>
__device__ __forceinline__ std::tuple<dtype_a_t, dtype_a_t> unpack2(
    dtype_b_t const& packed) {
  EP_STATIC_ASSERT(sizeof(dtype_a_t) * 2 == sizeof(dtype_b_t),
                   "Invalid dtypes");
