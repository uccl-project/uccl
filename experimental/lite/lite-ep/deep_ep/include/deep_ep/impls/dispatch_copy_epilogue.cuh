...
__global__ void __launch_bounds__(kNumThreads, 1) dispatch_copy_epilogue_impl(
    ...
    int const recv_sf_token_stride, int const recv_sf_hidden_stride,
  ...
  auto const expert_start_idx = kNumExpertsPerRank * rank_idx,
  ...
  // For no CPU sync case, the number of received tokens should be read from the
  // GPU tensor
...
#pragma unroll
  for (int i = global_warp_idx; i < num_recv_tokens; i += kNumWarps * kNumSMs) {
    ...
    auto const in_range =
        expert_start_idx <= dst_expert_idx and dst_expert_idx < expert_end_idx;
    auto const master_src_topk_idx =
    ...
    if constexpr (kDoCreateLinkedList) {
      if (ptx::elect_one_sync())
        channel_linked_list
            [tma_buffer.get_linked_list_idx_ptr()[master_src_topk_idx]] = i;
      __syncwarp();
    }
    ...
    if (kDoExpand ? (dst_tensor_idx >= 0) : ptx::elect_one_sync()) {
      ptx::tma_store_1d(
          math::advance_ptr(
              recv_x, static_cast<int64_t>(dst_tensor_idx) * kNumHiddenBytes),
...
#else
    {
      ...
      while (active_mask != 0) {
        ...
        int const b_dst_tensor_idx =
            __shfl_sync(0xffffffffu, dst_tensor_idx, src_lane);
        ptx::tma_store_1d_warp(
            math::advance_ptr(recv_x, static_cast<int64_t>(b_dst_tensor_idx) *
                                          kNumHiddenBytes),
...
#pragma unroll
      ...
      // Prepare strides
      auto const recv_sf_token_stride_i64 =
          static_cast<int64_t>(recv_sf_token_stride);
      auto const recv_sf_hidden_stride_i64 =
      ...
      while (mask) {
        int const valid_lane_idx = __ffs(mask) - 1;
        auto const gmem_dst = math::advance_ptr<sf_pack_t>(
            recv_sf, ptx::exchange(dst_tensor_idx, valid_lane_idx) *
...
#pragma unroll
        ...
        if (do_last_iter)
          gmem_dst[(kNumFullIters * 32 + lane_idx) *
                   recv_sf_hidden_stride_i64] = reg_src[kNumFullIters];
    ...
    // And:
    //   - Non-hybrid mode: the source scaleup peer rank index and master top-k
    //   lane index
...
#pragma unroll
      for (int j = 0; j < kNumScaleupRanksPerLane; ++j) {
        if (auto const k = j * 32 + lane_idx;
            j < (kNumScaleupRanksPerLane - 1) or k < kNumScaleupRanks) {
          channel_linked_list[*workspace_layout.get_channel_scaleup_tail_ptr(
              i, k)] = -1;
