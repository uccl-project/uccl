...
__global__ void __launch_bounds__(kNumThreads, 1)
    combine_reduce_epilogue_impl(nv_bfloat16* combined_x,
                                 ...
                                 int const num_combined_tokens,
                                 int const scaleout_rank_idx,
  ...
  auto const global_warp_idx =
      warp_idx * kNumSMs +
      sm_idx;  // NOTES: Here we prioritize distributing tasks to different SMs
               // to ensure that the last wave is evenly concentrated on each
               // SM.

  // Load buffers from scale-out or scale-up ranks
  extern __shared__ __align__(ptx::kNumTMAAlignBytes) int8_t smem[];
  auto const comm_token_layout =
      layout::TokenLayout(kNumHiddenBytes, 0, kNumTopk, false);
  auto const comm_buffer = layout::BufferLayout<false>(
      comm_token_layout, kNumTokensInLayout, kNumMaxTokensPerRank, recv_buffer);

  // Store buffers
  auto const output_token_layout =
      layout::TokenLayout(kNumHiddenBytes, 0, 0, false);
  auto const output_buffer = layout::BufferLayout<false>(
  ...
  // Bias layout
  auto const bias_0_buffer = layout::BufferLayout<false>(
      output_token_layout, 1, num_combined_tokens, bias_0);
  auto const bias_1_buffer = layout::BufferLayout<false>(
  ...
  for (int token_idx = global_warp_idx; token_idx < num_combined_tokens;
    ...
    if (lane_idx < kNumTopk) {
      stored_dst_expert_idx =
          static_cast<int>(combined_topk_idx[token_idx * kNumTopk + lane_idx]);
      stored_dst_rank_idx =
    ...
    auto const [should_deduplicate,
      ...
      if constexpr (kUseExpandedLayout and not kAllowMultipleReduction) {
        ...
        return {false, 0};
      } else if constexpr (kNumScaleoutRanks != 1 and not kUseExpandedLayout and
                           not kAllowMultipleReduction) {
        // Hybrid mode without expanded layout and multiple reduction. Should
        // deduplicate on a per-rank basis
    ...
    combine_reduce<kHiddenVec, kUnrollFactor, kNumTokensInLayout>(
        ...
        /* Bias 1 */ bias_1 == nullptr
            ? nullptr
            : static_cast<combine_vec_t*>(
                  bias_1_buffer.get_token_buffer(token_idx).get_base_ptr()));
    ptx::tma_store_fence();
    __syncwarp();
    ...
    if (combined_topk_weights != nullptr) {
      ...
      if (lane_idx < kNumTopk) {
        ...
        if (stored_dst_rank_idx >= 0) {
          auto const dst_ptr =
              comm_buffer
                  .get_rank_buffer(kUseRankLayout ? stored_dst_rank_idx
                                                  : master_lane_idx)
                  .get_token_buffer(token_idx)
