...
template <
    bool kIsScaleupNVLink, bool kUseExpandedLayout,
    bool kAllowMultipleReduction, int kNumSMs, int kNumWarps, int kNumRanks,
    int kHidden, int kNumMaxTokensPerRank, int kNumExperts, int kNumTopk,
    int kNumQPs, int64_t kNumTimeoutCycles, int kNumThreads = kNumWarps * 32,
    int kNumHiddenBytes = kHidden * sizeof(nv_bfloat16),
    bool kUseRankLayout =
        use_rank_layout<kAllowMultipleReduction, kNumRanks, kNumTopk>(),
    int kNumTokensInLayout = get_num_tokens_in_layout<kAllowMultipleReduction,
                                                      kNumRanks, kNumTopk>(),
    typename team_t =
...
__global__ void __launch_bounds__(kNumThreads, 1)
    combine_impl(nv_bfloat16* x, float* topk_weights, int* src_metadata,
                 ...
                 int const rank_idx, int num_reduced_tokens,
  ...
  auto const warp_idx = (ptx::get_warp_idx() + rank_idx) % kNumWarps;
  ...
  auto const tma_buffer =
      layout::BufferLayout<true>(token_layout, kNumWarps, 1, smem)
          ...
          .get_token_buffer(0);
  auto const recv_buffer = layout::BufferLayout<false>(
      token_layout, kNumTokensInLayout, kNumMaxTokensPerRank, buffer);
  auto const send_buffer = layout::BufferLayout<false>(
  ...
  int const token_start_idx = num_tokens_per_warp * global_warp_idx;
  int const token_end_idx =
  ...
  for (int i = token_start_idx; i < token_end_idx; ++i) {
    ...
    int const src_token_idx =
        __ldg(src_metadata + i * kMetadataStride) % kNumMaxTokensPerRank;
    int const src_rank_topk_idx = __ldg(src_metadata + i * kMetadataStride + 1);
    int const src_rank_idx = src_rank_topk_idx / kNumTopk;
    int const src_topk_idx = src_rank_topk_idx % kNumTopk;

    // Directly to the remote or via RDMA
    bool const nvlink_bypass = gin.is_nvlink_accessible<team_t>(src_rank_idx);
    ...
    // Hidden requirements
    EP_STATIC_ASSERT(kHidden % (32 * sizeof(int4) / sizeof(nv_bfloat16)) == 0,
                     "Invalid hidden");
    using combine_vec_t =
    ...
    if (no_local_reduce) {
      ...
      if constexpr (kUseExpandedLayout)
        token_idx_in_tensor = ptx::exchange(
            stored_topk_slot_idx, ptx::get_master_lane_idx(reduce_valid_mask));
      ...
      __syncwarp();
      ptx::tma_store_1d_warp(master_token_buffer.get_base_ptr(),
      ...
      compute_topk_slots(topk_slot_idx, reduce_valid_mask, [=](int const& idx) {
...
#pragma unroll
      for (int k = 0; k < kNumTopk; ++k) {
        ...
        if (slot_idx >= 0) {
          auto const src_token_ptr = math::advance_ptr<int4>(
              x, slot_idx * static_cast<int64_t>(kNumHiddenBytes));
          auto const token_buffer =
              recv_buffer.get_rank_buffer(k).get_token_buffer(src_token_idx);
          // Load (warp-cooperative on SM89)
          ptx::tma_store_wait();
          __syncwarp();
          ptx::tma_load_1d_warp(tma_buffer.get_base_ptr(), src_token_ptr,
          ...
          if (nvlink_bypass) {
            ...
            ptx::tma_store_1d_warp(
                gin.get_sym_ptr<team_t>(token_buffer.get_base_ptr(),
                                        src_rank_idx),
            ...
            if (ptx::elect_one_sync()) {
              gin.put<team_t>(token_buffer.get_base_ptr(),
                              send_token_buffer.get_base_ptr(), kNumHiddenBytes,
                              src_rank_idx);
            }
          }
          __syncwarp();
    ...
    if (not kUseExpandedLayout and topk_weights != nullptr and
        lane_idx < kNumTopk) {
      float const value = __ldg(topk_weights + (i * kNumTopk + lane_idx));
    ...
    if (not kDoExpandedSend and not nvlink_bypass and ptx::elect_one_sync()) {
      ptx::tma_store_wait();
      auto const dst_ptr =
          recv_buffer.get_rank_buffer(kUseRankLayout ? rank_idx : src_topk_idx)
              .get_token_buffer(src_token_idx)
              .get_base_ptr();
  ...
  comm::gpu_barrier<kIsScaleupNVLink, 1, kNumRanks, kNumSMs, kNumThreads,
                    kNumQPs, kNumTimeoutCycles, comm::kCombineTag1, true, true,
