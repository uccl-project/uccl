...
__global__ void __launch_bounds__(kNumThreads, 1) hybrid_combine_impl(
    nv_bfloat16* x, float* topk_weights, int* src_metadata,
    int* psum_num_recv_tokens_per_scaleup_rank, int* token_metadata_at_forward,
    int* channel_linked_list, const ncclDevComm_t nccl_dev_comm,
    const ncclWindow_t nccl_window, void* buffer, void* workspace,
    int const scaleout_rank_idx, int const scaleup_rank_idx,
    int num_reduced_tokens, uint64_t const* uccl_d2h_channel_addrs,
    int const uccl_num_d2h_channel_addrs, uint64_t* uccl_signal_shadow) {
  // Utils
  auto const sm_idx = static_cast<int>(blockIdx.x);
  auto const thread_idx = static_cast<int>(threadIdx.x);
  auto const warp_idx = ptx::get_warp_idx();
  ...
  bool const kAdjustRegisters =
      (kNumChannelsPerSM == 4 or kNumChannelsPerSM == 8) and
      not kUseExpandedLayout;
  ...
  if constexpr (kNumScaleupRanks == 1 and not kAllowMultipleReduction) {
    ...
    if (warp_idx >= kNumScaleupWarps) return;

    auto const channel_idx = sm_idx * kNumChannelsPerSM + warp_idx;

    if constexpr (kAdjustRegisters)
      ptx::warpgroup_reg_dealloc<kNumRegistersForScaleupWarps>();

    auto const direct_scaleout_send_buffer =
        scaleout_send_buffer.template get_channel_buffer<
          ...
          lane_idx == 0
              ? __ldg(channel_linked_list +
                      channel_idx *
                          ...
                          kNumScaleupRanks +
                      stored_ll_idx * kNumScaleupRanks)
              : -1;
      token_idx = ptx::exchange(token_idx, 0);
      if (token_idx < 0) break;

      auto const src_global_token_idx =
          __ldg(src_metadata + token_idx * kMetadataStride + 0);
      auto const src_token_idx = src_global_token_idx % kNumMaxTokensPerRank;
      auto const src_scaleout_rank_idx =
          src_global_token_idx / (kNumMaxTokensPerRank * kNumScaleupRanks);

      int stored_topk_slot_idx = -1;
      if constexpr (kUseExpandedLayout) {
        if (lane_idx < kNumTopk)
          stored_topk_slot_idx = __ldg(
              src_metadata + token_idx * kMetadataStride + (2 + lane_idx));
        __syncwarp();

#pragma unroll
        for (int k = 0; k < kNumTopk; ++k) {
          auto const slot_idx = ptx::exchange(stored_topk_slot_idx, k);
          ...
          auto const load_ptr = math::advance_ptr(
              x, static_cast<int64_t>(slot_idx) * kNumHiddenBytes);
          auto const recv_token_buffer =
              scaleout_recv_buffer.get_rank_buffer(k).get_token_buffer(
                  src_token_idx);
          auto const send_token_buffer =
              src_scaleout_rank_idx == scaleout_rank_idx
                  ? recv_token_buffer
                  : direct_scaleout_send_buffer.get_rank_buffer(k)
                        .get_token_buffer(stored_ll_idx);
          ...
          if (src_scaleout_rank_idx != scaleout_rank_idx and
              ptx::elect_one_sync()) {
            gin.put<ncclTeamTagRail>(recv_token_buffer.get_base_ptr(),
        ...
        auto const src_rank_topk_idx =
            __ldg(src_metadata + token_idx * kMetadataStride + 1);
        auto const src_topk_idx = src_rank_topk_idx % kNumTopk;
        auto const load_ptr = math::advance_ptr(
            x, static_cast<int64_t>(token_idx) * kNumHiddenBytes);
        ptx::tma_store_wait();
        __syncwarp();
        ptx::tma_load_1d_warp(tma_buffer.get_base_ptr(), load_ptr, mbarrier_ptr,
        ...
        auto const recv_buffer_rank_idx =
            kUseScaleoutRankLayout ? scaleout_rank_idx : src_topk_idx;
        auto const recv_token_buffer =
            scaleout_recv_buffer.get_rank_buffer(recv_buffer_rank_idx)
                .get_token_buffer(src_token_idx);
        auto const send_token_buffer =
            src_scaleout_rank_idx == scaleout_rank_idx
                ? recv_token_buffer
                : direct_scaleout_send_buffer
                      .get_rank_buffer(recv_buffer_rank_idx)
                      .get_token_buffer(stored_ll_idx);
        ptx::tma_store_1d_warp(send_token_buffer.get_base_ptr(),
                               tma_buffer.get_base_ptr(),
                               token_layout.get_num_bytes<false>(), lane_idx);
    ...
    if (ptx::elect_one_sync()) {
      auto const expected_signal = math::pack2<int, int64_t>(1, 0);
#pragma unroll
      for (int rank = 0; rank < kNumScaleoutRanks; ++rank) {
        auto const signal_ptr =
    ...
    if (lane_idx < kNumScaleoutRanks) {
      ...
      comm::timeout_while<kNumTimeoutCycles>([=](bool const& is_last_check) {
        ...
        if (is_last_check) {
          printf(
              "DeepEP direct combine (scale-out wait all) timeout, scale-out: "
              "%d/%d, "
  ...
  if (warp_idx < kNumScaleupWarps) {
    ...
    auto const update_tails = [&](bool const& finish = false) {
      ++update_counter;
...
#pragma unroll
        for (int i = 0; i < kNumScaleupRanksPerLane; ++i) {
          if (const auto j = i * 32 + lane_idx;
            ...
            // Also, we cannot rewrite a finished slot, if the peer is going to
            // clean it
            if (stored_num_tokens_sent[i] != stored_old_num_tokens_sent[i])
              ptx::st_release_sys(gin.get_sym_ptr<ncclTeamTagLsa>(tail_ptr, j),
    ...
    // Shape of `channel_linked_list`: `[kNumChannels, kNumMaxTokensPerChannel +
    // 1, kNumScaleupRanks]` Iterate until all scale-up peers finish
    int dst_scaleup_rank_idx = channel_idx;
    int stored_ll_idx[kNumScaleupRanksPerLane] = {},
        stored_token_idx[kNumScaleupRanksPerLane] = {};
#pragma unroll
    for (int i = 0; i < kNumScaleupRanksPerLane; ++i) stored_token_idx[i] = -1;
    while (true) {
// Load token indices in the list
#pragma unroll
      for (int i = 0; i < kNumScaleupRanksPerLane; ++i) {
        auto const j = i * 32 + lane_idx;
        stored_token_idx[i] =
            i < (kNumScaleupRanksPerLane - 1) or j < kNumScaleupRanks
                ? __ldg(channel_linked_list +
                        channel_idx *
...
#pragma unroll
      ...
      // Process tokens for all ranks together using bitmask to skip inactive
      // ranks
      EP_STATIC_ASSERT(kNumScaleupRanks <= 64,
                       "Too many scale-up ranks for 64-bit mask");
      using mask_t =
          std::conditional_t<(kNumScaleupRanks <= 32), uint32_t, uint64_t>;
      mask_t wip_mask = 0;
#pragma unroll
      for (int j = 0; j < kNumScaleupRanksPerLane; ++j)
        wip_mask |= static_cast<mask_t>(ptx::gather(stored_token_idx[j] >= 0))
                    << (j * 32);
      while (wip_mask) {
        // Find next active rank after `dst_scaleup_rank_idx` (round-robin)
        auto const start = (dst_scaleup_rank_idx + 1) % kNumScaleupRanks;
        auto const hi_mask = (wip_mask >> start) << start;
        dst_scaleup_rank_idx = hi_mask ? ptx::ffs(hi_mask) : ptx::ffs(wip_mask);
        wip_mask ^= static_cast<mask_t>(1) << dst_scaleup_rank_idx;

        // Exchange token index from the owning lane using static partition
        // iteration
        int token_idx = -1;
#pragma unroll
        for (int j = 0; j < kNumScaleupRanksPerLane; ++j) {
          auto const src_lane_idx = dst_scaleup_rank_idx - j * 32;
          token_idx =
              src_lane_idx == lane_idx ? stored_token_idx[j] : token_idx;
        }
        ...
        auto token_buffer = [&]() {
          if constexpr (kUseScaleupRankLayout) {
            ...
            return scaleup_buffer.get_rank_buffer(master_topk_idx)
                .get_token_buffer(src_scaleout_rank_idx * kNumMaxTokensPerRank +
                                  src_token_idx);
          }
        }();
        ...
        if (no_local_reduce) {
          ...
          auto const load_ptr = math::advance_ptr(
              x, static_cast<int64_t>(token_idx_in_tensor) * kNumHiddenBytes);
          ptx::tma_store_wait();
          __syncwarp();
          ptx::tma_load_1d_warp(tma_buffer.get_base_ptr(), load_ptr,
                                mbarrier_ptr, kNumHiddenBytes, lane_idx);
...
#pragma unroll
          for (int k = 0; k < kNumTopk; ++k) {
            ...
            auto const load_ptr = math::advance_ptr(
                x, static_cast<int64_t>(kDoExpandedSend ? topk_slot_idx
                                                        : token_idx) *
                       kNumHiddenBytes);
            ptx::tma_store_wait();
            __syncwarp();
            ...
            __syncwarp();
            // NOTES: We don't need to care about `topk_weights` since we are in
            // expand mode

            // Store (warp-cooperative on SM89)
            auto const dst_token_buffer =
                scaleup_buffer.get_rank_buffer(k).get_token_buffer(
                    src_scaleout_rank_idx * kNumMaxTokensPerRank +
                    src_token_idx);
            ptx::tma_store_1d_warp(
                gin.get_sym_ptr<ncclTeamTagLsa>(dst_token_buffer.get_base_ptr(),
        ...
        if constexpr (not kDoExpandedSend) {
          ...
          ptx::tma_store_1d_warp(token_buffer.get_base_ptr(),
                                 tma_buffer.get_base_ptr(),
                                 token_layout.get_num_bytes<false>(), lane_idx);
          ptx::tma_store_commit();
        }
...
#pragma unroll
    ...
    auto const forward_warp_idx = warp_idx - kNumScaleupWarps;
    auto const channel_idx = sm_idx * kNumChannelsPerSM + forward_warp_idx;
    ...
    scaleout_send_buffer =
        scaleout_send_buffer
            .get_channel_buffer<kNumScaleoutRanks * kNumMaxTokensPerChannel>(
                channel_idx);
    ...
    auto const flush_last_tma_and_issue_rdma = [&]() {
      if (last_src_scaleout_rank_idx >= 0 and ptx::elect_one_sync()) {
        ...
        if (last_src_scaleout_rank_idx != scaleout_rank_idx) {
          gin.put<ncclTeamTagRail>(
              ...
              token_layout.get_num_bytes<false>(), last_src_scaleout_rank_idx,
    ...
    for (int i = 0;; ++i) {
      auto const src_token_global_idx =
          __ldg(token_metadata_at_forward + i * kNumForwardMetadataDims);
      auto const is_token_last_in_chunk =
          __ldg(token_metadata_at_forward + i * kNumForwardMetadataDims + 1);
      auto const src_rank_idx = src_token_global_idx / kNumMaxTokensPerRank;
      auto const src_scaleout_rank_idx = src_rank_idx / kNumScaleupRanks;
      auto const src_token_idx = src_token_global_idx % kNumMaxTokensPerRank;
      auto stored_src_scaleup_rank_idx =
          lane_idx < kNumTopk
              ? __ldg(token_metadata_at_forward + i * kNumForwardMetadataDims +
                      2 + lane_idx)
              : -1;
      auto stored_src_slot_idx =
          lane_idx < kNumTopk
              ? __ldg(token_metadata_at_forward + i * kNumForwardMetadataDims +
                      2 + kNumTopk + lane_idx)
              : -1;
...
#pragma unroll
          for (int j = 0; j < kNumScaleupRanksPerLane; ++j) {
            printf(
                "DeepEP combine (scale-up wait) timeout, scale-out: %d/%d, "
                "scale-up: %d/%d, "
                "channel: %d, lane: %d, recv: %d, tail: %d (wait=%d)\n",
                scaleout_rank_idx, kNumScaleoutRanks, scaleup_rank_idx,
                kNumScaleupRanks, channel_idx, j * 32 + lane_idx,
                stored_num_tokens_recv[j], stored_cached_scaleup_tail[j],
                stored_is_scaleup_rank_needed[j]);
          }
        }
...
#pragma unroll
      ...
      if constexpr (not kAllowMultipleReduction) {
        // Cases where multiple reduction is disabled. We need to forward all
        // data from scaleup peers to scaleout peers
        // TODO: Let scale-up warps directly put data into `send_buffer`?
        auto const src_slot_idx =
            src_scaleout_rank_idx * kNumMaxTokensPerRank + src_token_idx;
        auto topk_valid_mask =
            kUseExpandedLayout
                ? ptx::gather(stored_src_scaleup_rank_idx >= 0)
                : ptx::gather(ptx::deduplicate(stored_src_scaleup_rank_idx,
                                               lane_idx) and
...
#pragma unroll
        for (int k = 0; k < kNumTopk; ++k) {
          ...
          auto const recv_buffer_ptr = scaleout_recv_buffer.get_rank_buffer(k)
                                           .get_token_buffer(src_token_idx)
                                           .get_base_ptr();
        ...
        if constexpr (kUseScaleupRankLayout) {
          stored_src_buffer_idx = stored_src_scaleup_rank_idx *
          ...
          stored_src_buffer_idx =
              stored_src_slot_idx == -1
                  ? -1
                  : lane_idx * scaleup_buffer.num_max_tokens_per_rank +
                        src_slot_idx;
        }
        ...
        // Do reduce
        constexpr int kUnrollFactor = get_max_unroll_factor < kHiddenVec,
                      kAdjustRegisters ? 8 : 4 > ();
        combine_reduce<kHiddenVec, kUnrollFactor,
                       math::constexpr_ceil_div(kNumTopk, kNumScaleoutRanks)>(
            lane_idx, topk_slot_idx,
            static_cast<combine_vec_t*>(tma_buffer.get_base_ptr()),
            /* Get source base */
            [=](int const& slot_idx) {
        ...
        stored_src_buffer_idx = ptx::exchange(
            stored_src_buffer_idx,
            ptx::get_master_lane_idx(ptx::match(stored_src_scaleup_rank_idx)));
        ...
        // NOTES: as we only have 1 destination, we will use "send" as "recv"
        // for local transfer
        int scaleout_recv_buffer_rank_idx;
        if constexpr (kUseScaleoutRankLayout) {
          scaleout_recv_buffer_rank_idx = scaleout_rank_idx;
        } else {
          int const src_topk_idx = ptx::get_master_lane_idx(
              ptx::gather(stored_src_scaleup_rank_idx >= 0));
          scaleout_recv_buffer_rank_idx = src_topk_idx;
        }
        auto const recv_token_buffer =
            scaleout_recv_buffer.get_rank_buffer(scaleout_recv_buffer_rank_idx)
                .get_token_buffer(src_token_idx);
        auto const send_token_buffer =
            src_scaleout_rank_idx == scaleout_rank_idx
                ? recv_token_buffer
...
#pragma unroll
    ...
    if (lane_idx < kNumScaleoutRanks) {
      ...
      comm::timeout_while<kNumTimeoutCycles>([=](bool const& is_last_check) {
        ...
        if (is_last_check) {
          printf(
              "DeepEP combine (scale-out wait all) timeout, scale-out: %d/%d, "
              "scale-up: %d/%d, "
              "channel: %d, lane: %d, signal: %lld, expected: %lld\n",
              scaleout_rank_idx, kNumScaleoutRanks, scaleup_rank_idx,
              kNumScaleupRanks, channel_idx, lane_idx, signal, expected_signal);
        }
        return false;
      });
    }
