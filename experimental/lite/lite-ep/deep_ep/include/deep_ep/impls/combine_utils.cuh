...
struct CombineVecTraits {
  ...
  // On SM100+, use longlong4_t (32 bytes) if hidden is aligned, otherwise fall
  // back to int4 (16 bytes)
...
__device__ __forceinline__ void compute_topk_slots(
    ...
    fetch_func_t const& fetch_func) {
#pragma unroll
  for (int k = 0; k < kNumValidTopk; ++k) {
    int const lowest_idx = __ffs(mask) - 1;
    // Here we perform the exchange unconditionally to avoid `BRA.DIV`
    auto const fetched = fetch_func(lowest_idx);
    mask &= mask - 1;
...
__device__ __forceinline__ void combine_reduce(
    int const& lane_idx, int (&topk_slot_idx)[kNumValidTopk],
    vec_t* dst_buffer_ptr,
    get_src_buffer_ptr_func_t const& get_src_buffer_ptr_func,
    wait_buffer_func_t const& wait_buffer_func, vec_t* bias_0 = nullptr,
  ...
  bool const enable_hadd_bypass = (bias_0 == nullptr and bias_1 == nullptr) and
...
#pragma unroll 1
    for (int i = 0; i < kHiddenVec / (kUnrollFactor * 32); ++i) {
      // Read values 0
      auto const slot_0 = topk_slot_idx[0];
      auto const src_base_ptr_0 = get_src_buffer_ptr_func(slot_0);
      vec_t values_0[kUnrollFactor] = {};
#pragma unroll
      for (int j = 0; j < kUnrollFactor; ++j) {
        values_0[j] = ptx::ldg_with_gez_pred(
            src_base_ptr_0 + (i * (kUnrollFactor * 32) + j * 32 + lane_idx),
            slot_0);
      }

      // Read values 1
      vec_t values_1[kUnrollFactor] = {};
      auto const slot_1 = kNumValidTopk == 1 ? -1 : topk_slot_idx[1];
      auto const src_base_ptr_1 = get_src_buffer_ptr_func(slot_1);
#pragma unroll
      for (int j = 0; j < kUnrollFactor; ++j) {
        values_1[j] = ptx::ldg_with_gez_pred(
            src_base_ptr_1 + (i * (kUnrollFactor * 32) + j * 32 + lane_idx),
            slot_1);
      }
      ...
      auto const bf162_view_0 = reinterpret_cast<nv_bfloat162*>(values_0);
      auto const bf162_view_1 = reinterpret_cast<nv_bfloat162*>(values_1);
#pragma unroll
      for (int j = 0; j < kUnrollFactor; ++j) {
#pragma unroll
...
#pragma unroll 1
    for (int i = 0; i < kHiddenVec / (kUnrollFactor * 32); ++i) {
      ...
      auto const add_bias = [&](vec_t const* base_ptr) {
        // Read
        vec_t values[kUnrollFactor];
#pragma unroll
        for (int j = 0; j < kUnrollFactor; ++j)
          values[j] =
              ptx::ldg(base_ptr + i * (kUnrollFactor * 32) + j * 32 + lane_idx);
...
#pragma unroll
      for (int k = 0; k < kNumValidTopk; ++k) {
        ...
        // Read values
        auto const src_base_ptr = get_src_buffer_ptr_func(topk_slot_idx[k]);
        vec_t values[kUnrollFactor] = {};
#pragma unroll
        for (int j = 0; j < kUnrollFactor; ++j) {
          values[j] = ptx::ldg_with_gez_pred(
              src_base_ptr + (i * (kUnrollFactor * 32) + j * 32 + lane_idx),
              topk_slot_idx[k]);
        }

        // Reduce
        auto const bf162_view = reinterpret_cast<nv_bfloat162*>(values);
#pragma unroll
        for (int j = 0; j < kUnrollFactor * kNumElemsPerVec / 2; ++j)
          ptx::accumulate(reduced[j], bf162_view[j]);
      }
...
#pragma unroll
        for (int l = 0; l < kNumElemsPerVec / 2; ++l)
          bf162_view[l] =
              __float22bfloat162_rn(reduced[j * (kNumElemsPerVec / 2) + l]);
        dst_buffer_ptr[i * (kUnrollFactor * 32) + j * 32 + lane_idx] =
            casted_value;
      }
    }
