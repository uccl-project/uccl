...
template <
    ...
    int kNumThreads = kNumNotifyThreads + kNumScaleoutSendThreads +
                      kNumForwardThreads>
__global__ void __launch_bounds__(kNumThreads, 1) hybrid_dispatch_impl(
    void* x, sf_pack_t* sf, topk_idx_t* topk_idx, float* topk_weights,
    topk_idx_t* copied_topk_idx, int* cumulative_local_expert_recv_stats,
    int* psum_num_recv_tokens_per_scaleup_rank,
    int* psum_num_recv_tokens_per_expert, int* dst_buffer_slot_idx,
    int* token_metadata_at_forward, int const num_tokens,
    int const sf_token_stride, int const sf_hidden_stride,
  ...
  auto const host_workspace_layout = layout::WorkspaceLayout(
      mapped_host_workspace, kNumScaleoutRanks, kNumScaleupRanks, kNumExperts);
  ...
  auto const [qp_idx, sharing_mode] =
  ...
  // warps
  auto const token_layout = layout::TokenLayout(
      kNumHiddenBytes, kNumSFPacks * sizeof(sf_pack_t), kNumTopk, true);
  auto const tma_buffer =
  ...
  auto scaleup_buffer = layout::BufferLayout<false>(
      token_layout, kNumScaleupRanks, kNumScaleoutRanks * kNumMaxTokensPerRank,
      buffer);
  ...
  auto const mbarrier_ptr = tma_buffer.get_mbarrier_ptr();
  ...
  if (warp_idx < kNumNotifyWarps) {
    ...
    auto const rank_expert_count = math::advance_ptr<int>(smem, 0);

    // Clean initial counts
    // NOTES: if you want to change the order of different warp roles, please
    // take care of the `thread_idx`
...
#pragma unroll
    ...
    auto const global_warp_idx = sm_idx * kNumNotifyWarps + warp_idx;
...
#pragma unroll
    for (int i = thread_idx; i < kNumRanks + kNumExperts;
      ...
      ptx::red_add(workspace_layout.get_notify_reduction_workspace_ptr() + i,
                   counter);
    }
...
#pragma unroll
      for (int i = thread_idx; i < kNumRanks + kNumExperts;
           i += kNumNotifyThreads) {
        comm::timeout_while<kNumTimeoutCycles>([=](bool const& is_last_check) {
          ...
          if (is_last_check) {
            printf(
                "DeepEP hybrid notify (GPU reduction) timeout, scale-out: "
                "%d/%d, scale-up: %d/%d, "
                "thread: %d, status: %d | %d, expected: %d\n",
                scaleout_rank_idx, kNumScaleoutRanks, scaleup_rank_idx,
                kNumScaleupRanks, thread_idx, static_cast<int>(status >> 32),
      ...
      if (thread_idx < kNumScaleoutRanks) {
        auto const dst_scaleout_rank_idx = thread_idx;
      ...
      auto const recv_and_reduce = [=](auto const& get_ptr_func,
                                       bool const& is_expert_reduction =
                                           false) -> int {
        int count = 0;
#pragma unroll
        for (int j = 0; j < kNumScaleoutRanks; ++j) {
          ...
          comm::timeout_while<kNumTimeoutCycles>(
              [&](const bool& is_last_check) {
                ...
                decoded = math::encode_decode_positive(static_cast<int>(
                    uccl::ld_cv_u32(reinterpret_cast<const uint32_t*>(ptr))));
#else
                decoded =
                    math::encode_decode_positive(ptx::ld_acquire_sys<int>(ptr));
#endif
                if (math::is_decoded_positive_ready(decoded)) return true;

                if (is_last_check) {
                  printf(
                      "DeepEP hybrid notify (scale-out %s reduction) timeout, "
...
#pragma unroll
      for (int i = thread_idx; i < kNumScaleupRanks; i += kNumNotifyThreads) {
        ...
        auto const count = recv_and_reduce([=](int const& scaleout_peer_idx) {
          return workspace_layout.get_scaleout_rank_count_ptr<false>(
              scaleout_peer_idx, i);
        });
...
#pragma unroll
      for (int i = thread_idx; i < kNumExpertsPerScaleout;
           i += kNumNotifyThreads) {
        // Wait scale-out arrival and reduce
        auto const count = recv_and_reduce(
            [=](int const& scaleout_peer_idx) {
              return workspace_layout.get_scaleout_expert_count_ptr<false>(
                  scaleout_peer_idx, i);
            },
            true);
        ...
        auto const dst_scaleup_rank_idx = i / kNumExpertsPerRank;
        auto const expert_idx_in_dst_rank = i % kNumExpertsPerRank;
      ...
      // NOTES: from now on, the `rank` and `expert`s size change into the local
      // size
      expert_count = rank_expert_count + kNumScaleupRanks;

      // Wait local counters to be ready
      // NOTES: here we only care the prefix sum by scale-up peers (used for
      // later epilogue), not all ranks
      EP_STATIC_ASSERT(
          kNumNotifyWarps == 0 or
              kNumScaleupRanks + kNumExpertsPerRank <= kNumNotifyWarps * 32,
          "Insufficient notify threads");
      comm::timeout_while<kNumTimeoutCycles>(
          thread_idx < kNumScaleupRanks + kNumExpertsPerRank,
          [&](bool const& is_last_check) {
            const auto status = ptx::ld_volatile<int64_t>(
                workspace_layout.get_scaleup_rank_expert_count_ptr<false>() +
                thread_idx);
            if ((status >> 32ull) == kNumScaleupRanks) {
              ...
              workspace_layout
                  .get_scaleup_rank_expert_count_ptr<false>()[thread_idx] = 0;
              if constexpr (kDoCPUSync) {
                host_workspace_layout
                    .get_scaleup_rank_expert_count_ptr<false>()[thread_idx] =
            ...
            if (is_last_check) {
              printf(
                  ...
                  scaleout_rank_idx, kNumScaleoutRanks, scaleup_rank_idx,
                  kNumScaleupRanks, thread_idx, static_cast<int>(status >> 32),
                  static_cast<int>(status & 0xffffffff), kNumScaleupRanks);
            }
            return false;
          });
      ptx::named_barrier<kNumNotifyThreads>(kNotifyBarrierIndex);
      ...
      // NOTES: we may have fast implementation with `cub::BlockScan`, but it is
      // too heavy to use
      auto const do_psum = [=](int const* count, int* out, int const n,
                               int const is_exclusive) {
        int psum = 0;
#pragma unroll
    ...
    int const scaleout_warp_idx = warp_idx - kNumNotifyWarps;
    int const channel_idx = sm_idx * kNumChannelsPerSM + scaleout_warp_idx;
    ...
    auto const update_scaleout_tail = [&](bool const& finish_flag = false) {
      if (lane_idx < kNumScaleoutRanks and
        ...
        const auto signaled_tail =
            math::pack2<int, int64_t>(finish_flag, stored_scaleout_tail);
        const auto ptr =
        ...
        // NOTES: the "release" scope will be `sys` for the local rank (we may
        // involve NVLink so not `gpu`) For RDMA requests, "release" is ensured
        // by "atomic"
#ifdef EP_USE_UCCL_PROXY
        gin.put_value<ncclTeamTagRail>(ptr, signaled_tail, lane_idx);
#else
        gin.red_add_rel<ncclTeamTagRail>(ptr, signaled_tail - old_signaled_tail,
                                         lane_idx);
#endif
    ...
    // Preload next token
    auto const preload_next_token = [&](int const& token_idx) {
      ...
      ptx::tma_load_1d_warp(
          ...
          math::advance_ptr(x, token_i64_idx * kNumHiddenBytes), mbarrier_ptr,
...
#pragma unroll
        ...
        if (kNumFullIters * 32 + lane_idx < kNumSFPacks) {
          ptx::cp_async_ca(
              gmem_src_ptr + (kNumFullIters * 32 + lane_idx) * sf_hidden_stride,
    ...
    for (int token_idx = channel_idx; token_idx < num_tokens;
      ...
      if (lane_idx < kNumTopk) {
        auto const uncasted_dst_expert_idx =
            __ldg(topk_idx + token_idx * kNumTopk + lane_idx);
        auto const dst_expert_idx = static_cast<int>(uncasted_dst_expert_idx);
        stored_dst_scaleout_rank_idx =
            dst_expert_idx >= 0 ? dst_expert_idx / kNumExpertsPerScaleout : -1;
      ...
      auto const stored_old_slot_idx = ptx::exchange(
          stored_scaleout_tail,
      ...
      // broadcast the matching slot index so all lanes participate in the
      // store.
      auto const local_match_mask = __ballot_sync(
          0xffffffffu, stored_dst_slot_idx >= 0 and
                           stored_dst_scaleout_rank_idx == scaleout_rank_idx);
      if (local_match_mask != 0) {
        int const src_lane = __ffs(local_match_mask) - 1;
        int const bcast_slot_idx =
            __shfl_sync(0xffffffffu, stored_dst_slot_idx, src_lane);
        ptx::tma_store_1d_warp(
            scaleout_recv_buffer.get_token_buffer(bcast_slot_idx)
                .get_base_ptr(),
            tma_buffer.get_base_ptr(), tma_buffer.get_num_bytes<false>(),
            lane_idx);
      }
      ...
      if (stored_dst_slot_idx >= 0 and
        ...
        gin.put<ncclTeamTagRail>(
            scaleout_recv_buffer.get_token_buffer(stored_dst_slot_idx)
                .get_base_ptr(),
            scaleout_send_buffer.get_token_buffer(token_idx).get_base_ptr(),
            tma_buffer.get_num_bytes<false>(), stored_dst_scaleout_rank_idx,
            ncclGinOptFlagsAggregateRequests);
      }
      __syncwarp();
    ...
    int const forward_warp_idx =
        warp_idx - (kNumNotifyWarps + kNumScaleoutWarps);
    int const channel_idx = sm_idx * kNumChannelsPerSM + forward_warp_idx;
    scaleout_recv_buffer =
        scaleout_recv_buffer.get_channel_buffer<kNumMaxTokensPerChannel>(
            channel_idx);
    scaleup_buffer = scaleup_buffer.get_rank_buffer(scaleup_rank_idx);

    // Shape of `token_metadata_at_forward`: `[kNumChannels, kNumScaleoutRanks *
    // kNumMaxTokensPerChannel + 1, kNumForwardMetadataDims]`
    ...
    dst_buffer_slot_idx +=
        channel_idx * (kNumScaleoutRanks * kNumMaxTokensPerChannel * kNumTopk);

    // Transform linked list index
    auto const transform_linked_list_idx = [=](int const& idx) {
    ...
    while ((wip_mask = ptx::gather(stored_scaleout_tail_idx >
      ...
      auto const offset = (recv_scaleout_rank_idx + 1) % kNumScaleoutRanks;
      auto const hi_mask = (wip_mask >> offset) << offset;
      ...
      comm::timeout_while<kNumTimeoutCycles>([&](bool const& is_last_check) {
        ...
        if (is_last_check) {
          if (lane_idx < kNumScaleoutRanks) {
            printf(
                "DeepEP hybrid dispatch (forwarding) timeout, scale-out: %d, "
                "scale-up: %d, "
                "channel: %d, lane: %d, old scale-out tail: %d, scale-out "
                "tail: (%d, %d)\n",
                scaleout_rank_idx, scaleup_rank_idx, channel_idx, lane_idx,
                stored_scaleout_old_tail_idx, stored_finish_flag,
...
#ifdef EP_USE_UCCL_PROXY
          const auto signaled_tail = static_cast<int64_t>(
              uccl::ld_cv_u64(reinterpret_cast<const uint64_t*>(wait_ptr)));
#else
          const auto signaled_tail = ptx::ld_acquire_sys<int64_t>(wait_ptr);
#endif
          math::unpack2<int, int64_t>(signaled_tail, stored_finish_flag,
                                      stored_scaleout_tail_idx);
        }
        __syncwarp();
        return false;
      ...
      auto const start_slot_idx =
          ptx::exchange(stored_scaleout_old_tail_idx, recv_scaleout_rank_idx);
      auto const end_slot_idx = std::min(
      ...
      for (int slot_idx = start_slot_idx; slot_idx < end_slot_idx; ++slot_idx) {
        auto const token_buffer = recv_buffer.get_token_buffer(slot_idx);
        ...
        ptx::tma_load_1d_warp(tma_buffer.get_base_ptr(),
        ...
        auto dst_expert_idx =
            lane_idx < kNumTopk ? tma_buffer.get_topk_idx_ptr()[lane_idx] : -1;
        dst_expert_idx -= scaleout_rank_idx * kNumExpertsPerScaleout;
        stored_dst_scaleup_rank_idx =
            0 <= dst_expert_idx and dst_expert_idx < kNumExpertsPerScaleout
...
#pragma unroll
        for (int j = 0; j < kNumScaleupRanksPerLane; ++j) {
          auto const src_lane_idx = stored_dst_scaleup_rank_idx - j * 32;
          bool const valid = 0 <= src_lane_idx and src_lane_idx < 32;
          auto const exchanged = ptx::exchange(stored_scaleup_send_counters[j],
        ...
        int stored_dst_slot_idx = -1;
        auto const dst_slot_idx_ptr =
            dst_buffer_slot_idx +
            recv_scaleout_rank_idx * (kNumMaxTokensPerChannel * kNumTopk) +
            slot_idx * kNumTopk;
        if constexpr (kReuseSlotIndices) {
          ...
          if (ptx::deduplicate(stored_dst_scaleup_rank_idx, lane_idx) and
              stored_dst_scaleup_rank_idx >= 0)
            stored_dst_slot_idx =
                atomicAdd(workspace_layout.get_scaleup_atomic_sender_counter() +
                              stored_dst_scaleup_rank_idx,
                          1);
        }
        __syncwarp();
        ...
        if (stored_dst_slot_idx >= 0) {
          auto const dst_ptr = gin.get_sym_ptr<ncclTeamTagLsa>(
        ...
        {
          auto active_mask =
              __ballot_sync(0xffffffffu, stored_dst_slot_idx >= 0);
          while (active_mask != 0) {
            int const src_lane = __ffs(active_mask) - 1;
            int const b_slot_idx =
                __shfl_sync(0xffffffffu, stored_dst_slot_idx, src_lane);
            int const b_scaleup_rank =
                __shfl_sync(0xffffffffu, stored_dst_scaleup_rank_idx, src_lane);
            auto const b_dst_ptr = gin.get_sym_ptr<ncclTeamTagLsa>(
        ...
        using mask_t = std::conditional_t<kNumScaleupRanks <= 32, unsigned,
                                          unsigned long long>;
        auto const scaleup_send_mask =
            ptx::reduce_or(stored_dst_scaleup_rank_idx >= 0
...
#pragma unroll
    ...
    // Assign the source token index part of the metadata into `-1` as an ending
    // mark
    ...
    if constexpr (not kReuseSlotIndices) {
      auto const tail_ptr = workspace_layout.get_channel_scaleup_tail_ptr(
          channel_idx, scaleup_rank_idx);
#pragma unroll
      for (int i = 0; i < kNumScaleupRanksPerLane; ++i) {
        if (auto const j = i * 32 + lane_idx;
    ...
    if (lane_idx < kNumScaleoutRanks)
      *workspace_layout.get_scaleout_channel_signaled_tail_ptr(channel_idx,
                                                               lane_idx) = 0;
    __syncwarp();
  }

  // Scale-up barrier to ensure data arrival
  // As scale-out tokens have already been consumed by forwarders, no need to do
  // scale-out barrier again
  comm::gpu_barrier<true, kNumScaleoutRanks, kNumScaleupRanks, kNumSMs,
                    kNumThreads, kNumQPs, kNumTimeoutCycles,
