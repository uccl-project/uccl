...
template <
    bool kIsScaleupNVLink, bool kDoCPUSync, bool kReuseSlotIndices, int kNumSMs,
    int kNumNotifyWarps, int kNumDispatchWarps, int kNumRanks,
    int kNumHiddenBytes, int kNumSFPacks, int kNumMaxTokensPerRank,
    int kNumExperts, int kNumTopk, int kExpertAlignment, int kNumQPs,
    int64_t kNumTimeoutCycles, int kNumNotifyThreads = kNumNotifyWarps * 32,
    int kNumDispatchThreads = kNumDispatchWarps * 32,
    int kNumThreads = kNumNotifyThreads + kNumDispatchThreads,
    typename team_t =
        std::conditional_t<kIsScaleupNVLink, ncclTeamTagLsa, ncclTeamTagWorld>>
__global__ void __launch_bounds__(kNumThreads, 1) dispatch_impl(
    ...
    int const num_tokens, int const sf_token_stride, int const sf_hidden_stride,
    const ncclDevComm_t nccl_dev_comm, const ncclWindow_t nccl_window,
    void* buffer, void* workspace, void* mapped_host_workspace,
    int const rank_idx, uint64_t const* uccl_d2h_channel_addrs,
  ...
  auto const host_workspace_layout =
      layout::WorkspaceLayout(mapped_host_workspace, 1, kNumRanks, kNumExperts);
  ...
  auto const gin =
      handle::NCCLGin(nccl_dev_comm, nccl_window, qp_idx, sharing_mode,
                      workspace, uccl_d2h_channel_addrs,
                      uccl_num_d2h_channel_addrs, rank_idx, uccl_signal_shadow,
                      /*local_scaleout_rank_idx=*/-1,
                      /*local_scaleup_rank_idx=*/-1, uccl_shared_per_rank_bytes,
...
#pragma unroll
    ...
    auto const global_warp_idx = warp_idx * kNumSMs + sm_idx;
...
#pragma unroll
            ...
            true, [=](bool const& is_last_check) {
              ...
              if ((status >> 32) == kNumSMs) {
                ...
                if constexpr (not kIsScaleupNVLink)
                  workspace_layout
                      .get_scaleup_rank_expert_count_ptr<true>()[i] = encoded;
              ...
              if (is_last_check) {
                printf(
                    "DeepEP notify (GPU reduction) timeout, rank: %d/%d, "
      ...
      if constexpr (kIsScaleupNVLink) {
        ...
        for (int i = thread_idx; i < kNumExperts; i += kNumNotifyThreads) {
          auto const idx =
              kNumExpertsPerRank * rank_idx + (i % kNumExpertsPerRank);
      ...
      ptx::named_barrier<kNumNotifyThreads>(kNotifyBarrierIndex);
      if (warp_idx == 0) gin.gin.flush(ncclCoopWarp());
      ptx::named_barrier<kNumNotifyThreads>(kNotifyBarrierIndex);

      // Wait for rank and expert count
      auto const start_clock = clock64();
      for (int i = thread_idx; i < kNumRanks + kNumExperts;
           i += kNumNotifyThreads) {
        comm::timeout_while<kNumTimeoutCycles>(
        ...
        // NOTES: the global memory type has 64 bits
              ...
              const auto count = static_cast<int>(static_cast<int64_t>(
                  uccl::ld_cv_u64(reinterpret_cast<uint64_t*>(
                      ...
                      i))));
#else
              const auto count = static_cast<int>(ptx::ld_volatile<int64_t>(
...
#pragma unroll
      ...
      if (warp_idx == 0) {
        ...
        do_psum(rank_count, psum_num_recv_tokens_per_scaleup_rank, kNumRanks,
                0);
      } else if (warp_idx == 1) {
    ...
    int const dispatch_warp_idx = warp_idx - kNumNotifyWarps;
    ...
    // Iterate all tokens
    auto const token_start = dispatch_warp_idx * kNumSMs + sm_idx;
    auto const token_stride = kNumDispatchWarps * kNumSMs;
    for (int token_idx = token_start; token_idx < num_tokens;
         token_idx += token_stride) {
      auto const token_i64_idx = static_cast<int64_t>(token_idx);
      ...
      if constexpr (kNumSFPacks > 0) {
        ...
        auto const gmem_src_ptr = math::advance_ptr<sf_pack_t>(
            sf, token_i64_idx * sf_token_stride * sizeof(sf_pack_t));
        auto const smem_dst_ptr = tma_buffer.get_sf_ptr();
...
#pragma unroll
      ...
      if constexpr (kReuseSlotIndices) {
        if (lane_idx < kNumTopk)
          stored_dst_slot_idx =
              __ldg(dst_buffer_slot_idx + token_idx * kNumTopk + lane_idx);
        stored_dst_slot_idx =
            stored_dst_slot_idx >= 0
                ? (stored_dst_slot_idx - rank_idx * kNumMaxTokensPerRank)
                : -1;
      } else {
      ...
      // If any active lane needs the RDMA fallback (dst_ptr == nullptr but slot
      // is valid), we must populate the local send_buffer first so the proxy
      // can pick it up.
      bool const needs_local_send_buf =
          stored_dst_slot_idx >= 0 and dst_ptr == nullptr;
      auto const warp_needs_local =
          ptx::reduce_or(needs_local_send_buf ? 1u : 0u);
      if constexpr (not kIsScaleupNVLink) {
        if (warp_needs_local) {
      ...
      // SM89: serialize per active lane and use the warp-cooperative store.
      {
        ...
        while (active_mask != 0) {
          ...
          auto const b_dst_ptr = reinterpret_cast<uint8_t*>(__shfl_sync(
              0xffffffffu, reinterpret_cast<uintptr_t>(dst_ptr), src_lane));
  ...
  comm::gpu_barrier<kIsScaleupNVLink, 1, kNumRanks, kNumSMs, kNumThreads,
                    kNumQPs, kNumTimeoutCycles, comm::kDispatchTag1, true, true,
                    false>(gin, workspace_layout, 0, rank_idx, sm_idx,
                           thread_idx);
