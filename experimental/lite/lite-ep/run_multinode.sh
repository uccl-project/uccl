...
DEFAULT_TRANSPORT="uccl-no-gdr"

COMMON_ENV="WORLD_SIZE=2 LOCAL_WORLD_SIZE=$GPUS_PER_NODE MASTER_ADDR=$MASTER_ADDR MASTER_PORT=$MASTER_PORT CUDA_VISIBLE_DEVICES=$GPU_LIST PYTHONPATH=$EP_DIR EP_NCCL_ROOT_DIR=$NCCL_ROOT LD_LIBRARY_PATH=$NCCL_ROOT/lib:\${LD_LIBRARY_PATH:-} LD_PRELOAD=$LD_PRELOAD_VALUE EP_SUPPRESS_NCCL_CHECK=1 LITE_EP_TRANSPORT=\${LITE_EP_TRANSPORT:-$DEFAULT_TRANSPORT} LITE_EP_NVLINK=\${LITE_EP_NVLINK:-0} NCCL_IB_HCA=\${NCCL_IB_HCA:-mlx5_0,mlx5_1} NCCL_GIN_TYPE=\${NCCL_GIN_TYPE:-2} DISABLE_SM90_FEATURES=1 EP_JIT_CACHE_DIR=$EP_JIT_CACHE_DIR"
