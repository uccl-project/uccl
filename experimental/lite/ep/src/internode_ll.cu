...
#pragma unroll
      ...
      if (dst_expert_idx >= 0) {
        ...
        __syncwarp();
        lane_id == 0 ? atomic_add_release_global<kUseAggressiveAtomic>(
...
#pragma unroll
      for (int i = lane_id; i < num_experts; i += WARP_SIZE)
        atomic_add_release_global<kUseAggressiveAtomic>(
...
#pragma unroll
    ...
    __syncwarp();
    if (lane_id == 0)
      atomic_add_release_global<kUseAggressiveAtomic>(atomic_clean_flag,
                                                      num_experts);
  }
...
#if defined(__NVCC__) && __CUDA_ARCH__ >= 900
    // TMA stuffs
...
#else
        // sm_89 fallback: use regular global loads/stores instead of TMA
        if constexpr (kUseLogFMT) {
          // LogFMT path: load, transform, store via registers
          constexpr float kThreshold = 1;
          constexpr float kMinClip = 32;
          constexpr int kNumBits = 10;
          constexpr int kNumValues = 1 << (kNumBits - 1);

          for (int i = lane_id * kNumUnrolls; i < hidden_bf16_int4_pad;
               i += WARP_SIZE * kNumUnrolls) {
            int4 int4_values[kNumUnrolls] = {make_int4(0, 0, 0, 0)};
            auto uint32_values = reinterpret_cast<uint32_t*>(int4_values);

            // Load from global memory
#pragma unroll
            for (int u = 0; u < kNumUnrolls; ++u) {
              if (i + u < hidden_bf16_int4)
                int4_values[u] = __ldg(cpy_src_int4_ptr + i + u);
            }
...
#pragma unroll
            for (int u = 0; u < kNumUnrolls; ++u) {
              if (i + u < hidden_bf16_int4)
                cpy_dst_int4_ptr[i + u] = int4_values[u];
            }
          }
        } else {
          // Non-LogFMT: simple warp copy via global loads/stores
          UNROLLED_WARP_COPY(kNumUnrolls, lane_id, hidden_bf16_int4,
