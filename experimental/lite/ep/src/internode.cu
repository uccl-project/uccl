...
struct SourceMeta {
  ...
  EP_STATIC_ASSERT(NUM_MAX_NVL_PEERS == 8 || NUM_MAX_NVL_PEERS == 1,
...
#else
        // PCIE_INTRANODE: each GPU is its own NVL group (NVL_PEERS=1)
        bool token_in_rank = is_token_in_rank[i * num_ranks +
            dst_rdma_rank * NUM_MAX_NVL_PEERS];
        per_nvl_rank_count[0] += token_in_rank;
        total_count += token_in_rank;
#endif
      }
