...
// DeepEPv2 UCCL mode uses the CUDA-mapped host D2H ring buffer backend.
// The legacy MSCCLPP FIFO path is kept in the copied reference code but is not
// used here, which avoids an extra runtime dependency and makes the command
// addresses directly GPU-visible host-pinned ring buffers.
