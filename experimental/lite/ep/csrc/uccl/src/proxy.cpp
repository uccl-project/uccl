...
void Proxy::init_common() {
  ...
  std::thread receiver_thread([this, num_ranks, my_rank, num_expected_conns]() {
    for (int i = 0; i < num_expected_conns; ++i) {
      int actual_peer;
  ...
  if (!wrs_to_post.empty()) {
    if (std::getenv("UCCL_PROXY_TRACE")) {
      fprintf(stderr,
              ...
              cfg_.rank, cfg_.thread_idx, cmds_to_post.size(),
              static_cast<int>(get_base_cmd(cmds_to_post.front().cmd_type)),
              cmds_to_post.front().dst_rank,
              static_cast<unsigned>(cmds_to_post.front().bytes),
              static_cast<unsigned long>(cmds_to_post.front().req_lptr),
              static_cast<unsigned long>(cmds_to_post.front().req_rptr));
    }
...
void Proxy::post_gpu_commands_mixed(
  ...
  // --- Intra-node: shared-memory scalar store for PUT_VALUE commands --------
  if (!shm_put_value_cmds.empty()) {
    for (size_t i = 0; i < shm_put_value_cmds.size(); ++i) {
      ...
      uint64_t value = static_cast<uint64_t>(cmd.bytes_and_val) |
      ...
      if (std::getenv("UCCL_PROXY_TRACE")) {
        ...
        fprintf(stderr,
                "[ProxyTrace] rank=%d thread=%d SHM_PUT_VALUE dst_rank=%d "
  ...
  if (!put_value_wrs.empty()) {
    ...
    for (size_t i = 0; i < put_value_cmds.size(); ++i) {
      ...
      sges[i].addr = reinterpret_cast<uintptr_t>(&values[i]);
      sges[i].length = bytes;
      sges[i].lkey = 0;

      wrs[i].wr_id = put_value_wrs[i];
      ...
      if (std::getenv("UCCL_PROXY_TRACE")) {
        fprintf(stderr,
                ...
                cfg_.rank, cfg_.thread_idx, dst_rank,
                (unsigned long long)remote_addr, bytes,
...
void Proxy::quiet(std::vector<uint64_t> wrs, std::vector<TransferCmd> cmds) {
  assert(wrs.size() == cmds.size() && "quiet wr/cmd size mismatch");
  quiet_cq();
  for (auto wr : wrs) acked_wrs_.insert(wr);
}
...
void Proxy::destroy(bool free_gpu_buffer) {
  ...
  // Release our reference to shared RDMA resources (context/pd/mr).
  // If we're not the last holder, this nulls the pointers so the
  // dereg/dealloc/close below become no-ops.
