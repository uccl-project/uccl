...
UcclProxy::UcclProxy(int thread_idx, uintptr_t gpu_buffer_addr,
  ...
  for (size_t i = 0; i < kChannelPerProxy; ++i) {
    ...
    d2h_channel_host_addrs_.push_back(addr);
    uintptr_t device_addr = get_cmd_ring_device_addr(addr);
#endif
...
#else
    d2h_channel_addrs_.push_back(device_addr);
#endif
  }
...
UcclProxy::~UcclProxy() {
  ...
  for (auto d2h_channel_addr : d2h_channel_host_addrs_) {
    free_cmd_ring(d2h_channel_addr);
  }
  d2h_channel_host_addrs_.clear();
#endif
