...
template <bool kUseAggressiveAtomic, typename dtype_t>
...
#elif __CUDA_ARCH__
  // sm_89 fallback: no TMA bulk ops
  EP_DEVICE_ASSERT(false && "tma_store_wait requires sm_90+");
#else
  EP_DEVICE_ASSERT(false);
#endif
}
...
__device__ __forceinline__ void mbarrier_arrive_and_expect_tx(
  ...
  asm volatile(
      ...
      "r"(mbar_int_ptr));
#elif __CUDA_ARCH__
  EP_DEVICE_ASSERT(false && "mbarrier_arrive_and_expect_tx requires sm_90+");
#else
  EP_DEVICE_ASSERT(false);
#endif
}
...
#elif __CUDA_ARCH__
  EP_DEVICE_ASSERT(false && "mbarrier_wait requires sm_90+");
#else
  EP_DEVICE_ASSERT(false);
#endif
}
...
#elif __CUDA_ARCH__
  EP_DEVICE_ASSERT(false && "tma_store_1d requires sm_90+");
#else
  EP_DEVICE_ASSERT(false);
#endif
}
...
__device__ __forceinline__ void tma_load_1d(void const* smem_ptr,
  ...
  asm volatile(
      ...
      : "memory");
#elif __CUDA_ARCH__
  EP_DEVICE_ASSERT(false && "tma_load_1d requires sm_90+");
#else
  EP_DEVICE_ASSERT(false);
#endif
}
...
__device__ __forceinline__ void mbarrier_init(uint64_t* mbar_ptr,
  ...
  asm volatile("mbarrier.init.shared::cta.b64 [%1], %0;" ::"r"(arrive_count),
               "r"(mbar_int_ptr));
#elif __CUDA_ARCH__
  EP_DEVICE_ASSERT(false && "mbarrier_init requires sm_90+");
#else
  EP_DEVICE_ASSERT(false);
#endif
}
...
__device__ __forceinline__ uint32_t elect_one_sync(int lane_id) {
  ...
  asm volatile(
      ...
      : "r"(WARP_MASK));
#else
  // sm_89 fallback: lane 0 is the elected leader
  pred = (lane_id == 0) ? 1 : 0;
#endif
  return pred;
}
...
#if defined(__HIP_PLATFORM_AMD__) || defined(__HIPCC__)
  __hip_atomic_store(const_cast<int*>(ptr), val, __ATOMIC_RELAXED,
                     __HIP_MEMORY_SCOPE_SYSTEM);
#else
