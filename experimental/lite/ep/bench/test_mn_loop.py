"""Multi-node test with multiple iterations using working test_internode_simple pattern"""
import torch
...
def main(rank, num_ranks, group):
    ...
    print(f"[rank {rank}] dispatch_recv_data_offset set", flush=True)
    ...
    cumulative = torch.zeros((num_local_experts,), dtype=torch.int, device=f"cuda:{device_index}")
    ...
    time.sleep(2)  # Let straggling RDMA ops complete
...
if __name__ == "__main__":
    ib = detect_ib_hca()
    if ib and ib.startswith("mlx"):
        os.environ["NCCL_IB_HCA"] = ib
    local_rank = int(os.environ["LOCAL_RANK"])
    num_local_ranks = int(os.environ["LOCAL_WORLD_SIZE"])
    rank, num_ranks, group = init_dist(local_rank, num_local_ranks)
    try:
        main(rank, num_ranks, group)
    except:
        import traceback
        traceback.print_exc()
