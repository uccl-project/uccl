...
def get_cpu_proxies_meta(proxies, rank, scratch_ptr, scratch_bytes, num_ranks, group):
    ...
    meta = {
        "rank": rank,
        "ptr": scratch_ptr.data_ptr() if hasattr(scratch_ptr, 'data_ptr') else int(scratch_ptr),
...
def check_nvlink_connections(group: dist.ProcessGroup):
    ...
    In PCIE_INTRANODE mode (no NVLink), this is a no-op since all communication
    goes through RDMA or host-staged PCIe paths.

    Arguments:
        group: the communication group.
    """
    # For L4 and other GPUs without NVLink, skip the check entirely.
    # NUM_MAX_NVL_PEERS=1 means every GPU is treated as its own NVLink group,
    # so all peers are RDMA peers and NVLink is not required.
    device_name = torch.cuda.get_device_name()
    no_nvlink_gpus = ["L4", "L40", "T4", "A10", "RTX"]
    if any(gpu in device_name for gpu in no_nvlink_gpus):
        rank = dist.get_rank(group)
        if rank == 0:
            print(f"[DeepEP-Lite] Skipping NVLink check for {device_name} "
                  f"(PCIe-only mode, NUM_MAX_NVL_PEERS=1)", flush=True)
        return
...
def initialize_uccl(
    ...
    # When compiled with PCIE_INTRANODE (NUM_MAX_NVL_PEERS=1), treat each
    # GPU as its own NVL node regardless of actual LOCAL_WORLD_SIZE.
    pcie_intranode = (ep.get_num_max_nvl_peers() == 1)
    if pcie_intranode:
        effective_nproc = 1
    else:
        effective_nproc = nproc_per_node

    node_idx = rank // effective_nproc if effective_nproc > 0 else 0

    # Only check WORLD_SIZE consistency if it's defined
    if "WORLD_SIZE" in os.environ and effective_nproc > 0:
    ...
    for i in range(ep.get_num_proxy_threads()):
        ...
        buf_addr = scratch_ptr.data_ptr() if hasattr(scratch_ptr, 'data_ptr') else int(scratch_ptr)
        proxy = ep.Proxy(
            thread_idx=i,
