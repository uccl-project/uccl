...
def test_simple_internode(rank: int, num_ranks: int, group: dist.ProcessGroup):
    ...
    try:
        # Buffer handles its own proxy/RDMA buffer allocation internally
        buffer = Buffer(
            group=group,
            num_nvl_bytes=0,
    ...
    # Reset CUDA device to clear any lingering errors from EP kernels
    # before NCCL operations
...
def test_worker(local_rank: int, num_local_ranks: int):
    ...
    try:
        ...
        # Avoid NCCL barriers after EP kernel errors
        try:
