...
class Bootstrap {
 public:
  ...
  ///
  /// Data sent via `send(senderBuff, size, receiverRank, tag)` can be received
  /// via `recv(receiverBuff, size, senderRank, tag)`. Multiple calls to send()
  /// with the same @p peer and @p tag will be ordered by the order of calls,
  /// corresponding to the order of recv() calls on the receiving side. In cases
  /// where the execution order of multiple send()s or recv()s between two ranks
  /// is unknown, they should be differentiated by using different @p tag values
  ...
  ///
  /// When called by rank `r`, this sends data from `allData[r * size]` to
  /// `allData[(r + 1) * size - 1]` to all other ranks. The data sent by rank
  /// `r` is received into `allData[r * size]` of other ranks.
  ///
  ...
  void groupBarrier(std::vector<int> const& ranks);

  /// Wrapper of send() that sends a vector of characters.
  /// @param data The data to send.
  /// @param peer The rank of the process to send the data to.
  /// @param tag The tag to send the data with.
  void send(std::vector<char> const& data, int peer, int tag);
...
class TcpBootstrap : public Bootstrap {
 public:
  ...
  /// Initialize the TcpBootstrap with a given unique ID. The unique ID can be
  /// generated by any method; it can be created by createUniqueId() or can be
  /// any arbitrary bit array provided by the user.
  ...
  /// Initialize the TcpBootstrap with a string formatted as "ip:port" or
  /// "interface:ip:port".
  /// @param ifIpPortTrio The string formatted as "ip:port" or
  /// "interface:ip:port".
  /// @param timeoutSec The connection timeout in seconds.
  void initialize(std::string const& ifIpPortTrio, int64_t timeoutSec = 30);
  ...
  /// Broadcast data from the root process to all processes using a ring-based
  /// algorithm.
  ///
  /// When called by the root rank, this sends the `size` bytes starting at
  /// memory location `data` to all other ranks. Non-root ranks receive these
  /// bytes into their own `data` buffer, overwriting its previous contents. The
  /// data propagates sequentially through a logical ring of processes until all
  /// ranks have received it.
  ///
...
static_assert(TransportFlagsSize ==
                  static_cast<size_t>(Transport::NumTransports),
...
class TransportFlags : private detail::TransportFlagsBase {
 public:
  ...
  /// @param other The other TransportFlags to compare with.
  /// @return True if the two TransportFlags objects are not equal, false
  /// otherwise.
  bool operator!=(TransportFlags other) const;

  /// Convert the TransportFlags object to a bitset representation.
  /// @return A detail::TransportFlagsBase object representing the
  /// TransportFlags object.
...
struct EndpointConfig {
  /// InfiniBand-specific configuration options that control queue pair behavior
  /// and performance characteristics. These settings are only used when the
  /// transport is an InfiniBand type (IB0-IB7); they are ignored for other
  /// transports.
  struct Ib {
    /// IB mode for signaling, used to select between different implementations.
    enum class Mode {
      Default,  // Use the MSCCLPP_IBV_MODE environment variable (or "host" if
                // unset).
      Host,     // Use the host stack with RDMA atomics.
      HostNoAtomic  // Use the host stack with write-with-immediate signaling
                    // (no RDMA atomics).
    };
    ...
    /// Device index. Currently ignored; use transport type (IB0-IB7) to select
    /// device.
    int deviceIndex;
    /// Port number.
    int port;
    ...
    int maxSendWr;
    /// Maximum number of outstanding receive work requests (used in
    /// HostNoAtomic mode for write-with-immediate).
    int maxRecvWr;
    /// Maximum number of work requests per send operation.
    int maxWrPerSend;
    /// IB mode for signaling. When set to Default, uses the MSCCLPP_IBV_MODE
    ...
    /// @param maxRecvWr Maximum outstanding receive work requests (for
    /// HostNoAtomic mode).
    /// @param maxWrPerSend Maximum work requests per send operation.
    /// @param mode IB mode for signaling (Default uses MSCCLPP_IBV_MODE env
    /// variable).
    Ib(int deviceIndex = -1, int port = DefaultPort,
       int gidIndex = DefaultGidIndex, int maxCqSize = DefaultMaxCqSize,
       int maxCqPollNum = DefaultMaxCqPollNum, int maxSendWr = DefaultMaxSendWr,
       int maxRecvWr = DefaultMaxRecvWr, int maxWrPerSend = DefaultMaxWrPerSend,
  ...
  /// Constructs endpoint configuration with specified transport, device, and
  /// optional settings.
  ...
  EndpointConfig(Transport transport = Transport::Unknown,
                 Device device = DeviceType::GPU, int maxWriteQueueSize = -1,
                 Ib ib = {})
      : transport(transport),
        device(device),
...
class Endpoint {
 public:
  ...
  EndpointConfig const& config() const;
  ...
  /// @return A vector of characters representing the serialized Endpoint
  /// object.
  std::vector<char> serialize() const;

  /// Deserialize an Endpoint object from a vector of characters.
  /// @param data A vector of characters representing a serialized Endpoint
  /// object.
  /// @return A deserialized Endpoint object.
  static Endpoint deserialize(std::vector<char> const& data);
...
/// Context for communication. This provides a low-level interface for forming
/// connections in use-cases where the process group abstraction offered by
/// Communicator is not suitable, e.g., ephemeral client-server connections.
/// Correct use of this class requires external synchronization when finalizing
/// connections with the connect() method.
///
/// As an example, a client-server scenario where the server will write to the
/// client might proceed as follows:
///   1. The client creates an endpoint with createEndpoint() and sends it to
///   the server.
///   2. The server receives the client endpoint, creates its own endpoint with
///   createEndpoint(), sends it to the
///      client, and creates a connection with connect().
///   3. The client receives the server endpoint, creates a connection with
///   connect() and sends a
///      RegisteredMemory to the server.
///   4. The server receives the RegisteredMemory and writes to it using the
///   previously created connection.
/// The client waiting to create a connection before sending the
/// RegisteredMemory ensures that the server cannot write to the
/// RegisteredMemory before the connection is established.
///
/// While some transports may have more relaxed implementation behavior, this
...
class Context : public std::enable_shared_from_this<Context> {
 public:
  ...
  /// @return A RegisteredMemory object representing the registered memory
  /// region.
  RegisteredMemory registerMemory(void* ptr, size_t size,
  ...
  /// Establish a connection between two endpoints. While this method
  /// immediately returns a connection object, the connection is only safe to
  /// use after the corresponding connection on the remote endpoint has been
  /// established. This method must be called on both endpoints to establish a
  /// connection.
  ///
  /// @param localEndpoint The local endpoint.
  /// @param remoteEndpoint The remote endpoint.
  /// @return A connection object.
  Connection connect(Endpoint const& localEndpoint,
...
/// RegisteredMemory does not own the memory it points to, but it provides a way
/// to transfer metadata about the memory to other processes, hence allowing
/// their access to the memory block.
class RegisteredMemory {
 public:
  /// Constructor.
  ...
  /// @return A vector of characters representing the serialized
  /// RegisteredMemory object.
  std::vector<char> serialize() const;

  /// Deserialize a RegisteredMemory object from a vector of characters.
  /// @param data A vector of characters representing a serialized
  /// RegisteredMemory object.
...
class Connection {
 public:
  ...
  /// Write data from a source RegisteredMemory to a destination
  /// RegisteredMemory.
  ///
  /// @param dst The destination RegisteredMemory.
  /// @param dstOffset The offset in bytes from the start of the destination
  /// RegisteredMemory.
  /// @param src The source RegisteredMemory.
  /// @param srcOffset The offset in bytes from the start of the source
  /// RegisteredMemory.
  /// @param size The number of bytes to write.
  void write(RegisteredMemory dst, uint64_t dstOffset, RegisteredMemory src,
             uint64_t srcOffset, uint64_t size);

  /// Update an 8-byte value in a destination RegisteredMemory and synchronize
  /// the change with the remote process.
  ///
  /// @param dst The destination RegisteredMemory.
  /// @param dstOffset The offset in bytes from the start of the destination
  /// RegisteredMemory.
...
/// SemaphoreStub object only used for constructing Semaphore, not for direct
/// use by the user.
class SemaphoreStub {
 public:
  /// Constructor.
  /// @param connection The connection associated with this semaphore.
  SemaphoreStub(Connection const& connection);

  /// Get the memory associated with this semaphore.
  /// @return A reference to the registered memory for this semaphore.
  RegisteredMemory const& memory() const;

  /// Serialize into a vector of characters.
  /// @return A vector of characters representing the serialized SemaphoreStub
  /// object.
  std::vector<char> serialize() const;

  /// Deserialize a SemaphoreStub object from a vector of characters.
  /// @param data A vector of characters representing a serialized SemaphoreStub
  /// object.
  /// @return A deserialized SemaphoreStub object.
  static SemaphoreStub deserialize(std::vector<char> const& data);
...
class Semaphore {
 public:
  ...
  Semaphore(SemaphoreStub const& localStub, SemaphoreStub const& remoteStub);
...
/// A class that sets up all registered memories and connections between
/// processes.
///
/// A typical way to use this class:
///   1. Call connect() to declare connections between the calling process and
///   other processes.
///   2. Call registerMemory() to register memory regions that will be used for
///   communication.
///   3. Call sendMemory() or recvMemory() to send/receive registered memory
///   regions to/from
///      other processes.
///   4. Call get() on futures returned by connect(). Use the returned
///   connections to create flags.
///   5. Call buildSemaphore() to create a Semaphore out of the flags.
///   6. Call get() on all futures returned by buildSemaphore() and
///   recvMemory().
///   7. All done; use semaphores and registered memories to build channels.
///
/// CAUTION: The order of method calls matters when the same remote rank and
/// tags are used. That is, the i-th "sending" method call (sendMemory(),
/// connect(), and buildSemaphore()) on the local rank must be matched by the
/// i-th "receiving" method call (recvMemory(), connect(), and buildSemaphore())
/// on the remote rank.
///
/// Correct Example 1:
/// ```cpp
...
/// auto mem0 = communicator.recvMemory(0, tag);
/// auto connection = communicator.connect(Transport::CudaIpc, 0, tag); //
/// undefined behavior communicator.sendMemory(memory1, 0, tag);
/// ```
/// In the wrong example, the connection information from rank 1 will be sent to
/// the `mem1` object on rank 0, where the object type is RegisteredMemory, not
/// Connection.
///
class Communicator {
 public:
  ...
  ///
  /// @param bootstrap An implementation of the Bootstrap that the communicator
  /// will use.
  /// @param context An optional context to use for the communicator. If not
  /// provided, a new context will be created.
  ...
  ///
  /// The send will be started upon calling this function, but this function
  /// returns immediately without waiting for the completion of the send.
  /// RegisteredMemory sent via `sendMemory(memory, remoteRank, tag)` can be
  /// received via `recvMemory(remoteRank, tag)`.
  ///
  /// Multiple calls to either sendMemory() or connect() with the same @p
  /// remoteRank and @p tag will be ordered by the order of calls, corresponding
  /// to the order of recvMemory() or connect() calls on the receiving side. In
  /// cases where the execution order is unknown between two ranks, they should
  /// be differentiated by using different @p tag values to prevent unexpected
  /// behavior.
  ///
  ...
  [[deprecated(
      "Use sendMemory() instead. This will be removed in a future "
      "release.")]] void
  sendMemoryOnSetup(RegisteredMemory memory, int remoteRank, int tag) {
    sendMemory(memory, remoteRank, tag);
  }

  /// Receive memory information from a corresponding sendMemory call on the
  /// remote side.
  ///
  /// This function returns a future immediately. The actual receive will be
  /// performed upon calling the first get() on the future. RegisteredMemory
  /// sent via `sendMemory(memory, remoteRank, tag)` can be received via
  /// `recvMemory(remoteRank, tag)`.
  ///
  /// Multiple calls to either sendMemory() or connect() with the same @p
  /// remoteRank and @p tag will be ordered by the order of calls, corresponding
  /// to the order of recvMemory() or connect() calls on the receiving side. In
  ...
  /// behavior.
  ///
  /// @note To guarantee the receiving order, calling get() on a future returned
  /// by recvMemory() or connect() may start receiving other RegisteredMemory or
  /// Connection objects of which futures were returned by an earlier call to
  /// recvMemory() or connect() with the same @p remoteRank and @p tag. For
  /// example, if we call recvMemory() or connect() five times with the same @p
  /// remoteRank and @p tag and then call get() on the last future, it will
  /// start receiving the five RegisteredMemory or Connection objects in order,
  /// back to back.
  ///
  /// @param remoteRank The rank of the remote process.
  ...
  [[deprecated(
      "Use recvMemory() instead. This will be removed in a future "
      "release.")]] NonblockingFuture<RegisteredMemory>
  ...
  ///
  /// This function will start (but not wait for) sending metadata about the
  /// local endpoint to the remote rank, and return a future connection without
  /// waiting for the remote rank to respond. The connection will be established
  /// when the remote rank responds with its own endpoint and the local rank
  /// calls the first get() on the future. Note that this function is two-way
  /// and a connection from rank `i` to remote rank `j` needs to have a
  /// counterpart from rank `j` to rank `i`. Note that with IB, buffers are
  /// registered at a page level and if a buffer is spread through multiple
  /// pages and does not fully utilize all of them, IB's QP has to register for
  /// all involved pages. This potentially has security risks if the
  /// connection's accesses are given to a malicious process.
  ///
  /// Multiple calls to either sendMemory() or connect() with the same @p
  /// remoteRank and @p tag will be ordered by the order of calls, corresponding
  /// to the order of recvMemory() or connect() calls on the receiving side. In
  ...
  ///
  std::shared_future<Connection> connect(Endpoint const& localEndpoint,
                                         int remoteRank, int tag = 0);

  /// Connect to a remote rank. Wrapper of `connect(localEndpoint, remoteRank,
  /// tag)`.
  /// @param localConfig The configuration for the local endpoint.
  /// @param remoteRank The rank of the remote process.
  /// @param tag The tag to use for identifying the send and receive.
  /// @return A future of the connection.
  std::shared_future<Connection> connect(EndpointConfig const& localConfig,
                                         int remoteRank, int tag = 0);
  ...
  std::shared_future<Semaphore> buildSemaphore(Connection const& connection,
                                               int remoteRank, int tag = 0);
  ...
  int remoteRankOf(Connection const& connection);
  ...
  int tagOf(Connection const& connection);

  [[deprecated(
      "setup() is now no-op and no longer needed. This will be removed in a "
      "future release.")]] void
  setup() {}
