...
void test_uds_oob() {
  ...
  std::thread rank0([&]() {
    ...
    std::cout << "[INFO] rank0 received IpcCacheWire"
              << " seq=" << seq << " is_send=" << (int)got.is_send
              << " offset=" << got.offset << " size=" << got.size << "\n";
    ...
    // handle is opaque; here we just sanity-check that it matches the pattern
    // we set
  ...
  std::thread rank1([&]() {
    ...
    // Fill recognizable pattern into handle bytes (no CUDA call needed for unit
    // test)
