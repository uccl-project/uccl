...
static inline double time_diff_us(const timespec& beg, const timespec& end) {
    const double s  = (double)(end.tv_sec  - beg.tv_sec);
    const double ns = (double)(end.tv_nsec - beg.tv_nsec);
    return s * 1e6 + ns / 1e3;
}
...
static void print_usage(const char* path) {
    std::cout
        ...
        << "  -C            Use cold cache mode (only meaningful for caching mapping)\n";
}
...
*/
void bench_nvidia_gpu_gdrcopy(int dev_id,
    size_t req_size,
    int num_write_iters,
    int num_read_iters,
    bool do_cumemcpy,
    ...
    for (int n = 0; n < ndev; ++n) {
        ...
        CK(cuDeviceGetAttribute(&dom,  CU_DEVICE_ATTRIBUTE_PCI_DOMAIN_ID, dev), "pci domain");
        CK(cuDeviceGetAttribute(&bus,  CU_DEVICE_ATTRIBUTE_PCI_BUS_ID,    dev), "pci bus");
        CK(cuDeviceGetAttribute(&devfn,CU_DEVICE_ATTRIBUTE_PCI_DEVICE_ID, dev), "pci device");
        std::cout << "  GPU id:" << n << "; name:" << name
                  << "; Bus:" << std::hex << std::setfill('0')
                  << std::setw(4) << dom << ":"
                  << std::setw(2) << bus << ":"
                  << std::setw(2) << devfn
                  << std::dec << "\n";
    ...
    std::cout << "Selected GPU " << dev_id << ", allocated " << size << " bytes, dptr=0x"
              << std::hex << (uint64_t)d_A << std::dec << "\n";
    ...
    if (rc != 0) {
        std::fprintf(stderr, "ERROR: gdr_pin_buffer failed rc=%d (need root? or driver mismatch?)\n", rc);
        std::exit(1);
    }
    ...
    const int off = (int)((uint64_t)info.va - (uint64_t)d_A);
    ...
    if (use_cold_cache && info.mapping_type != GDR_MAPPING_TYPE_CACHING) {
        std::fprintf(stderr, "ERROR: -C only meaningful for caching mapping (e.g., Grace-Hopper cache mapping)\n");
        std::exit(1);
    }

    std::cout << "\n[gdr_copy_to_mapping] write iters=" << num_write_iters << "\n";
    std::cout << "WARNING: measures CPU-side API invocation latency; may not imply full GPU visibility ordering.\n";
    std::printf("Test\t\t\t\tSize(B)\t\tAvg.Time(us)\n");
...
void bench_amd_gpu_gdrcopy(int dev_id,
    size_t req_size,
    int num_write_iters,
    int num_read_iters,
    bool do_hipmemcpy,
    bool use_cold_cache) {
#ifdef ENABLE_HIP
    auto HK = [](hipError_t e, const char* msg) {
        if (e != hipSuccess) {
    ...
    uint32_t* init_buf = nullptr;
    uint32_t* h_buf    = nullptr;
    HK(hipHostMalloc((void**)&init_buf, size, hipHostMallocDefault), "hipHostMalloc(init_buf)");
    ...
    if (do_hipmemcpy) {
        ...
        for (size_t copy_size = 1; copy_size <= size; copy_size <<= 1) {
            ...
            clock_gettime(CLOCK_MONOTONIC, &beg);
            for (int i = 0; i < num_write_iters; ++i) {
                HK(hipMemcpy(d_A, init_buf, copy_size, hipMemcpyHostToDevice), "hipMemcpy H2D");
            }
            clock_gettime(CLOCK_MONOTONIC, &end);
            double lat = time_diff_us(beg, end) / (double)num_write_iters;
        ...
        for (size_t copy_size = 1; copy_size <= size; copy_size <<= 1) {
            ...
            clock_gettime(CLOCK_MONOTONIC, &beg);
            for (int i = 0; i < num_read_iters; ++i) {
                HK(hipMemcpy(h_buf, d_A, copy_size, hipMemcpyDeviceToHost), "hipMemcpy D2H");
            }
            clock_gettime(CLOCK_MONOTONIC, &end);
            double lat = time_diff_us(beg, end) / (double)num_read_iters;
    ...
    std::cout << "\n[CPU memcpy -> hipMalloc(device)] write iters=" << num_write_iters << "\n";
    std::cout << "WARNING: measures CPU-side memcpy into device memory (host zero-copy access).\n";
    std::printf("Test\t\t\t\tSize(B)\t\tAvg.Time(us)\n");
    ...
    std::cout << "\n[CPU memcpy <- hipMalloc(device)] read iters=" << num_read_iters << "\n";
    std::printf("Test\t\t\t\tSize(B)\t\tAvg.Time(us)\n");
...
int main(int argc, char** argv) {
    ...
    while ((opt = getopt(argc, argv, "hd:s:w:r:cC")) != -1) {
        switch (opt) {
            ...
            case 'd': dev_id = std::strtol(optarg, nullptr, 0); break;
            case 's': size   = (size_t)std::strtoull(optarg, nullptr, 0); break;
            case 'w': w      = std::strtol(optarg, nullptr, 0); break;
            case 'r': r      = std::strtol(optarg, nullptr, 0); break;
            case 'c': do_cumemcpy = true; break;
            case 'C': cold = true; break;
            default:
                print_usage(argv[0]);
                return 1;
        }
