...
│   ├── uccl_gin_rail.cuh      # rail_put / rail_put_tail_add / rail_red_add / rail_write_value
...
struct UCCLGinResources {
    ...
    uint64_t window_bytes;          // registered window 大小（device 端边界检查）
...
内部逻辑：
1. device 端写 `TransferCmd{WRITE_VALUE, value=...}` 到 D2H ring。
2. CPU proxy 收到后，把 `cmd.value` 拷到按 `(ring, slot)` 索引的 host bounce slot。
3. proxy 从这个注册过的 host MR 发 4-byte EFA WRITE 到远端 `sym_ptr`。

这样避免了旧 GPU staging slot 的生命周期问题：D2H slot 没有 CQE ack 前不会复用，
host bounce slot 也按同一个 ring slot 生命周期绑定。
...
写一条 `QUIET` cmd 到 lane 的 D2H ring，自旋等待 proxy ack。当前实现会先 drain
该 proxy thread 上已 post WRITE/WRITE_VALUE 的 send CQE，再 ack quiet，因此 source
buffer 可安全复用。

注意：ordering 只在同一个 lane_hint / D2H queue 内成立。跨 lane 的 `put -> red_add_rel`
需要显式 `quiet` 或更高层协议保证。
...
依次对本 context 的所有 D2H queue 发 `quiet`。这比 `quiet(lane_hint)` 更重，但语义不再
假装只 drain lane 0 就覆盖整个 context。
...
- **`flush(coop)`**：NCCL 的 cooperative flush 是"组内一次 flush"（选举 + 组同步）。
  standalone 没有 coop API，按线程退化会变成 N 倍静默 drain，所以留 `static_assert`
  编译期报错；DeepEP 集成时再实现选举版本。
- **`WRITE_VALUE` 只在 EFA normal-mode 实现**：fast-mode 和非 EFA
  （SOFTWARE_ORDERING / 其他）路径会把 union 里的 inline value 误解码为本地
  offset，这些路径现在遇到 WRITE_VALUE 直接 abort（loud guard）。

## 4. 底层 rail 操作

`uccl_gin/uccl_gin_rail.cuh` 提供四个不依赖 handle 的裸函数。handle 调它们，DeepEP
...
| `rail_write_value(dst, value, roff)` | WRITE_VALUE | — | — | host bounce → 4-byte EFA WRITE |
...
- **WRITE_VALUE cmd** (`cmd_type = WRITE_VALUE`):
  - `value` = inline 32-bit payload
  - `bytes` = 4
  - `req_rptr` = 远端 window offset，4-byte 粒度
  - proxy 使用 host bounce MR 发 4-byte WRITE；不读取 GPU staging buffer。
...
Context cfg(rank, world_size, local_world_size, max_message_bytes, ifname)
  ...
  → proxy 分配并注册 WRITE_VALUE host bounce slots
...
| UCCL-put_value | put_value + red_add_rel | WRITE_VALUE inline staging + completion |
