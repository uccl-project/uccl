...
## 2026-06-12（续）：Phase 2 完成 —— 2 节点 put/quiet 在 RoCE 上 PASS ✅✅

### 新增代码（已 commit/push）
- `tests/put_quiet_smoke.cu`：`smoke_put_quiet` kernel（`gin.put<Rail>` +
  `gin.quiet`）+ `run_put_quiet_smoke(ctx, peer, bytes)` launcher
  （fill pattern → poison recv → put → quiet → MPI_Barrier → 逐字节校验 peer
  pattern）。裸 cuda* 由 hipify 翻译。
- `bindings.cpp`：`Context.put_quiet_smoke(peer, bytes) -> bool`。
- `setup.py`：sources 加 `tests/put_quiet_smoke.cu`。
- `python/uccl_gin/put_quiet_smoke.py`：2 节点 driver（paired-remote，size sweep）。

### 编译期修的坑
- `__nanosleep` 在 HIP host pass 下未定义（amd_nanosleep.cuh 只在 device pass
  include，但 HIP host pass 也 parse `__device__` 函数体）→ platform.cuh 加
  host-pass no-op stub。

### 运行期修的坑
- **import 顺序**：`_uccl_gin.so` 链接 libtorch，必须先 `import torch` 再 import
  扩展，否则 2 节点 mpirun 下 PyInit 段错误。`__init__.py` 加 `import torch`。
  （单 rank 偶然没崩是因为手测时先 import 了 torch。）

### 运行配置（记录，供复现）
```bash
cd ~/nfs/danyang/uccl-danyang/experimental/uccl_gin
VENV=~/nfs/danyang/.venv-uccl-gin
export LD_LIBRARY_PATH=/opt/cluster-test/lib:/opt/rocm/lib:$VENV/lib/python3.12/site-packages/torch/lib
export PYTHONPATH=$PWD/python LOCAL_WORLD_SIZE=1 NCCL_SOCKET_IFNAME=enp49s0f1np1
export UCCL_GIN_SMOKE_SIZES=1024,65536,1048576
/opt/cluster-test/bin/mpirun --host chi-mi325x-pod2-098:1,chi-mi325x-pod2-099:1 \
  -np 2 --map-by node --bind-to none --mca pml ob1 \
  -x LD_LIBRARY_PATH -x PYTHONPATH -x LOCAL_WORLD_SIZE -x NCCL_SOCKET_IFNAME -x UCCL_GIN_SMOKE_SIZES \
  $VENV/bin/python -m uccl_gin.put_quiet_smoke
```
- bootstrap iface `enp49s0f1np1`（=mlx5_1 RoCE netdev，10.162.224.x）。
- `--mca pml ob1` 避开 UCX（不支持 MPI_THREAD_MULTIPLE）。
- `--bind-to none` 避开 prte 的 "allocate mask for invalid number"。

### 结果
```
QP modified to RTR/RTS state ...（RC 建链成功）
put/quiet      1024 B: PASS
put/quiet     65536 B: PASS
put/quiet   1048576 B: PASS
uccl_gin put/quiet smoke PASS world=2 local_world=1 peer_of_0=1   EXIT=0
```
**UCCL-GIN 的 put + quiet 在 AMD MI325X / ROCm 6.4 / RoCE 上跨节点跑通，1KB–1MB 全
PASS。** 走的是非 EFA 标准 RC `ibv_post_send` WRITE 路径，与 ep 的 AMD 路径同源。

### 仍未做（按需推进）
- **native-atomic 后端（计划 Phase 4）**：`red_add_rel`/`put_tail_add`/`put_value`
