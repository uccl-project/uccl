...
RDMAContext::RDMAContext(PeerID peer_id, TimerManager *rto, uint32_t *engine_unacked_bytes,
    ...
    {
        struct ibv_sge sge;
        for (int i = 0; i < (eqds::CreditChunkBuffPool::kNumChunk - 1) / 2;
             i++) {
            uint64_t chunk_addr;
    ...
    if constexpr (!kRCMode) {
        ...
        void *retr_mr_addr = mmap(
            nullptr, kRetrMRSize,
            PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
        if (retr_mr_addr == MAP_FAILED)
            throw std::runtime_error("mmap failed");
        retr_mr_ = ibv_reg_mr(
            pd_, retr_mr_addr, kRetrMRSize,
            IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
        if (retr_mr_ == nullptr)
        ...
        for (int i = 0; i < kMaxBatchCQ; i++) {
            ...
            retr_wrs_[i].next =
                (i == kMaxBatchCQ - 1) ? nullptr : &retr_wrs_[i + 1];
        }
    ...
    for (int i = 0; i < kPostRQThreshold; i++) {
        ...
        imm_wrs_[i].next =
            (i == kPostRQThreshold - 1) ? nullptr : &imm_wrs_[i + 1];
...
bool RDMAContext::senderCC_tx_message(struct ucclRequest *ureq) {
    ...
    while (*sent_offset < size || size == 0 /* zero-length message */ ) {
...

        {
            ...
            // Enforce global cwnd.
            queued = EventOnQueueData(subflow, wr_ex, chunk_size + hdr_overhead,
                                      now);
...
int RDMAContext::receiver_poll_rc_cq(void) {
    ...
    while (1) {
        ...
        if (++cq_budget == kMaxBatchCQ || ibv_next_poll(cq_ex))
            break;
    }
...
int RDMAContext::sender_poll_uc_cq(void) {
    ...
    if (likely(pending_signal_poll_ == 0))
        return 0;
...
bool RDMAContext::try_retransmit_chunk(SubUcclFlow *subflow,
    ...
    if (!EventOnTxRTXData(subflow, wr_ex))
        return false;
    ...
    if (retr_hdr_pool_->alloc_buff(&retr_hdr))
        return false;
        ...
        if (subflow->pcb.duplicate_acks < kFastRexmitDupAckThres) {
            ...
            while (sack_bitmap_count && index < kSackBitmapSize &&
                ...
                if ((sack_bitmap & (1ULL << cursor)) == 0) {
                    ...
                    auto chunk =
                        subflow->txtracking.get_unacked_chunk_from_idx(index);
                    if (seqno == chunk.csn) {
            ...
            if (cq_ex->status == IBV_WC_SUCCESS) {
                ...
                if (ibv_wc_read_opcode(cq_ex) == IBV_WC_RECV) {
                    ...
                    for (int i = 0; i < num_ack; i++) {
                        auto pkt_addr =
                            chunk_addr + i * CtrlChunkBuffPool::kPktSize;
                        rx_ack(pkt_addr);
                    }
                    post_ctrl_rq_cnt_++;
                }
...
void RDMAContext::burst_timing_wheel(void) {
    ...
    auto num_chunks =
        std::min(kMaxBurstTW, (uint32_t)wheel->ready_queue_.size());

    for (auto i = 0; i < num_chunks; i++) {
        struct wr_ex *wr_ex = reinterpret_cast<struct wr_ex *>(
            wheel->ready_queue_.front().sslot_);
        auto *wr = &wr_ex->wr;
...
void RDMAContext::rx_rtx_data(struct list_head *ack_list) {
    ...
    auto chunk_len =
        ibv_wc_read_byte_len(cq_ex) - sizeof(struct retr_chunk_hdr);
...
// Accept this retransmission chunk.
    ...
    memcpy(reinterpret_cast<void *>(hdr->remote_addr),
            ...
            chunk_len);
#else
    cudaMemcpy(reinterpret_cast<void *>(hdr->remote_addr),
                reinterpret_cast<void *>(chunk_addr +
                                        sizeof(struct retr_chunk_hdr)),
...
void RDMAContext::__retransmit_for_flow(void *context, bool rto) {
    ...
    if (subflow->pcb.rto_rexmits_consectutive >= kRTOAbortThreshold) {
        LOG_FIRST_N(ERROR, 1)
            << "RTO retransmission threshold reached." << subflow->fid_;
    }
    ...
    for (int fid = 0; fid < flow_cnt_; fid++) {
        {
            ...
            if (flow) {
                ...
                stats_maxooo =
                    std::max(stats_maxooo, subflow->pcb.stats_maxooo);
