...
class UcclRDMAEngine {
  public:
    ...
    UcclRDMAEngine(int dev, int engine_id, Channel *channel, eqds::EQDS *eqds)
        : engine_idx_(engine_id), dev_(dev), channel_(channel), eqds_(eqds),
          last_periodic_tsc_(rdtsc()), last_sync_clock_tsc_(rdtsc()),
          rto_tm_(kRTOUSec),
...
class UcclFlow {
  ...
  public:
    ...
    UcclFlow(RDMAEndpoint *ep, int bootstrap_fd, int dev, PeerID peer_id,
        ...
        : ep_(ep), dev_(dev), peer_id_(peer_id), flow_id_(flow_id),
          remote_ctx_(remote_ctx), remote_ip_(remote_ip),
          remote_dev_(remote_dev), is_send_(is_send) {
        ...
        if (!is_send_) {
            ...
            UCCL_INIT_CHECK(modify_qp_rts(recv_comm_.gpu_flush_qp, 0, true) ==
                                0,
