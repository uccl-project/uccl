...
//#define MANAGED                     // Use cudaMallocManaged rather than cudaMalloc
//////////////////////////////////////////////////////////////////////
...
void exchange_qpns(const char *peer_ip, metadata *local_meta,
    ...
    if (mode == 's') {
        int listensock = sock;
        DCHECK(bind(listensock, (struct sockaddr *)&addr, sizeof(addr)) == 0) << "Failed to bind";
    ...
    VLOG(1) << "local addr=" << (uint64_t)local_meta->addr << ", local rkey=" << local_meta->rkey << ", remote addr=" << (uint64_t)remote_meta->addr << ", remote rkey=" << remote_meta->rkey;
...
void modify_qp_rtr(struct rdma_context *rdma)
{
    ...
    int attr_mask = IBV_QP_STATE | IBV_QP_PATH_MTU | IBV_QP_AV |
                    IBV_QP_DEST_QPN | IBV_QP_RQ_PSN | IBV_QP_MIN_RNR_TIMER | IBV_QP_MAX_DEST_RD_ATOMIC;
...
{
    struct ibv_qp_attr attr = {};
    int attr_mask = IBV_QP_STATE | IBV_QP_SQ_PSN | IBV_QP_TIMEOUT | IBV_QP_RETRY_CNT | IBV_QP_RNR_RETRY |
...
struct rdma_context *init_rdma(const char *server_ip) {
    struct rdma_context *rdma = (struct rdma_context *)calloc(1, sizeof(struct rdma_context));
    ...
    DCHECK(ibv_query_device(rdma->ctx, &rdma->dev_attr) == 0) << "Failed to query device";

    DCHECK(rdma->dev_attr.phys_port_cnt == 1) << "Only one port is supported";

    DCHECK(ibv_query_port(rdma->ctx, 1, &rdma->port_attr) == 0) << "Failed to query port";

    DCHECK(rdma->port_attr.state == IBV_PORT_ACTIVE) << "Port is not active";

    DCHECK(rdma->port_attr.link_layer == IBV_LINK_LAYER_ETHERNET) << "RoCE is not supported";

    DCHECK(ibv_query_gid(rdma->ctx, 1, GID_INDEX, &rdma->gid) == 0) << "Failed to query gid";
    ...
    #else
    if (cudaMallocManaged(&rdma->local_buf, OUTSTNADING_MSG * MSG_SIZE) != cudaSuccess) {
    #endif
    ...
    #ifdef MANAGED
    DCHECK(cudaMemAdvise(rdma->local_buf, OUTSTNADING_MSG * MSG_SIZE, cudaMemAdviseSetPreferredLocation, USE_GPU) == cudaSuccess) << "Failed to set preferred location";
    DCHECK(cudaMemAdvise(rdma->local_buf, OUTSTNADING_MSG * MSG_SIZE, cudaMemAdviseSetAccessedBy, USE_GPU) == cudaSuccess) << "Failed to set accessed by";
    DCHECK(cudaMemPrefetchAsync(rdma->local_buf, OUTSTNADING_MSG * MSG_SIZE, USE_GPU) == cudaSuccess) << "Failed to prefetch GPU memory";
    ...
    VLOG(1) << "RX MR: addr=" << (uint64_t)rdma->mr->addr << ", len=" << rdma->mr->length << ", lkey=" << rdma->mr->lkey << ", rkey=" << rdma->mr->rkey;
...
void create_qp(struct rdma_context *rdma) {
    ...
    for (int i = 0; i < NUM_QPS; i++) { 
        ...
        DCHECK(ibv_modify_qp(rdma->qp[i], &qp_attr, IBV_QP_STATE | IBV_QP_PKEY_INDEX |
                                            IBV_QP_PORT | IBV_QP_ACCESS_FLAGS) == 0) << "Failed to modify QP";
    }
}
...
uint32_t poll_cq(struct rdma_context *rdma, uint32_t expect_opcode)
{
    ...
    while (!force_exit) {
        DCHECK(cq_ex->status == IBV_WC_SUCCESS) << "Failed to poll CQ: " << cq_ex->status;
        DCHECK(ibv_wc_read_opcode(cq_ex) == expect_opcode) << "Unexpected opcode: " << ibv_wc_read_opcode(cq_ex);
...
int main(int argc, char *argv[]) {
    ...
    std::thread stats_thread([&]() {
        while (!force_exit) {
            ...
            // unit: Gbps
            auto rx_stats_bytes_per_sec = 8 * (rx_stats_bytes - last_rx_stats_bytes) / 1e9;
            auto tx_stats_bytes_per_sec = 8 * (tx_stats_bytes - last_tx_stats_bytes) / 1e9;
            #ifdef USE_GPU
            printf("[GPU%d <---> mlx5_%d] RX: %.2f Gbps, TX: %.2f Gbps\n", 
                USE_GPU, DEV_INDEX, rx_stats_bytes_per_sec, tx_stats_bytes_per_sec);
            #else
