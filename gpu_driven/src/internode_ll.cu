...
#pragma unroll
      ...
      if (dst_expert_idx >= 0) {
        ...
        auto const dst_p2p_ptr = ipc_base_ptrs ? 
            uccl::get_ipc_p2p_ptr(
                reinterpret_cast<void*>(rdma_recv_x + dst_offset),
                ipc_base_ptrs, rank, dst_rank, NUM_MAX_NVL_PEERS, 0) : nullptr;
...
#pragma unroll
  ...
  if (responsible_expert_idx < num_experts and sub_warp_id == 0 and
    ...
    auto const dst_p2p_ptr = ipc_base_ptrs ? 
        uccl::get_ipc_p2p_ptr(
            reinterpret_cast<void*>(rdma_recv_count + dst_expert_local_idx * num_ranks + rank),
            ipc_base_ptrs, rank, dst_rank, NUM_MAX_NVL_PEERS, 0) : nullptr;
...
#define DISPATCH_LAUNCH_CASE(hidden)                                           \
  {                                                                            \
    ...
    LAUNCH_KERNEL(&cfg, dispatch_func, packed_recv_x, packed_recv_x_scales,    \
                  ...
                  round_scale, phases, ring_addrs, num_ring_addrs, ipc_base_ptrs); \
  }                                                                            \
  break
...
#pragma unroll
  ...
  if (responsible_expert_idx < num_experts) {
    ...
    for (int token_idx = offset + sub_warp_id;
      ...
      auto const x_int4 = static_cast<int4 const*>(x) + src_idx * hidden_bf16_int4;
...
#pragma unroll
    ...
    if (sub_warp_id == 1 and lane_id == 0) {
      ...
      // Try to use IPC for intra-node atomic operations
      auto const dst_p2p_ptr_flag = ipc_base_ptrs ? 
          uccl::get_ipc_p2p_ptr(
