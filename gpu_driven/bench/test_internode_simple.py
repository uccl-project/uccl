...
def test_simple_internode(rank: int, num_ranks: int, group: dist.ProcessGroup):
    ...
    if rank == 0:
        print(f'[simple-test] Testing with {num_tokens} tokens, {hidden} hidden, {num_experts} experts', flush=True)
        print(f'[simple-test] Running on {num_ranks} ranks across {num_ranks} nodes', flush=True)
    ...
    try:
        ...
        buffer = deep_ep.Buffer(group, 0, int(1e9), low_latency_mode=True,
        ...
        cumulative_local_expert_recv_stats = torch.zeros((num_experts // num_ranks,), dtype=torch.int, device='cuda')
...
if __name__ == '__main__':
    ...
    torch.multiprocessing.spawn(test_worker, args=(args.num_processes, args), 
