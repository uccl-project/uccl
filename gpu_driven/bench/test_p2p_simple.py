...
def test_multi_gpu(num_gpus):
    ...
    if torch.cuda.device_count() < num_gpus:
        print(f"Error: Requested {num_gpus} GPUs but only {torch.cuda.device_count()} available")
        return
    ...
    for src_gpu in range(num_gpus):
        for dst_gpu in range(num_gpus):
            if src_gpu != dst_gpu:
                ...
                print(f"  GPU {src_gpu} -> GPU {dst_gpu}: {'✓ P2P available' if can_access else '✗ P2P not available'}")
...
def test_distributed():
    ...
    for i in range(world_size):
        ...
        print(f"  Rank {i}: Node {all_node_ids[i]} ({'same' if same_node else 'different'} node)")
...
def main():
    ...
    parser.add_argument("--mode", choices=["single", "multi", "distributed"], 
