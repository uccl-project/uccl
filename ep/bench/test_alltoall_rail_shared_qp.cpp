...
// - Each NIC: ONE shared SRD QP + multiple AHs (one per remote GPU)
// - Rail-optimized: GPU i on each node only communicates with GPU i on other
// nodes
//
// Uses one SRD QP per NIC with different address handlers
// for each destination, instead of one QP per destination.
...
constexpr size_t MSG_SIZE = 16 * 1024;  // 8KB per message
...
// Base NIC context - NOW WITH SHARED QP
struct BaseNicCtx {
  ...
  ibv_mr* mr = nullptr;
  uint32_t lkey = 0;
  ibv_qp* qp = nullptr;  // ONE shared SRD QP per NIC
...
struct PeerEndpoint {
  ibv_ah* ah = nullptr;  // Different AH for each destination
...
void run_gpu_thread(int gpu_id, int my_rank, int num_nodes,
  ...
  for (int nic = 0; nic < NUM_NICS_PER_GPU; nic++) {
    ...
    // CREATE ONE SHARED SRD QP PER NIC
...

    {
      ...
      printf("[Rank %d GPU %d NIC %d] Created shared SRD QP with qp_num=%u\n",
             my_rank, gpu_id, nic, base_nics[nic].qp->qp_num);
    }
  }
  ...
  for (int nic = 0; nic < NUM_NICS_PER_GPU; nic++) {
    for (int remote_rank = 0; remote_rank < num_nodes; remote_rank++) {
      ...
      local_info.qp_num = base_nics[nic].qp->qp_num;  // Use shared QP's qp_num
...

  {
    std::lock_guard<std::mutex> lock(print_mutex);
    printf(
        "[Rank %d GPU %d] Rail-optimized with SHARED QP: only communicating "
        "with GPU %d on %d remote nodes\n",
  ...
  while (total_completed < total_msgs) {
    ...
    while (total_posted < total_msgs) {
      ...
      ibv_wr_rdma_write(qpx, peers[nic][next_remote_rank].remote_rkey,
                        remote_addr);
      // Use different AH for each destination
  ...
  {
    std::lock_guard<std::mutex> lock(print_mutex);
    printf("\n=== [Rank %d GPU %d] Rail-Optimized Results (SHARED QP) ===\n",
           my_rank, gpu_id);
  ...
  for (int nic = 0; nic < NUM_NICS_PER_GPU; nic++) {
    for (int r = 0; r < num_nodes; r++) {
      ...
      // No need to destroy per-peer QP since we use shared QP
    }
    // Destroy shared QP
...
int main(int argc, char** argv) {
  if (argc != 3) {
    ...
    printf("\n*** USES ONE SHARED SRD QP PER NIC ***\n");
    return 1;
  }
  ...
  printf("=== Rail-Optimized Multi-NIC EFA All-to-All Test (SHARED QP) ===\n");
  ...
  printf("\n*** RAIL-OPTIMIZED MODE with SHARED SRD QP ***\n");
  printf("Each GPU only communicates with same GPU ID on other nodes\n");
  printf("Each NIC uses ONE shared SRD QP with different AH per destination\n");
