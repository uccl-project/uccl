...
void run_throughput_test(uint32_t num_warps, uint32_t num_proxies, uint32_t payload_size) {
    ThroughputConfig config = {
        .num_warps = num_warps,
    ...
    // Fixed mapping to avoid dead warps - ensure at least one producer per active proxy
