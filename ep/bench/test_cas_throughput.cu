 ...
 * Goal: Test GPU-CPU communication throughput with CAS-based ring buffer sharing
 * Architecture: Multiple warps share ring buffers via CAS (atomic_set_and_commit)
 *               Similar to production uccl_ibgda.cuh design
 */
...
__global__ void cas_throughput_kernel(
    DeviceToHostCmdBuffer** ring_buffers,
    uint32_t num_ring_buffers,
    TestConfig config,
    WarpMetrics* metrics,
    ...
    uint64_t test_duration_cycles = (uint64_t)config.test_duration_ms * 1980000; // ~1.98GHz
    ...
    while (!(*stop_flag)) {
        ...
        // Note: atomic_set_and_commit always succeeds eventually (it loops internally)
        // So we count cycles instead of failures
        metrics[warp_id].total_cycles += (op_end - op_start);
    }
}

// CPU proxy thread - uses head/tail batch processing like proxy.cpp
void cpu_proxy_thread(DeviceToHostCmdBuffer* ring_buffer,
                     int proxy_id,
                     volatile bool* stop_flag,
...
// Print results
void print_results(const std::vector<WarpMetrics>& metrics,
                  const std::vector<std::atomic<uint64_t>>& proxy_counts,
                  double duration_sec,
                  TestConfig config) {
    uint64_t total_processed = 0;
    for (const auto& count : proxy_counts) {
    ...
    double avg_cycles = total_successful > 0 ? total_cycles / (double)total_successful : 0;
    
    printf("Warps: %3u, Rings: %2u, Proxies: %2u, Throughput: %.2f Mops/sec, Avg cycles/op: %.0f\n",
           config.num_warps, config.num_ring_buffers, config.num_proxies, 
           msg_throughput / 1e6, avg_cycles);
}

// Run test
void run_test(uint32_t num_warps, uint32_t num_ring_buffers, uint32_t num_proxies,
             uint32_t payload_size = 64, bool verbose = false) {
    TestConfig config = {
        .num_warps = num_warps,
    ...
    cudaMallocManaged(&d_ring_buffers, sizeof(DeviceToHostCmdBuffer*) * num_ring_buffers);
    ...
    *d_stop = false;
    ...
    volatile bool h_stop_flag = false;
    ...
    for (uint32_t i = 0; i < num_proxies; i++) {
        ...
        proxy_threads.emplace_back(cpu_proxy_thread,
                                  h_ring_buffers[ring_idx], i,
                                  &h_stop_flag, &proxy_counts[i],
                                  verbose);
    }
    ...
    cas_throughput_kernel<<<grid, block>>>(
        d_ring_buffers, num_ring_buffers, config, d_metrics, d_stop
    );
...
int main(int argc, char** argv) {
    ...
    printf("\n========== CAS-based Ring Buffer Throughput Test (Head/Tail Batch Mode) ==========\n");
