...
void exchange_all_experts_info(
  ...
  int low_global_gpu =
      (my_global_gpu < remote_global_gpu) ? my_global_gpu : remote_global_gpu;
  int high_global_gpu =
...
void run_gpu_thread(int gpu_id, int my_rank, int num_nodes,
  ...
  for (int e = 0; e < NUM_EXPERTS_PER_GPU; e++) {
    ...
    expert_qp->mr =
        ibv_reg_mr_iova2(expert_qp->pd, gpu_buf, total_size, iova,
    ...
    expert_qp->qp =
        create_srd_qp_ex(expert_qp->context, expert_qp->pd, expert_qp->cq);
    if (!expert_qp->qp) {
