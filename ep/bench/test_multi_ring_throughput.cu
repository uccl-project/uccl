...
void print_results(const std::vector<WarpMetrics>& metrics,
    ...
    uint32_t rings_per_proxy = (config.num_warps + config.num_proxies - 1) / config.num_proxies;

    printf("Warps: %3u, Proxies: %2u, Rings/proxy: %2u, Throughput: %.2f Mops/sec\n",
           config.num_warps, config.num_proxies, rings_per_proxy, msg_throughput / 1e6);
}

// Run single test
void run_test(uint32_t num_warps, uint32_t payload_size = 64, bool verbose = false) {
    ...
    cudaMallocManaged(&d_ring_buffers, sizeof(DeviceToHostCmdBuffer*) * num_warps);
    ...
    *d_stop = false;
    ...
    for (uint32_t p = 0; p < num_proxies; p++) {
        ...
        proxy_threads.emplace_back(cpu_proxy_thread_multi,
                                  proxy_rings, p,
                                  &h_stop_flag, &proxy_counts[p],
                                  verbose);
    }
    ...
    multi_ring_throughput_kernel<<<grid, block>>>(
        d_ring_buffers, config, d_metrics, d_stop
    );
...
void run_test_fixed_proxies(uint32_t num_warps, uint32_t payload_size, bool verbose, uint32_t fixed_proxies) {
    // Each warp gets its own ring buffer
