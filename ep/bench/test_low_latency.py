...
def test_main(
    ...
    print(
        ...
        f"avg_t={avg_t * 1e9:.2f} us, min_t={min_t * 1e9:.2f} us, max_t={max_t * 1e9:.2f} us",
        flush=True,
    )
    # Separate profiling
    for return_recv_hook in (False, True):
        ...
        if not return_recv_hook:
            print(
                f"[rank {rank}] Dispatch bandwidth: {num_dispatch_comm_bytes / 1e9 / dispatch_t:.2f} GB/s, avg_t={dispatch_t * 1e9:.2f} us | "
                f"Combine bandwidth: {num_combine_comm_bytes / 1e9 / combine_t:.2f} GB/s, avg_t={combine_t * 1e9:.2f} us",
                flush=True,
            )
        else:
            print(
                f"[rank {rank}] Dispatch send/recv time: {dispatch_t[0] * 1e9:.2f} + {dispatch_t[1] * 1e9:.2f} us | "
                f"Combine send/recv time: {combine_t[0] * 1e9:.2f} + {combine_t[1] * 1e9:.2f} us",
                flush=True,
            )
    return hash_value
