...
torch::Tensor pack_moe_data(
    ...
    auto per_rank_bytes = torch::zeros({world_size},
        torch::TensorOptions().dtype(torch::kInt32).device(x.device()));

    pack_moe_data_cuda(
        x, topk_idx, topk_weights, buffers,
...
unpack_moe_data(
    ...
    auto [recv_x, recv_topk_idx, recv_topk_weights, expert_counts] =
        unpack_moe_data_cuda(
            buffers, per_rank_recv_bytes,
            num_local_experts, hidden_dim, world_size,
            x_dtype, idx_dtype, weight_dtype
        );
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("pack_moe_data", &moe_pack_unpack::pack_moe_data,
          "Pack MoE data into per-rank buffers (CUDA)",
          py::arg("x"),
          py::arg("topk_idx"),
          py::arg("topk_weights"),
          py::arg("buffers"),
    ...
    m.def("unpack_moe_data", &moe_pack_unpack::unpack_moe_data,
          "Unpack MoE data from per-rank buffers (CUDA)",
          py::arg("buffers"),
          py::arg("per_rank_recv_bytes"),
