...
configs = [
    ...
    {'tokens': 4096, 'hidden': 7168, 'experts': 256, 'topk': 8, 'name': 'Prefill batch'},
]
...
def verify_correctness(x, topk_idx, topk_weights, recv_x, recv_idx, recv_weights, expert_counts):
    ...
    if actual_items < expected_items:
        return False, f"Item count mismatch: expected {expected_items}, got {actual_items}"
...
def benchmark_backend(backend_name, pack_fn, unpack_fn, x, topk_idx, topk_weights, 
                      buffers, cfg, world_size, num_local_experts, num_iterations=100):
    ...
    for _ in range(50):
        per_rank_bytes = pack_fn(
            x_copy, topk_idx_copy, topk_weights_copy, cfg['experts'], 
            world_size, torch.device('cuda'), buffers_copy
        )
    torch.cuda.synchronize()

    pack_events_start = [torch.cuda.Event(enable_timing=True) for _ in range(num_iterations)]
    pack_events_end = [torch.cuda.Event(enable_timing=True) for _ in range(num_iterations)]
    ...
    torch.cuda.synchronize()
    pack_times = np.array([s.elapsed_time(e) for s, e in zip(pack_events_start, pack_events_end)])
    ...
    for _ in range(50):
        recv_x, recv_idx, recv_weights, expert_counts = unpack_fn(
            packed_buffers, packed_per_rank_bytes, num_local_experts, cfg['hidden'], world_size,
            torch.device('cuda'), torch.bfloat16, torch.int64, torch.float32
        )
    torch.cuda.synchronize()

    unpack_events_start = [torch.cuda.Event(enable_timing=True) for _ in range(num_iterations)]
    unpack_events_end = [torch.cuda.Event(enable_timing=True) for _ in range(num_iterations)]
    ...
    torch.cuda.synchronize()
    unpack_times = np.array([s.elapsed_time(e) for s, e in zip(unpack_events_start, unpack_events_end)])
    ...
    is_correct, correctness_msg = verify_correctness(
        x_copy, topk_idx_copy, topk_weights_copy, recv_x, recv_idx, recv_weights, expert_counts
    )
...
for cfg in configs:
    ...
    topk_idx = torch.randint(0, cfg['experts'], (cfg['tokens'], cfg['topk']), dtype=torch.int64, device='cuda')
    topk_weights = torch.randn(cfg['tokens'], cfg['topk'], dtype=torch.float32, device='cuda')
    ...
    buffers = [torch.zeros(max_items * bytes_per_item, dtype=torch.uint8, device='cuda') for _ in range(world_size)]
    ...
    try:
        triton_result = benchmark_backend(
            ...
            pack_moe_data_to_buffers_triton,
            unpack_moe_data_from_buffers_triton,
            x, topk_idx, topk_weights, buffers, cfg, world_size, num_local_experts
        )
    ...
    print("\n📦 Pack Performance:")
    print("-" * 70)
    print(f"{'Backend':<12} {'Mean (ms)':<12} {'Min':<10} {'Max':<10} {'Std':<10} {'P99':<10} {'Tokens/s':<12} {'GB/s':<10}")
    print("-" * 70)
    ...
    for result in results:
        if result is None:
            continue
        p = result['pack']
        print(f"{result['backend']:<12} {p['mean']:>11.3f}  {p['min']:>9.3f}  {p['max']:>9.3f}  "
    ...
    for result in results:
        if result is None:
            continue
        u = result['unpack']
        print(f"{result['backend']:<12} {u['mean']:>11.3f}  {u['min']:>9.3f}  {u['max']:>9.3f}  "
    ...
    if len(results) == 2 and results[0] is not None and results[1] is not None:
        ...
        if total_speedup > 1.0:
            ...
            if total_speedup > 10:
                print(f"  ⚠️  Large speedup expected: Triton uses Python loops, CUDA uses GPU kernels")
