...
__global__ void pack_moe_kernel(
    const scalar_t* __restrict__ x,           // (num_tokens, hidden_dim)
    const idx_t* __restrict__ topk_idx,       // (num_tokens, experts_per_token)
    const weight_t* __restrict__ topk_weights,// (num_tokens, experts_per_token)
    ...
    const scalar_t* token_data = x + token_id * hidden_dim;
    uint8_t* out_ptr = target_buf + byte_offset;
    const uint8_t* in_ptr = reinterpret_cast<const uint8_t*>(token_data);
    ...
    in_ptr = reinterpret_cast<const uint8_t*>(&local_expert_id_typed);
    ...
    out_ptr = target_buf + byte_offset;
    in_ptr = reinterpret_cast<const uint8_t*>(&weight_val);
...
__global__ void unpack_moe_kernel(
    const uint8_t* const* __restrict__ buffers,  // world_size buffer pointers
    const int* __restrict__ per_rank_recv_bytes, // (world_size,)
    ...
    const uint8_t* buf = buffers[sender_rank];

    // Each thread unpacks one item
    for (int item_idx = threadIdx.x; item_idx < num_items; item_idx += blockDim.x) {
        ...
        const uint8_t* in_ptr = buf + byte_offset;
...
__global__ void count_expert_tokens_kernel(
    const idx_t* __restrict__ recv_topk_idx,  // (total_items,)
...
void pack_moe_data_cuda(
    ...
    auto per_rank_item_counts = torch::zeros({world_size},
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16,
                                     x.scalar_type(), "pack_moe_kernel", [&] {
        using scalar_t = scalar_t;
        AT_DISPATCH_INDEX_TYPES(topk_idx.scalar_type(), "pack_moe_kernel_idx", [&] {
            using idx_t = index_t;
            using weight_t = float; // topk_weights is always float32

            pack_moe_kernel<scalar_t, idx_t, weight_t><<<blocks, threads>>>(
                x.data_ptr<scalar_t>(),
                topk_idx.data_ptr<idx_t>(),
                topk_weights.data_ptr<weight_t>(),
                d_buffer_ptrs,
                per_rank_item_counts.data_ptr<int>(),
                num_tokens,
                hidden_dim,
                experts_per_token,
                num_local_experts,
...
unpack_moe_data_cuda(
    const std::vector<torch::Tensor>& buffers,
    const torch::Tensor& per_rank_recv_bytes,
    int num_local_experts,
    int hidden_dim,
    int world_size,
    ...
    TORCH_CHECK(per_rank_recv_bytes.is_cuda(), "per_rank_recv_bytes must be a CUDA tensor");
    ...
    auto recv_x = torch::empty({total_items, hidden_dim},
        torch::TensorOptions().dtype(x_dtype).device(device));
    auto recv_topk_idx = torch::empty({total_items},
        torch::TensorOptions().dtype(idx_dtype).device(device));
    auto recv_topk_weights = torch::empty({total_items},
        torch::TensorOptions().dtype(weight_dtype).device(device));

    // Copy offsets to GPU
    auto d_item_offsets = torch::from_blob(h_item_offsets.data(), {world_size}, torch::kInt32)
        .to(device);

    // Create buffer pointer array on GPU
    std::vector<const uint8_t*> h_buffer_ptrs(world_size);
    for (int i = 0; i < world_size; i++) {
        h_buffer_ptrs[i] = buffers[i].data_ptr<uint8_t>();
    }

    const uint8_t** d_buffer_ptrs;
    CUDA_CHECK(cudaMalloc(&d_buffer_ptrs, world_size * sizeof(const uint8_t*)));
    CUDA_CHECK(cudaMemcpy(d_buffer_ptrs, h_buffer_ptrs.data(),
                          world_size * sizeof(const uint8_t*), cudaMemcpyHostToDevice));
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16,
        ...
        AT_DISPATCH_INDEX_TYPES(idx_dtype, "unpack_moe_kernel_idx", [&] {
            ...
            unpack_moe_kernel<scalar_t, idx_t, weight_t><<<blocks, threads>>>(
                d_buffer_ptrs,
                per_rank_recv_bytes.data_ptr<int>(),
                recv_x.data_ptr<scalar_t>(),
                recv_topk_idx.data_ptr<idx_t>(),
                recv_topk_weights.data_ptr<weight_t>(),
                d_item_offsets.data_ptr<int>(),
                hidden_dim,
                world_size
            );
        });
    ...
    if (total_items > 1) {  // Skip if empty
        ...
        AT_DISPATCH_INDEX_TYPES(idx_dtype, "count_expert_tokens_kernel", [&] {
            using idx_t = index_t;
            count_expert_tokens_kernel<idx_t><<<blocks, threads>>>(
                recv_topk_idx.data_ptr<idx_t>(),
    ...
    return std::make_tuple(recv_x, recv_topk_idx, recv_topk_weights, expert_counts);
}
