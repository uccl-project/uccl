...
from utils import init_dist, bench, calc_diff, inplace_unique, per_token_cast_to_fp8, per_token_cast_back, initialize_uccl, destroy_uccl
...
def test_main(args: argparse.Namespace, num_sms: int, local_rank: int, num_ranks: int, rank: int,
              buffer: Buffer, group: dist.ProcessGroup):
    # Settings
    ...
    dispatch_config = Config(best_dispatch_results[0], best_dispatch_results[1], nvl_buffer_size)
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    if test_ll_compatibility:
        ...
        num_rdma_bytes = Buffer.get_low_latency_rdma_size_hint(ll_num_tokens, ll_hidden, num_ranks, ll_num_experts)
    ...
    buffer = Buffer(group, scratch.data_ptr(), num_nvlink_bytes, num_rdma_bytes, low_latency_mode=test_ll_compatibility,
