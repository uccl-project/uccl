"""
Core Buffer class for UCCL Expert Parallel communication.
"""
...
class Buffer:
    ...
    Attributes:
        ...
        proxies: list of UCCL proxy threads for communication.
        workers: peer copy workers (optional).
    """
    ...
    def __init__(
        ...
        # Make C++ runtime available
        root_unique_id = None
        self.runtime.sync(device_ids, ipc_handles, root_unique_id, rdma_ipc_handles)
        assert self.runtime.is_available()

        # Connect atomic buffer for proxies
    ...
    def connect_atomic_buffer(self, proxy):
        """
        Connect the atomic buffer between proxy and runtime.

        Arguments:
            proxy: the proxy object to connect.
        """
    ...
    def is_sm90_compiled():
        """
        Check if the library is compiled with SM90 features.

        Returns:
            bool: True if SM90 features are enabled.
        """
    ...
    def _unpack_bias(bias: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]):
        """
        Unpack bias tensor(s).

        Arguments:
            bias: single tensor or tuple of two tensors.

        Returns:
            bias_0, bias_1: unpacked bias tensors.
        """
    ...
    def low_latency_dispatch(
        ...
        """
        # Calculate dispatch offset for each proxy
        for proxy in self.proxies:
