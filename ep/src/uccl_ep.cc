...
// The JAX FFI TU (ep/src/uccl_ep_jax.cc) exposes ``register_jax_bindings``
// which is called below in NB_MODULE(ep, m).
...
NB_MODULE(ep, m) {
  ...
  // JAX FFI integration
  // ----------------------------------------------------------------------
  // Handlers + registry + nanobind plumbing live in a separate
  // translation unit (ep/src/uccl_ep_jax.cc). Since ``Buffer`` is now
  // declared in ep/include/uccl_ep.hpp, the JAX TU calls ``Buffer``
  // methods directly and no trampoline bridge is needed on this side.
