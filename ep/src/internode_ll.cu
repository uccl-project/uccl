...
#pragma unroll
  ...
  LOW_LATENCY_DISPATCH_RECV:
    ...
    if (responsible_expert_idx < num_experts) {
      ...
      if (sub_warp_id == 1 and lane_id == 0) {
        ...
        // printf(
        //     "[RECV_COUNT_DECODED] Decoded token count: %d (from received
        //     value"
        //     "%d), count_addr; %p\n",
        //     num_recv_tokens, -num_recv_tokens - 1,
        //     (void*)(rdma_recv_count + local_expert_idx * num_ranks +
      ...
      for (int i = sub_warp_id; i < num_recv_tokens; i += num_warps_per_group) {
        ...
        if constexpr (kUseFP8) {
          ...
          //   (num_tokens, (num_packed,
          //   num_elems_per_pack)):(num_elems_per_pack, (num_tokens *
          //   num_elems_per_pack, 1))
  ...
  void dispatch(
      ...
      bool round_scale, bool use_ue8m0, void* workspace, int num_device_sms,
      cudaStream_t stream, int phases, uint64_t const* ring_addrs,
...
#undef DISPATCH_LAUNCH_CASE
  ...
  __global__ __launch_bounds__(1024, 1) void combine(
      void* combined_x, void* rdma_recv_x, int* rdma_recv_flag,
      ...
      int* next_clean, int num_next_clean_int, int* atomic_clean_flag,
      ...
      int max_nvl_peers, void** ipc_base_ptrs = nullptr,
      void* rdma_buffer_ptr = nullptr, void* atomic_buffer_ptr = nullptr) {
    auto const sm_id = static_cast<int>(blockIdx.x);
    auto const num_sms = static_cast<int>(gridDim.x);
    auto const thread_id = static_cast<int>(threadIdx.x);
...
#pragma unroll
    ...
    if (responsible_expert_idx < num_experts) {
      auto const dst_rank = responsible_expert_idx / num_local_experts;
      auto const local_expert_idx = responsible_expert_idx % num_local_experts;
      auto const global_expert_idx =
          rank * num_local_experts + local_expert_idx;
      auto const layout =
      ...
      auto tma_buffer = PatternVisitor([=](int const& i) {
        return reinterpret_cast<int4*>(smem_ptr +
                                       i * (kNumTMABufferBytes + 16));
      });
...
#pragma unroll
              ...
              if (amax <= kThreshold and log_amin < log_amax) {
                // Transform
                auto transform =
                    [=](float const& log_abs_value) -> nv_bfloat16 {
                  auto const encoded =
...
#pragma unroll
      ...
      if (sub_warp_id == 1 and lane_id == 0) {
        ...
        // need to calculate the offset from rdma_recv_x (data buffer) to the
        // flag buffer Try to use IPC for intra-node atomic operations
        auto dst_ptr =
        ...
        for (int i = 0; i < num_topk; ++i)
          if (reg_topk_idx[i] >= 0) {
            ...
            auto rdma_buffer_type = reinterpret_cast<int const*>(
                static_cast<uint8_t*>(rdma_recv_x) +
                (reg_topk_idx[i] * num_max_dispatch_tokens_per_rank +
                 token_idx) *
                    num_bytes_per_slot);
