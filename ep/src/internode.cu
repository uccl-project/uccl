...
__global__ void notify_dispatch(
    ...
    int rank, uint64_t const* d2h_channel_addrs, int num_d2h_channel_addrs,
    void* atomic_buffer_ptr) {
  void* original_rdma_buffer_ptr = rdma_buffer_ptr;
  auto sm_id = static_cast<int>(blockIdx.x);
  auto thread_id = static_cast<int>(threadIdx.x),
       warp_id = thread_id / WARP_SIZE, lane_id = get_lane_id();
...
__global__ void __launch_bounds__(
  ...
  auto const thread_id = static_cast<int>(threadIdx.x),
             warp_id = thread_id / WARP_SIZE, lane_id = get_lane_id();
  auto const num_channels = num_sms / 2, channel_id = sm_id / 2;
  ...
  auto num_bytes_per_token =
      get_num_bytes_per_token(hidden_int4, num_scales, num_topk, num_topk);
  ...
  // Allocate buffers
  auto nvl_channel_x =
      AsymBuffer<uint8_t>(ws_rr_buffer_ptr,
  ...
  auto nvl_channel_prefix_start =
      AsymBuffer<int>(ws_rr_buffer_ptr, kNumRDMARanks, NUM_MAX_NVL_PEERS,
  ...
  auto nvl_channel_head =
      AsymBuffer<int>(rs_wr_buffer_ptr, 1, NUM_MAX_NVL_PEERS, channel_id,
                      num_channels, ws_rr_rank)
          .advance_also(ws_rr_buffer_ptr);
  auto nvl_channel_tail =
      AsymBuffer<int>(ws_rr_buffer_ptr, 1, NUM_MAX_NVL_PEERS, channel_id,
  ...
  auto sync_rdma_sender_smem = []() {
    sync_barrier(0, (kNumDispatchRDMASenderWarps + 1) * WARP_SIZE);
  };
...
// TMA stuffs
  ...
  if ((warp_role == WarpRole::kRDMAAndNVLForwarder or
       warp_role == WarpRole::kNVLReceivers) and
      lane_id == 0) {
  ...
  __shared__ int volatile forward_channel_head[NUM_MAX_NVL_PEERS]
                                              [kNumRDMARanks];
  __shared__ bool volatile forward_channel_retired[NUM_MAX_NVL_PEERS];

  // Per-warp shared memory scratchpads for replacing __shfl_sync with LDS reads.
  // Indexed by [warp_id][rank]. Each warp writes its per-lane value, then all
  // lanes read from the target rank's slot.
  constexpr int kNumTotalWarps =
      kNumDispatchRDMASenderWarps + 1 + NUM_MAX_NVL_PEERS;
  __shared__ int smem_warp_i32[kNumTotalWarps][kNumRDMARanks];
  __shared__ uint64_t smem_warp_u64[kNumTotalWarps][kNumRDMARanks];
  __shared__ void* smem_warp_ptr[kNumTotalWarps][kNumRDMARanks];
  auto sync_forwarder_smem = []() {
    sync_barrier_1((NUM_MAX_NVL_PEERS + 1) * WARP_SIZE);
  };
  ...
  if (warp_role == WarpRole::kRDMASender) {
    ...
    for (token_idx = token_start_idx; token_idx < token_end_idx; ++token_idx) {
      ...
      // Broadcast tails via shared memory
      if (lane_id < kNumRDMARanks) {
        smem_warp_i32[warp_id][lane_id] = rdma_tail_idx;
        smem_warp_u64[warp_id][lane_id] = is_token_in_rank_uint64;
        smem_warp_ptr[warp_id][lane_id] = send_buffer;
      }
      __syncwarp();
      SourceMeta src_meta;
...
#pragma unroll
      for (int i = 0, slot_idx; i < kNumRDMARanks; ++i)
        if ((slot_idx = smem_warp_i32[warp_id][i]) >= 0) {
          slot_idx = slot_idx % num_max_rdma_chunked_recv_tokens;
          topk_ranks[num_topk_ranks] = i;
          auto recv_is_token_in_rank_uint64 = smem_warp_u64[warp_id][i];
          ...
          dst_send_buffers[num_topk_ranks++] =
              reinterpret_cast<uint8_t*>(smem_warp_ptr[warp_id][i]) +
              slot_idx * num_bytes_per_token;
        }
      EP_DEVICE_ASSERT(num_topk_ranks <= kNumTopkRDMARanks);

      // Copy `x` into symmetric send buffer
      auto st_broadcast = [=](int const key, int4 const& value) {
#pragma unroll
      ...
      UNROLLED_WARP_COPY(5, lane_id, hidden_int4, 0,
                         x + token_idx * hidden_int4, ld_nc_global,
                         st_broadcast);
#pragma unroll
...
#pragma unroll
    ...
    // Store per-lane values to shared memory for broadcast
    if (lane_id < kNumRDMARanks) {
      smem_warp_i32[warp_id][lane_id] = num_tokens_to_send;
      smem_warp_u64[warp_id][lane_id] = last_issued_tail;
    }
    __syncwarp();
    auto start_time = clock64();
    while (__any_sync(WARP_MASK, num_tokens_to_send > 0)) {
      // Timeout check
      if (clock64() - start_time > NUM_TIMEOUT_CYCLES and
          lane_id < kNumRDMARanks) {
        printf(
            "DeepEP RDMA sender coordinator timeout, channel: %d, IB: %d, nvl "
            "%d, dst IB: %d, tail: %d, remaining: %d\n",
            channel_id, rdma_rank, nvl_rank, lane_id, last_issued_tail,
            num_tokens_to_send);
        trap();
      }
      ...
      for (int i = 0, synced_num_tokens_to_send; i < kNumRDMARanks; ++i) {
        ...
        // rank for different ranks and channels
        int dst_rdma_rank = (i + channel_id + rdma_rank) % kNumRDMARanks;
        synced_num_tokens_to_send = smem_warp_i32[warp_id][dst_rdma_rank];
        ...
        auto synced_last_issued_tail =
            static_cast<int>(smem_warp_u64[warp_id][dst_rdma_rank]);
      ...
      while (true) {
        ...
        if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
          printf(
              ...
              "%d, %d, %d\n",
              channel_id, rdma_rank, nvl_rank, lane_id, dst_nvl_rank, meta_0,
    ...
    // Store per-lane forwarder state to shared memory for broadcast
    if (lane_id < kNumRDMARanks) {
      smem_warp_i32[warp_id][lane_id] = num_tokens_to_recv_from_rdma;
    }
    __syncwarp();
    while (__any_sync(WARP_MASK, num_tokens_to_recv_from_rdma > 0)) {
      ...
      while (true) {
        ...
        if (smem_warp_i32[warp_id][src_rdma_rank] > 0) {
          auto tail_val = static_cast<int>(
              ld_acquire_sys_global<kUseAggressiveAtomic>(
                  rdma_channel_tail.buffer(src_rdma_rank)));
          if (lane_id == src_rdma_rank) cached_rdma_channel_tail = tail_val;
          auto head_val = forward_channel_head[target_rank][src_rdma_rank];
      ...
      auto src_rdma_head =
          static_cast<int>(forward_channel_head[target_rank][src_rdma_rank]);
      auto src_rdma_tail = static_cast<int>(
          ld_acquire_sys_global<kUseAggressiveAtomic>(
              rdma_channel_tail.buffer(src_rdma_rank)));
      ...
      for (int i = src_rdma_head, num_tokens_sent = 0; i < src_rdma_tail; ++i) {
        ...
        auto dst_shifted =
            nvl_channel_x.buffer() + dst_slot_idx * num_bytes_per_token;
...
#pragma unroll
    ...
    int const local_expert_begin = rank * (num_experts / num_ranks);
    int const local_expert_end = local_expert_begin + (num_experts / num_ranks);
    ...
    // Store per-lane total_offset to shared memory for broadcast
    if (lane_id < kNumRDMARanks)
      smem_warp_u64[warp_id][lane_id] = total_offset;
    __syncwarp();
    while (num_tokens_to_recv > 0) {
      ...
      for (int chunk_idx = 0; chunk_idx < num_recv_tokens;
        ...
        auto meta = ld_nc_global(reinterpret_cast<SourceMeta*>(
            shifted + hidden_bytes + scale_bytes));
        int64_t recv_token_idx =
            static_cast<int64_t>(smem_warp_u64[warp_id][meta.src_rdma_rank]);
        ...
        mbarrier_wait(tma_mbarrier, tma_phase);
        if (lane_id == 0) {
          tma_store_1d(tma_buffer, recv_x + recv_token_idx * hidden_int4,
                       hidden_bytes, false);
          if (scale_aligned)
        ...
        if (not scale_aligned) {
          UNROLLED_WARP_COPY(1, lane_id, num_scales,
                             recv_x_scales + recv_token_idx * num_scales,
                             reinterpret_cast<float*>(shifted), ld_nc_global,
                             st_na_global);
        }
        shifted += scale_bytes;
        ...
        if (lane_id < num_topk) {
          // Read
          auto idx_value = static_cast<int64_t>(
              ld_nc_global(reinterpret_cast<int*>(shifted) + lane_id));
          auto weight_value = ld_nc_global(
              reinterpret_cast<float*>(shifted + sizeof(int) * num_topk) +
              lane_id);
          auto recv_idx = recv_token_idx * num_topk + lane_id;

          // Transform and write
          idx_value =
              (idx_value >= local_expert_begin and idx_value < local_expert_end)
  ...
  if (num_worst_tokens > 0) {
    if (is_forwarder) return;
    // get the actual number of num_recv_tokens on the current rank
    int num_recv_tokens = recv_gbl_rank_prefix_sum[num_ranks - 1];
    // some ForwarderCoordinator threads exit early, so we only use
    // non-forwarder in clean-up channel_id * num_threads is the offset of the
    // current non-forwarder sms
    auto const clean_start =
        num_recv_tokens * num_topk + channel_id * num_threads;
    auto const clean_end = num_worst_tokens * num_topk;
    auto const clean_stride = num_channels * num_threads;
#pragma unroll
...
#define DISPATCH_LAUNCH_CASE(num_rdma_ranks)                                   \
  {                                                                            \
    ...
    LAUNCH_KERNEL(                                                             \
        ...
        recv_gbl_channel_prefix_matrix, rdma_channel_prefix_matrix,            \
        recv_rdma_rank_prefix_sum, gbl_channel_prefix_matrix,                  \
        recv_gbl_rank_prefix_sum, is_token_in_rank, num_tokens,                \
        ...
        num_max_nvl_chunked_recv_tokens, rank, num_ranks, d2h_channel_addrs,   \
        num_d2h_channel_addrs, atomic_buffer_ptr);                             \
  }                                                                            \
  break
...
#pragma unroll
    ...
    for (int channel_start = 0; channel_start < num_channels;
      ...
      if (lane_id < num_rdma_ranks and warp_id < num_channels) {
        int token_start_idx, token_end_idx;
        get_channel_task_range(num_combined_tokens, num_channels, warp_id,
                               token_start_idx, token_end_idx);
    ...
    for (int channel_start = 0; channel_start < num_channels;
      ...
      if (warp_id < num_channels) {
        ...
        constexpr int num_tokens_per_batch =
            tma_batch_size / num_bytes_per_token;
        ...
        auto tma_mbarrier =
            reinterpret_cast<uint64_t*>(tma_buffer + tma_batch_size);
        uint32_t tma_phase = 0;
        if (lane_id == 0) {
          mbarrier_init(tma_mbarrier, 1);
          fence_view_async_shared();
        ...
        for (int dst_rdma_rank = sm_id - 2; dst_rdma_rank < num_rdma_ranks;
             dst_rdma_rank += num_channels * 2 - 2) {
          // Iterate in reverse order
          int token_start_idx =
              warp_id == 0
                  ? 0
          ...
          // NOTES: `1 << 25` is a heuristic large number
          int last_head = 1 << 25;
          for (int batch_end_idx = token_end_idx;
               batch_end_idx > token_start_idx;
            ...
            for (int token_idx = batch_end_idx - 1;
                 token_idx >= batch_start_idx; --token_idx) {
              if (lane_id < NUM_MAX_NVL_PEERS) {
#if defined(__HIP_PLATFORM_AMD__) || defined(__HIPCC__)
                auto current_head =
                    __ldg(combined_nvl_head + token_idx * NUM_MAX_NVL_PEERS +
                          lane_id);
                if (current_head < 0) {
              ...
              if (current_head < 0) {
                reinterpret_cast<int*>(
                    tma_buffer)[(token_idx - batch_start_idx) *
                                    NUM_MAX_NVL_PEERS +
                                lane_id] = -last_head - 1;
              } else {
                last_head = current_head;
...
template <int kNumRanks, bool kMaybeWithBias, typename dtype_t,
          int kMaxNumRanks, bool kUseTMA, int kNumStages, int kSmemStride,
...
__forceinline__ __device__ int combine_token(
    ...
    int* smem_is_token, int* smem_head_idx, uint8_t* smem_ptr,
    uint32_t (&tma_phase)[kNumStages]) {
  constexpr auto kDtypePerInt4 = sizeof(int4) / sizeof(dtype_t);

  // Broadcast current heads via shared memory (lane `i` holds rank `i`).
  EP_STATIC_ASSERT(kMaxNumRanks <= WARP_SIZE, "Too many ranks");
  int const phys_warp = threadIdx.x / WARP_SIZE;
  int const smem_base = phys_warp * kSmemStride;
  __syncwarp();
  if (lane_id < kNumRanks) {
    smem_is_token[smem_base + lane_id] = is_token_in_rank ? 1 : 0;
    smem_head_idx[smem_base + lane_id] = head_idx;
  }
  __syncwarp();
  int num_topk_ranks = 0, topk_ranks[kMaxNumRanks], slot_indices[kMaxNumRanks];
#pragma unroll
  for (int i = 0; i < kNumRanks; ++i)
    if (smem_is_token[smem_base + i]) {
      slot_indices[num_topk_ranks] =
          smem_head_idx[smem_base + i] % num_max_recv_tokens;
      topk_ranks[num_topk_ranks++] = i;
    }
...
__global__ void __launch_bounds__((kNumForwarders + 1) * WARP_SIZE, 1)
  ...
  auto const num_bytes_per_token =
      get_num_bytes_per_token(hidden_int4, 0, 0, num_topk);
  ...
  constexpr int kCombineSmemCols =
      NUM_MAX_NVL_PEERS > kNumRDMARanks ? NUM_MAX_NVL_PEERS : kNumRDMARanks;
  __shared__ int smem_combine_i32[kNumForwarders][kCombineSmemCols];
  __shared__ int smem_combine_i32_2[kNumForwarders][kCombineSmemCols];
  __shared__ int smem_combine_i32_3[kNumForwarders][kCombineSmemCols];
  __shared__ int smem_nvl_sender_dst_slot[kNumForwarders];
  auto const phys_warp_id = thread_id / WARP_SIZE;
  ...
  if (warp_role == WarpRole::kNVLSender) {
    ...
    auto nvl_channel_head =
        AsymBuffer<int>(local_buffer_ptr, kNumRDMARanks, NUM_MAX_NVL_PEERS,
                        channel_id, num_channels, dst_nvl_rank)
            .advance_also(dst_buffer_ptr);
    auto nvl_channel_tail =
        AsymBuffer<int>(dst_buffer_ptr, kNumRDMARanks, NUM_MAX_NVL_PEERS,
    ...
    while (true) {
      // Exit if possible
      if (__all_sync(WARP_MASK, token_start_idx >= token_end_idx)) break;
      ...
      while (true) {
        ...
        is_lane_ready =
            ...
            num_max_nvl_chunked_recv_tokens_per_rdma - num_used_slots >=
                num_max_nvl_chunked_send_tokens;
        if (__any_sync(WARP_MASK, is_lane_ready)) break;
      ...
      // Sync token start index and count (shared memory instead of shuffles)
      for (int i = 0; i < kNumRDMARanks; ++i) {
        current_rdma_idx = (current_rdma_idx + 1) % kNumRDMARanks;
        __syncwarp();
        if (lane_id < kNumRDMARanks) {
          smem_combine_i32[phys_warp_id][lane_id] = token_start_idx;
          smem_combine_i32_2[phys_warp_id][lane_id] = token_end_idx;
          smem_combine_i32_3[phys_warp_id][lane_id] = is_lane_ready ? 1 : 0;
        }
        __syncwarp();
        int const peer_ts = smem_combine_i32[phys_warp_id][current_rdma_idx];
        int const peer_te = smem_combine_i32_2[phys_warp_id][current_rdma_idx];
        bool const peer_ready =
            smem_combine_i32_3[phys_warp_id][current_rdma_idx] != 0;
        if ((peer_ts >= peer_te) or (not peer_ready)) continue;

        auto token_idx = static_cast<int64_t>(peer_ts);
        int num_tokens_in_chunk =
            min(num_max_nvl_chunked_send_tokens, peer_te - peer_ts);
        ...
        for (int chunk_idx = 0; chunk_idx < num_tokens_in_chunk;
          ...
          __syncwarp();
          if (lane_id == current_rdma_idx)
            smem_nvl_sender_dst_slot[phys_warp_id] = dst_slot_idx;
          __syncwarp();
          dst_slot_idx = smem_nvl_sender_dst_slot[phys_warp_id];
          ...
          if (lane_id < num_topk)
            st_na_global(
                reinterpret_cast<float*>(shifted_x_buffers + hidden_bytes +
                                         sizeof(SourceMeta) +
...
#pragma unroll
    ...
    auto nvl_channel_head = AsymBuffer<int, NUM_MAX_NVL_PEERS>(
                                nvl_buffers, kNumRDMARanks, NUM_MAX_NVL_PEERS,
                                channel_id, num_channels, nvl_rank)
                                .advance_also(local_nvl_buffer);
    auto nvl_channel_tail =
        AsymBuffer<int>(local_nvl_buffer, kNumRDMARanks, NUM_MAX_NVL_PEERS,
                        channel_id, num_channels)
            .advance_also<NUM_MAX_NVL_PEERS>(nvl_buffers);
    ...
    auto sync_forwarder_smem = [=]() {
#if defined(__HIP_PLATFORM_AMD__) || defined(__HIPCC__)
      sync_barrier(0, kNumForwarders * WARP_SIZE);
#else
      sync_barrier(0, (kNumForwarders + 1) * WARP_SIZE);
#endif
    };
...
#pragma unroll
      ...
      auto sync_large_warp = [=]() {
        if (kNumWarpsPerForwarder == 1) {
          ...
          sync_barrier(dst_rdma_rank + 2, kNumWarpsPerForwarder * WARP_SIZE);
        }
      };
      ...
      for (int token_start_idx = 0; token_start_idx < num_tokens_to_combine;
        ...
        for (int token_idx = token_start_idx + sub_warp_id;
          ...
          combine_token<NUM_MAX_NVL_PEERS, false, dtype_t, NUM_MAX_NVL_PEERS,
              ...
              get_addr_fn, recv_tw_fn, &smem_combine_i32[0][0],
              &smem_combine_i32_2[0][0], nullptr, dummy_tma_phases);

#else
          combine_token<NUM_MAX_NVL_PEERS, false, dtype_t, NUM_MAX_NVL_PEERS,
                        true, kNumStages, kCombineSmemCols, kNumTMALoadBytes>(
              expected_head >= 0, expected_head, lane_id, hidden_int4, num_topk,
              static_cast<int4*>(shifted),
              reinterpret_cast<float*>(static_cast<int8_t*>(shifted) +
                                       hidden_bytes + sizeof(SourceMeta)),
              nullptr, nullptr, num_max_nvl_chunked_recv_tokens_per_rdma,
              get_addr_fn, recv_tw_fn, &smem_combine_i32[0][0],
              &smem_combine_i32_2[0][0], smem_ptr, tma_phase);
#endif
...
#pragma unroll
      ...
      for (int64_t token_idx = token_start_idx + warp_id;
        ...
        combine_token<kNumRDMARanks, true, dtype_t, kNumTopkRDMARanks, false,
            ...
            &smem_combine_i32[0][0], &smem_combine_i32_2[0][0], nullptr,
            dummy_tma_phases);
      }
...
#if defined(__HIP_PLATFORM_AMD__) || defined(__HIPCC__)
      EP_DEVICE_ASSERT(false and "Invalid warp role on AMD GPUs");
#endif
      // Coordinator
      ...
      int dst_rdma_rank = lane_id < kNumRDMARanks ? lane_id : 0;
      int dst_nvl_rank = lane_id < NUM_MAX_NVL_PEERS ? lane_id : 0;
      EP_STATIC_ASSERT(kNumCombineForwarderWarps <= WARP_SIZE,
                       "Invalid number of forwarder warps");
      while (true) {
        // Retired
        if (not is_forwarder_sm and
            __all_sync(WARP_MASK, lane_id >= kNumRDMAReceivers or
                                      rdma_receiver_retired[lane_id]))
          break;
        if (is_forwarder_sm and
            __all_sync(WARP_MASK,
...
#define COMBINE_LAUNCH_CASE(num_rdma_ranks)                                 \
  {                                                                         \
    ...
    LAUNCH_KERNEL(                                                          \
        ...
        rank, num_ranks, d2h_channel_addrs, num_d2h_channel_addrs,          \
        atomic_buffer_ptr);                                                 \
  }                                                                         \
  break
  ...
  EP_HOST_ASSERT(num_forwarder_warps > NUM_MAX_NVL_PEERS and
...
#if defined(__HIP_PLATFORM_AMD__) || defined(__HIPCC__)
  EP_HOST_ASSERT(num_forwarder_warps * WARP_SIZE <= MAX_NTHREADS);
  // 16 warps is the maximum number of warps that can be launched on AMD GPUs
  // MI300-MI355.
  SETUP_LAUNCH_CONFIG(num_channels * 2, num_forwarder_warps * WARP_SIZE,
                      stream);
#else
  SETUP_LAUNCH_CONFIG(num_channels * 2, (num_forwarder_warps + 1) * WARP_SIZE,
                      stream);
#endif
