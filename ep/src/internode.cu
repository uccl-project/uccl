...
struct alignas(16) SourceMeta {
  int src_rdma_rank;
  // nvl_bits layout (lower 16 used):
  //   [7:0]   membership (1 => token belongs to that NVL rank)
  //   [15:8]  delivered  (1 => pending for that NVL rank; cleared by consumer)
  //   [23:16]  COMB_READY (combine)   - per RDMAReceiver warp READY bit
  //   [31:24]  reserved
  ...
  __device__ __forceinline__ SourceMeta(int rdma_rank,
    ...
    // delivered_mask            = 0xFF; // Always 8 bits set.
#pragma unroll
    for (int i = 1; i < NUM_MAX_NVL_PEERS; ++i)
      is_token_in_nvl_rank_bits |= is_token_in_nvl_ranks[i] << i;
    is_token_in_nvl_rank_bits |= 0xFF << 8;  // Always 8 bits set.
  }
...
// READY bit base for combine; bit = 1u << (kCombReadyBitBase + warp_id)
static __device__ __constant__ uint32_t kCombReadyBitBase = 16u;
__device__ __forceinline__ uint32_t comb_ready_bit_for_warp(int warp_local_id) {
  return 1u << (kCombReadyBitBase + warp_local_id);
}
...
__global__ void notify_dispatch(
  ...
  if (sm_id == 0) {
    ...
    if (thread_id == 32) {
      uccl::nvshmemi_ibgda_quiet(ring_addrs, num_ring_addrs, nvl_rank);
    }
    __syncthreads();
...
#define NOTIFY_DISPATCH_LAUNCH_CASE(num_rdma_ranks)                           \
  {                                                                           \
    ...
    LAUNCH_KERNEL(                                                            \
        ...
        buffer_ptrs, barrier_signal_ptrs, rank, ring_addrs, num_ring_addrs,   \
        atomic_buffer_ptr);                                                   \
  }                                                                           \
  break
...
// Copy `topk_idx` and `topk_weights` into symmetric send buffer (before meta)
#pragma unroll
      for (int i = lane_id; i < num_topk * num_topk_ranks; i += 32) {
        ...
        auto* topk_base_i = reinterpret_cast<int*>(
            reinterpret_cast<SourceMeta*>(dst_send_buffers[rank_idx]) + 1);
        auto* topk_base_f = reinterpret_cast<float*>(
            reinterpret_cast<SourceMeta*>(dst_send_buffers[rank_idx]) + 1);
        st_na_global(topk_base_i + copy_idx, idx_value);
        st_na_global(topk_base_f + num_topk + copy_idx, weight_value);
      }
      __syncwarp();
      ...
      if (lane_id < num_topk_ranks) {
        ...
        tmp.is_token_in_nvl_rank_bits = 0;
        st_na_global(meta_ptr, tmp);
    ...
    while (__any_sync(0xffffffff, num_tokens_to_recv_from_rdma > 0)) {
      ...
      for (int i = src_rdma_head, num_tokens_sent = 0; i < src_rdma_tail; ++i) {
        ...
        uint32_t seen_bits = __shfl_sync(0xffffffff, seen_bits_lane0, 0);
        bool is_in_dst_nvl_rank = __shfl_sync(0xffffffff, in_rank_lane0, 0);
        __syncwarp();
#endif
        if (lane_id == src_rdma_rank) {
        ...
        // Clear our NVL bit only AFTER we’ve consumed the payload.
#ifdef FALSE
        if (lane_id == 0) {
    ...
    while (num_tokens_to_recv > 0) {
      ...
      while (true) {
        ...
        cached_channel_tail_idx = __shfl_sync(
            0xffffffff, ld_acquire_sys_global(nvl_channel_tail.buffer()), 0);
...
#define DISPATCH_LAUNCH_CASE(num_rdma_ranks)                                   \
  {                                                                            \
    ...
    LAUNCH_KERNEL(                                                             \
        ...
        num_max_nvl_chunked_recv_tokens, rank, num_ranks, ring_addrs,          \
  ...
  SETUP_LAUNCH_CONFIG(
      num_channels * 2,
      (kNumDispatchRDMASenderWarps + 1 + NUM_MAX_NVL_PEERS) * 32, stream);
...
__global__ void cached_notify(
  ...
  if (sm_id == 0) {
    if (thread_id == 32)
      uccl::nvshmemi_ibgda_quiet(ring_addrs, num_ring_addrs, nvl_rank, 3);
    __syncthreads();

    // Barrier for RDMA
    if (thread_id == 32)
      uccl::nvshmem_sync_with_same_gpu_idx(ring_addrs, num_ring_addrs, nvl_rank,
                                           3);
    ...
    if (warp_id < num_channels) {
      ...
      for (int dst_rdma_rank = sm_id - 2; dst_rdma_rank < num_rdma_ranks;
        ...
        for (int batch_end_idx = token_end_idx; batch_end_idx > token_start_idx;
          ...
          for (int token_idx = batch_end_idx - 1; token_idx >= batch_start_idx;
               --token_idx) {
            if (lane_id < NUM_MAX_NVL_PEERS) {
...
void cached_notify(int hidden_int4, int num_scales, int num_topk_idx,
  ...
  LAUNCH_KERNEL(&cfg, cached_notify_func, rdma_clean_meta.first,
                ...
                ring_addrs, num_ring_addrs, atomic_buffer_ptr);
}
    ...
    if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
...
#ifdef FALSE
      int ch_token_start_idx, ch_token_end_idx;
      get_channel_task_range(num_combined_tokens, num_channels, channel_id,
                             ch_token_start_idx, ch_token_end_idx);
#endif
      ...
      for (int64_t token_idx = token_start_idx + warp_id;
        ...
        combine_token<kNumRDMARanks, true, dtype_t, kNumTopkRDMARanks, false,
            ...
            dummy_tma_phases);
#ifdef FALSE
        // Clear my readiness bit once consumed
        if (expected_head >= 0 && lane_id == 0) {
          uint32_t my_ready = comb_ready_bit_for_warp(warp_id);
          atomicAnd(reinterpret_cast<unsigned int*>(
                        &ready_meta->is_token_in_nvl_rank_bits),
                    ~my_ready);
      ...
      while (true) {
        ...
        if (not is_forwarder_sm and
            __all_sync(0xffffffff, lane_id >= kNumRDMAReceivers or
...
#define COMBINE_LAUNCH_CASE(num_rdma_ranks)                                 \
  {                                                                         \
    ...
    LAUNCH_KERNEL(                                                          \
        ...
        rank, num_ranks, ring_addrs, num_ring_addrs, atomic_buffer_ptr);    \
  }                                                                         \
  break
