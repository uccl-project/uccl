...
#define NOTIFY_DISPATCH_LAUNCH_CASE(ranks)                                 \
  ...
  EP_HOST_ASSERT(num_experts / num_ranks <= kNumThreads and
...
#define CACHED_NOTIFY_DISPATCH_LAUNCH_CASE(ranks)                        \
  LAUNCH_KERNEL(&cfg, cached_notify_dispatch<ranks>, rank_prefix_matrix, \
...
__global__ void __launch_bounds__(kNumThreads, 1)
  ...
  auto const responsible_rank =
      (static_cast<int>(thread_id)) / num_threads_per_rank;
  ...
  auto channel_x_buffers = Buffer<int4>(
      ptr, num_channels_total * num_recv_buffer_tokens * hidden_int4,
      channel_rank_offset * num_recv_buffer_tokens * hidden_int4);
  auto channel_src_idx_buffers =
      Buffer<int>(ptr, num_channels_total * num_recv_buffer_tokens,
                  channel_rank_offset * num_recv_buffer_tokens);
  auto channel_topk_idx_buffers = Buffer<int64_t>(
      ptr, num_channels_total * num_recv_buffer_tokens * num_topk,
  ...
  auto channel_x_scales_buffers = Buffer<float>(
      ptr, num_channels_total * num_recv_buffer_tokens * num_scales,
  ...
  auto tma_mbarrier =
      reinterpret_cast<uint64_t*>(tma_buffer + half_hidden_bytes);
  ...
  if (is_sender) {
    ...
    auto const send_warp_id_in_rank =
        send_thread_id % num_threads_per_rank / WARP_SIZE;
    ...
    if (lane_id == 0 and send_warp_id_in_rank == 0) {
      ...
      value = channel_prefix_matrix[responsible_rank * num_channels +
                                    responsible_channel];
      ...
      while (chunk_token_idx < num_max_send_tokens and
        ...
        if (cached_channel_tail_idx % num_send_warps_per_rank ==
          ...
          if (lane_id < num_topk) {
            ...
            idx_value =
                (idx_value >= recv_expert_begin and idx_value < recv_expert_end)
...
#pragma unroll
          for (int i = lane_id; i < num_scales; i += WARP_SIZE) {
            auto offset =
                token_idx * scale_token_stride + i * scale_hidden_stride;
...
#pragma unroll
    ...
    auto const channel_id = sm_id - 1;
    auto const thread_id = static_cast<int>(threadIdx.x);
    auto const rank_id = thread_id / WARP_SIZE;
    auto const lane_id = thread_id % WARP_SIZE;
...
#pragma unroll
    for (int token_idx_tail = token_end_idx - 1;
         token_idx_tail >= token_start_idx; token_idx_tail -= WARP_SIZE) {
...
#define CACHED_NOTIFY_COMBINE(ranks)                                        \
  LAUNCH_KERNEL(&cfg, cached_notify_combine<ranks>, buffer_ptrs, send_head, \
                num_channels, num_recv_tokens, num_memset_int,              \
                barrier_signal_ptrs, rank);                                 \
  break

  int const num_threads = std::max(128, WARP_SIZE * num_ranks);
...
__global__ void __launch_bounds__(kNumThreads, 1)
  ...
  auto const num_sms = static_cast<int>(gridDim.x);
  auto const thread_id = static_cast<int>(threadIdx.x);
  auto const sm_id = static_cast<int>(blockIdx.x), lane_id = get_lane_id();
  auto const num_channels = num_sms / 2;
  bool const is_sender = sm_id % 2 == 0;
  int const responsible_channel = sm_id / 2;
  EP_DEVICE_ASSERT(num_topk <= WARP_SIZE);

  constexpr int kDtypePerInt4 = sizeof(int4) / sizeof(dtype_t);
  int hidden_int4 = hidden * sizeof(dtype_t) / sizeof(int4);
  auto x_int4 = reinterpret_cast<int4 const*>(x);
  auto bias_0_int4 = reinterpret_cast<int4 const*>(bias_0);
  auto bias_1_int4 = reinterpret_cast<int4 const*>(bias_1);
  ...
  if (is_sender) {
    ...
    constexpr int num_send_warps_per_rank =
        (kNumThreads / WARP_SIZE) / kNumRanks;
    constexpr int num_send_warps = num_send_warps_per_rank * kNumRanks;
    auto const num_threads_per_rank = num_send_warps_per_rank * WARP_SIZE;
    auto const send_thread_id = thread_id;
    auto const send_warp_id = send_thread_id / WARP_SIZE;
    auto const send_rank_id = (responsible_channel + send_warp_id) % kNumRanks;
    auto const send_warp_id_in_rank = send_warp_id / kNumRanks;
    EP_STATIC_ASSERT(num_send_warps * WARP_SIZE == kNumThreads,
                     "Invalid warp count");

    // Calculate pointers by the specific layout
    auto ptr = reinterpret_cast<void*>(
        static_cast<int8_t*>(buffer_ptrs[send_rank_id]));
    auto num_channels_total = num_channels * kNumRanks;
...
#pragma unroll
    ...
    constexpr int num_recv_warps = kNumThreads / WARP_SIZE;
    auto const recv_warp_id = thread_id / WARP_SIZE;
    EP_DEVICE_ASSERT(kNumRanks <= WARP_SIZE and kNumThreads > WARP_SIZE);
    EP_DEVICE_ASSERT(thread_id >= 0 and kNumThreads % WARP_SIZE == 0);
...
#pragma unroll
      for (int i = 0; i < kNumRanks; ++i) {
        ...
        auto ptr =
            reinterpret_cast<void*>(static_cast<int8_t*>(buffer_ptrs[rank]) +
      ...
      for (int64_t token_idx = token_start_idx + recv_warp_id - 1;
           token_idx < token_end_idx; token_idx += num_recv_warps - 1) {
        // Read expected head
        int expected_head = -1;
        ...
        auto start_time = clock64();
        while (
            __any_sync(WARP_MASK, channel_tail_idx[lane_id] <= expected_head and
                                      expected_head >= 0)) {
          // Timeout check
          if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
            printf(
...
#pragma unroll
        ...
        EP_STATIC_ASSERT(
            kNumStages * WARP_SIZE * sizeof(int4) <= kNumTMABytesPerWarp,
            "Invalid count");
#endif
#pragma unroll
...
#pragma unroll
          ...
          reinterpret_cast<int4*>(
              tma_buffer)[tma_stage_idx * WARP_SIZE + lane_id] = out_int4;

          // Issue TMA
          tma_store_fence();
          __syncwarp();
          if (lane_id == 0) {
            auto tma_bytes = min(WARP_SIZE, hidden_int4 - i) *
                             static_cast<int>(sizeof(int4));
            tma_store_1d(
                reinterpret_cast<int4*>(tma_buffer) + tma_stage_idx * WARP_SIZE,
...
#define COMBINE_LAUNCH_CASE(dtype, ranks)                                     \
  {                                                                           \
    ...
    LAUNCH_KERNEL(&cfg, kernel, reinterpret_cast<dtype*>(recv_x),             \
                  ...
                  topk_weights, reinterpret_cast<dtype const*>(bias_0),       \
                  reinterpret_cast<dtype const*>(bias_1), src_idx,            \
...
#define COMBINE_DTYPE_LAUNCH_CASE(dtype)               \
  ...
  // Even-numbered blocks for sending, odd-numbered blocks for receiving
  EP_HOST_ASSERT(num_sms % 2 == 0);
  EP_HOST_ASSERT(kNumThreads >= num_ranks * WARP_SIZE);
  SETUP_LAUNCH_CONFIG(num_sms, kNumThreads, stream);
