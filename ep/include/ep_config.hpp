...
struct Config {
  ...
  size_t get_rdma_buffer_size_hint(int num_max_dispatch_tokens_per_rank, int64_t hidden_bytes, int num_ranks, bool decoupled_mode, bool return_recv_hook) const {
    ...
    num_bytes += num_channels * num_rdma_ranks *
                 num_slots_per_rdma_chunk * hidden_bytes * 2;
    num_bytes += num_channels * num_rdma_ranks *
                 num_slots_per_rdma_chunk *
                 uccl::internode::get_source_meta_bytes() * 2;
    num_bytes += num_channels * num_rdma_ranks *
                 num_slots_per_rdma_chunk * kNumMaxTopK *
                 sizeof(int64_t) * 2;
    num_bytes += num_channels * num_rdma_ranks *
                 num_slots_per_rdma_chunk * kNumMaxTopK *
                 sizeof(float) * 2;
    num_bytes += num_channels * num_rdma_ranks *
                 num_slots_per_rdma_chunk * kNumMaxScales *
                 sizeof(float) * 2;
    num_bytes += num_channels * num_rdma_ranks *
                 num_slots_per_rdma_chunk * sizeof(int4) * 2;
    num_bytes = ((num_bytes + 127) / 128) * 128;
    return num_bytes;
#else
