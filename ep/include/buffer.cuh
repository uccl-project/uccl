...
struct Buffer {
 ...
 public:
  ...
  __device__ __forceinline__ Buffer(void*& gbl_ptr, int num_elems,
                                    int offset = 0) {
    total_bytes = static_cast<uint64_t>(num_elems) * sizeof(dtype_t);
    ptr = reinterpret_cast<uint8_t*>(gbl_ptr) +
          static_cast<size_t>(offset) * sizeof(dtype_t);
    gbl_ptr = reinterpret_cast<uint8_t*>(gbl_ptr) +
...
struct AsymBuffer {
 ...
 public:
  ...
  __device__ __forceinline__ AsymBuffer(void*& gbl_ptr, int num_elems,
                                        int num_ranks, int sm_id = 0,
                                        int num_sms = 1, int offset = 0) {
    EP_STATIC_ASSERT(kNumRanks == 1, "");
    num_bytes = static_cast<uint64_t>(num_elems) * sizeof(dtype_t);

    uint64_t per_channel_bytes = num_bytes * num_ranks;
    total_bytes = per_channel_bytes * num_sms;
    ptrs[0] = reinterpret_cast<uint8_t*>(gbl_ptr) +
              static_cast<size_t>(per_channel_bytes * sm_id +
                                  num_bytes * offset);
    gbl_ptr = reinterpret_cast<uint8_t*>(gbl_ptr) +
              static_cast<size_t>(total_bytes);
  }
  ...
  __device__ __forceinline__ AsymBuffer(void** gbl_ptrs, int num_elems,
    ...
    for (int i = 0; i < kNumRanks; ++i) {
      ptrs[i] = reinterpret_cast<uint8_t*>(gbl_ptrs[i]) +
                static_cast<size_t>(per_channel_bytes * sm_id +
                                    num_bytes * offset);
      gbl_ptrs[i] = reinterpret_cast<uint8_t*>(gbl_ptrs[i]) +
                    static_cast<size_t>(total_bytes);
    }
  }
...
#pragma unroll
    for (int i = 0; i < kNumRanks; ++i)
      ptrs[i] = ptrs[i] + static_cast<int64_t>(shift) * sizeof(dtype_t);
  }
  ...
  __device__ __forceinline__ dtype_t* buffer(int idx = 0) {
    ...
    return reinterpret_cast<dtype_t*>(ptrs[0] +
                                      static_cast<size_t>(num_bytes * idx));
  }

  __device__ __forceinline__ dtype_t* buffer_by(int rank_idx, int idx = 0) {
    EP_STATIC_ASSERT(kNumRanks > 1,
                     "`buffer` is only available for single rank case");
    return reinterpret_cast<dtype_t*>(ptrs[rank_idx] +
                                      static_cast<size_t>(num_bytes * idx));
  }
};
...
struct SymBuffer {
 private:
  // recv_ptr only participates when decoupling send/recv paths
  uint8_t* send_ptr;
  uint8_t* recv_ptr;
  uint64_t num_bytes;
 ...
 public:
  ...
  __device__ __forceinline__ SymBuffer(void*& gbl_ptr, int num_elems,
                                       int num_ranks, int sm_id = 0,
                                       int num_sms = 1) {
    num_bytes = static_cast<uint64_t>(num_elems) * sizeof(dtype_t);
    ...
    send_ptr = reinterpret_cast<uint8_t*>(gbl_ptr) +
               static_cast<size_t>(per_channel_bytes * sm_id);
    recv_ptr = reinterpret_cast<uint8_t*>(gbl_ptr) +
               static_cast<size_t>(per_channel_bytes * (sm_id + num_sms));
    gbl_ptr = reinterpret_cast<uint8_t*>(gbl_ptr) +
              static_cast<size_t>(total_bytes);
  }
  ...
  __device__ __forceinline__ dtype_t* send_buffer(int idx = 0) {
    ...
    return reinterpret_cast<dtype_t*>(send_ptr +
                                      static_cast<size_t>(num_bytes * idx));
  }
  ...
  __device__ __forceinline__ dtype_t* recv_buffer(int idx = 0) {
    ...
    return reinterpret_cast<dtype_t*>(recv_ptr +
                                      static_cast<size_t>(num_bytes * idx));
  }
