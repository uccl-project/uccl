...
class InternodeLLTester:
    ...
    def _setup_buffer(self):
        ...
        nvlink_buffer_bytes = int(self.config.nvlink_buffer_gb * 1024**3) if not self.config.single_node else 0
        ...
        if rdma_buffer_bytes < min_rdma_size:
            ...
            if self.config.rank == 0:
                print(f"Warning: Adjusted RDMA buffer size to minimum required: {rdma_buffer_bytes / 1024**3:.2f} GB")
        ...
        if self.config.verbose and self.config.rank == 0:
            print(f"Initialized buffer - RDMA: {rdma_buffer_bytes / 1024**3:.2f} GB, "
    ...
    def _generate_test_data(self):
        ...
        self.input_x = torch.randn(
            ...
            device=self.device
        ) * 0.1  # Small values to avoid overflow
        ...
        if self.config.verbose and self.config.rank == 0:
            print(f"Generated test data - tokens: {self.config.num_tokens}, "
    ...
    def _generate_expert_assignments(self) -> torch.Tensor:
        ...
        for i in range(self.config.num_tokens):
            if self.config.single_node:
                ...
                available_experts = list(range(local_start, local_start + num_local_experts))
    ...
    def _run_single_iteration(self) -> Tuple[float, float]:
        ...
        recv_data, recv_count, handle, dispatch_event, dispatch_hook = self.buffer.low_latency_dispatch(
            x=self.input_x,
    ...
    def run_performance_test(self):
        ...
        if self.config.rank == 0:
            ...
            print(f"Mode: {'Single-Node (Intra-node only)' if self.config.single_node else 'Multi-Node (Inter + Intra-node)'}")
            print(f"World size: {self.config.world_size}")
            print(f"Tokens: {self.config.num_tokens}, Hidden: {self.config.hidden_size}")
        ...
        for i in range(self.config.num_iterations):
            ...
            if self.config.verbose and self.config.rank == 0 and i % 5 == 0:
                print(f"Iteration {i}: dispatch={dispatch_time*1000:.2f}ms, combine={combine_time*1000:.2f}ms")
    ...
    def _print_performance_results(self, all_dispatch_times: List, all_combine_times: List):
        """Print performance analysis"""
        print(f"\n=== Performance Results ===")
        
        # Calculate statistics
        avg_dispatch = sum(sum(times) for times in all_dispatch_times) / (len(all_dispatch_times) * len(all_dispatch_times[0]))
        avg_combine = sum(sum(times) for times in all_combine_times) / (len(all_combine_times) * len(all_combine_times[0]))
        ...
        total_data_bytes = self.config.num_tokens * self.config.hidden_size * 2  # bfloat16
        ...
        for rank in range(self.config.world_size):
            rank_dispatch_avg = sum(all_dispatch_times[rank]) / len(all_dispatch_times[rank])
            rank_combine_avg = sum(all_combine_times[rank]) / len(all_combine_times[rank])
            print(f"  Rank {rank}: dispatch={rank_dispatch_avg*1000:.2f}ms, combine={rank_combine_avg*1000:.2f}ms")
        ...
        if self.config.single_node:
            ...
            print(f"  - {self.config.num_nodes} nodes with {self.config.gpus_per_node} GPUs each")
    ...
    def run_correctness_test(self):
        ...
        num_local_experts = self.config.num_experts // self.config.world_size
        expected_shape = (num_local_experts, self.config.max_tokens_per_rank * self.config.world_size, self.config.hidden_size)
        ...
        assert recv_count.shape == (num_local_experts,), f"Count shape mismatch: {recv_count.shape}"
        ...
        if self.config.rank == 0:
            ...
            print(f"  - Total tokens received: {total_received} (expected ~{expected_tokens})")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Test internode_ll kernels with mixed workload support")
    ...
    parser.add_argument("--hidden", type=int, default=7168,
                       help="Hidden size (default: 7168)")
    parser.add_argument("--experts", type=int, default=8,
                       help="Number of experts (default: 8)")
    parser.add_argument("--topk", type=int, default=2,
                       help="Top-K value (default: 2)")
    ...
    parser.add_argument("--logfmt", action="store_true",
                       help="Use LogFMT compression")
    parser.add_argument("--zero-copy", action="store_true",
                       help="Use zero-copy mode")
    ...
    # Buffer sizes
    parser.add_argument("--rdma-buffer-gb", type=float, default=2.0,
                       help="RDMA buffer size in GB (default: 2.0)")
    parser.add_argument("--nvlink-buffer-gb", type=float, default=1.0,
...
def main():
    ...
    if config.num_experts % config.world_size != 0:
        print(f"Error: Number of experts ({config.num_experts}) must be divisible by world size ({config.world_size})")
        sys.exit(1)
